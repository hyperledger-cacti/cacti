...
update_import_paths() {
  ...
  # Excluding go.mod and Makefile, that will be update later on, else generating go.sum will be having issues.
  ...
  local proto_files=$(find "$ROOT_DIR/weaver/common" -type f -name "*.proto" 2>/dev/null)
  go_files="$go_files $proto_files"
