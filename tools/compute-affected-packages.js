...
function detectChangedPackages(packages) {
  ...
  const codeWasModified = realCodeChangeRegex.test(diff);

  if (!codeWasModified && onlyDocsFiles) {
    console.warn("Only documentation + comment/whitespace changes detected.");
    return [];
  }
