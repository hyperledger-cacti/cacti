...
async function TLSSetup(client, enrollmentID, secret) {
  ...
  const caService = new classFabricCAService(
    config.cartradeInfo.fabric.ca.URL,
    tlsOptions,
    config.cartradeInfo.fabric.ca.name
  );
  const req = {
...
async function setupChannel(channelName) {
  ...
  await TLSSetup(
    client,
    config.cartradeInfo.fabric.submitter.name,
    config.cartradeInfo.fabric.submitter.secret
  );
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const { proposal, txId } = channel.generateUnsignedProposal(
        ...
        config.cartradeInfo.fabric.mspID,
        certPem
      );
