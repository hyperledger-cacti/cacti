...
export function makeRawTransaction(txParam: {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const provider = new libWeb3.providers.HttpProvider(
        config.cartradeInfo.ethereum.gethURL
      );
      ...
      // web3_v1.2.9_support
      web3.eth.getTransactionCount(txParam.fromAddress).then((_nonce) => {
        ...
        logger.debug(
          `####makeRawTransaction(): fromAddress: ${
            txParam.fromAddress
          }, txnCount: ${web3.utils.toHex(
            txnCount
          )}, latestNonce: ${web3.utils.toHex(latestNonce)}`
        );
        if (txnCount <= latestNonce) {
          txnCount = latestNonce + 1;
          logger.debug(
            `####makeRawTransaction(): Adjust txnCount, fromAddress: ${
              txParam.fromAddress
            }, txnCount: ${web3.utils.toHex(
              txnCount
            )}, latestNonce: ${web3.utils.toHex(latestNonce)}`
        ...
        logger.debug(
          `#####(B) _nonce: ${_nonce}, latestNonce: ${latestNonce}, txnCount: ${txnCount}`
        );
