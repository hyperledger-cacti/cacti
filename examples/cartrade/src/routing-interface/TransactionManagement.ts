...
export class TransactionManagement {
    ...
    startBusinessLogic(requestInfo: RequestInfo): string {
        ...
        // object judgment
        if (requestInfo.businessLogicID === "guks32pf") {
            if (this.businessLogicPlugin == null) {
                ...
                try {
                    const blp = getTargetBLPInstance(requestInfo.businessLogicID);
                    if (blp === null) {
                        logger.warn(`##WARN: not found BusinessLogicPlugin. businessLogicID: ${requestInfo.businessLogicID}`);
                        return;
                    }
                    ...
                    // TODO: for debug
                    // Save Instance
                    // logger.debug(`##(F)businessLogicID: ${requestInfo.businessLogicID}`);
                    // this.blpMap.set(requestInfo.businessLogicID, this.businessLogicPlugin);
                }
                catch (err) {
            ...
            this.businessLogicPlugin.startTransaction(requestInfo);
