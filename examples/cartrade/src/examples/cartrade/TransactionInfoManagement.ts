...
import { TradeInfo } from '../../routing-interface/TradeInfo';
...
export class TransactionInfoManagement {
    ...
    fileDump() {
        ...
        const arrayData: TransactionInfo[] = JSON.parse(confirmData).table as TransactionInfo[];
    ...
    addTransactionInfo(transactionInfo: TransactionInfo) {
        ...
        const transactionInfoFileData: string = fs.readFileSync(this.fileName, 'utf8');
        transactionInfoTable = JSON.parse(transactionInfoFileData);
        transactionInfoTable.table.push(transactionInfoJson);
        const transactionInfoTableJson: string = JSON.stringify(transactionInfoTable);
