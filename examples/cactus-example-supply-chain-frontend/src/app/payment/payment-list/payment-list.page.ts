...
export class PaymentListPage implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    try {
      // First check if wallet is connected - don't try to auto-connect
      ...
      this.walletService.walletAddress$.subscribe((address) => {
        ...
        if (this.walletConnected && !this.signedHeaders) {
          this.initializeApiAndLoadData().catch((err) => {
            this.log.error("Failed to initialize API in subscription:", err);
          });
        }
      });

      // If wallet is already connected, initialize API
  ...
  async ionViewDidEnter(): Promise<void> {
    try {
      ...
      this.log.error("Error in ionViewDidEnter:", error);
    }
  }
  ...
  async loadPayments(): Promise<void> {
    if (!this.walletConnected) {
      this.log.warn("Wallet not connected, cannot load payments");
      this.errorMessage = "Please connect your wallet to view payments";
      return;
    }
