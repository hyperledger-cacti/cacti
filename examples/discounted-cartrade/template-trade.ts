...
router.post('/execSyncFunction', (req: Request, res: Response, next: NextFunction) => {
  try {

    const functionName = "execSyncFunction";
    logger.info(`#####[${moduleName}], functionName: ${functionName}`);

    templateTradeManagement.execTemplateTrade(functionName, req).then(result => {
        logger.debug(`#####[moduleName]`);
        logger.debug("result(execTemplateTrade) = " + JSON.stringify(result));
        res.status(200).json(result);
    }).catch((err) => {
...
router.post('/sendSignedTransaction', (req: Request, res: Response, next: NextFunction) => {
  try {

    const functionName = "sendSignedTransaction";
    logger.info(`#####[${moduleName}], functionName: ${functionName}`);

    const tradeID: string = templateTradeManagement.execTemplateTradeAsync(functionName, req);
