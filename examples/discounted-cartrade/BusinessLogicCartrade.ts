...
export class BusinessLogicCartrade extends BusinessLogicBase {
    ...
    async Pricing(requestInfo: RequestInfo) {
        ...
        // price list
        let priceList = {default: "50", employee: "25"};

        logger.debug(JSON.stringify(requestInfo.tradeInfo.proofJson));
        let isPreferredCustomer = await this.isPreferredCustomer(requestInfo.tradeInfo.proofJson);
    ...
    async isPreferredCustomer(input_obj) {
        ...
        try {
            ...
            logger.error('Error while reading proof and proof request. returning false.');
            return false;
        }
        ...
        let [schemasJson, credDefsJson, revRegDefsJson, revRegsJson] = await this.verifierGetEntitiesFromLedger(null, proofJson['identifiers']);
    
        assert('Permanent' === proofJson['requested_proof']['revealed_attrs']['attr1_referent']['raw']);

        logger.debug(`##isPreferredCustomer verifierGetEntitiesFromLedger schemasJson : ${JSON.stringify(schemasJson)}`);
        logger.debug(`##isPreferredCustomer verifierGetEntitiesFromLedger credDefsJson : ${JSON.stringify(credDefsJson)}`);
        logger.debug(`##isPreferredCustomer verifierGetEntitiesFromLedger revRegDefsJson : ${JSON.stringify(revRegDefsJson)}`);
        logger.debug(`##isPreferredCustomer verifierGetEntitiesFromLedger revRegsJson : ${JSON.stringify(revRegsJson)}`);
    
        try {
            const verif_result = await indy.verifierVerifyProof(proofRequestJson, proofJson, schemasJson, credDefsJson, revRegDefsJson, revRegsJson);
            logger.debug('verify proof: ' + (verif_result? 'ok' : 'not ok'));
            return verif_result;
        }
        catch (err) {
            logger.error('error raised during indy.verifierVerifyProof() invocation. defaulting to false (= NO DISCOUNT)');
            logger.error(err);
            return false;
        }
    }
    ...
    async verifierGetEntitiesFromLedger(did, identifiers) {
        ...
        for(let referent of Object.keys(identifiers)) {
            ...
            let responseSchema = await getDataFromIndy(args_request_getSchema, identifierSchema);
            let [receivedSchemaId, receivedSchema] = responseSchema["data"];
            schemas[receivedSchemaId] = JSON.parse(receivedSchema);
    
            let args_request_getCredDef = {"did": did, "schemaId": item['cred_def_id']};
            let responseCredDef = await getDataFromIndy(args_request_getCredDef, identifierCredDef);
            let [receivedCredDefId, receivedCredDef] = responseCredDef["data"];
