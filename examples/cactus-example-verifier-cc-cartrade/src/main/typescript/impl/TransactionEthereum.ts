...
import { TransactionSigner } from "@hyperledger/cactus-plugin-verifier-cc";
...
let xVerifierEthereum: VerifierBase | null = null;
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      getNewNonce(txParam.fromAddress).then((result) => {
        ...
        const resp: { data: any; txId: string } = {
          data: { serializedTx: signedTx["serializedTx"] },
          txId: signedTx["txId"] as string,
        };
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (xVerifierEthereum === null) {
        ...
        const ledgerPluginInfo: string = xConnectInfo.getLegerPluginInfo(
          "84jUisrs",
        );
      ...
      xVerifierEthereum
        ...
        .then((result) => {
          ...
          if (txnCount <= latestNonce) {
            ...
            if (!xVerifierEthereum) {
              throw new Error(
                "TransactionEthereum#getNewNonce() xVerifierEthereum cannot be falsy.",
              );
            }
            xVerifierEthereum
              .execSyncFunction(contract, method, template, args)
...
function getLatestNonce(fromAddress: string): number {
  if (mapFromAddressNonce.has(fromAddress)) {
    return mapFromAddressNonce.get(fromAddress) as number;
  }
  //return 0;
  return -1;
}
