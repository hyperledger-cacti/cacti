...
import { ApiInfo, RequestedData } from "./ledger-plugin";

// for debug
export function json2str(jsonObj: any) {
  try {
...
export function deleteAndDisconnectSocket(index: number) {
  try {
    console.log(`##deleteAndDisconnectSocket, index = ${index}`);
    ...
    console.log(`##error:deleteAndDisconnectSocket, index = ${index}, ${err}`);
  }
}
