...
function _preventMalleability(sig, curveParams) {
  ...
  if (!halfOrder) {
    throw new Error(
      'Can not find the half order needed to calculate "s" value for non-malleable signatures. Unsupported curve name: ' +
        curveParams.name,
    );
  }
...
async function setupChannel(channelName) {
  ...
  // TODO: channel.initialize() should not require an signing identity
