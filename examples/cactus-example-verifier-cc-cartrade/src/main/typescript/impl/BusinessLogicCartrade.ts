...
import { TradeInfo } from "@hyperledger/cactus-plugin-verifier-cc";
...
import { transactionManagement } from "@hyperledger/cactus-plugin-verifier-cc";
import { verifierFactory } from "@hyperledger/cactus-plugin-verifier-cc";
...
import { LedgerEvent } from "@hyperledger/cactus-plugin-verifier-cc";
...
export class BusinessLogicCartrade implements BusinessLogicBase {
  ...
  public setConfig(data: []): Record<string, unknown> {
    return { data };
  }
  ...
  dummyTransaction(tradeInfo: TradeInfo) {
  ...
  firstTransaction(requestInfo: RequestInfo, tradeInfo: TradeInfo) {
    ...
    const txParam: {
      ...
      fromAddress: requestInfo.tradeInfo.ethereumAccountFrom as string,
    ...
    makeRawTransaction(txParam)
      .then((result: any) => {
        ...
        verifierEthereum.sendSignedTransaction(
          contract,
          method,
          template,
          args,
  ...
  onEvent(ledgerEvent: LedgerEvent, targetIndex: number): void {
    ...
    logger.debug(`##onEvent(): ${JSON.stringify(ledgerEvent)}`);

    switch (ledgerEvent.verifierId) {
      case config.cartradeInfo.ethereum.validatorID:
        this.onEvenEthereum(ledgerEvent.data, targetIndex);
        break;
  ...
  onEvenEthereum(event: any, targetIndex: number): void {
    logger.debug(`##in onEvenEthereum()`);
    const tx = this.getTransactionFromEthereumEvent(event, targetIndex);
    if (tx == null) {
      logger.error(
        `##onEvenEthereum(): invalid event: ${JSON.stringify(event)}`,
      );
      return;
    }
    ...
    try {
      ...
      if (status !== 200) {
        logger.error(
          `##onEvenEthereum(): error event, status: ${status}, txId: ${txId}`,
        );
        return;
      }
      ...
      logger.error(
        `##onEvenEthereum(): err: ${err}, event: ${JSON.stringify(event)}`,
      );
    }
  }

  getTransactionFromEthereumEvent(event: any, targetIndex: number): any {
    try {
  ...
  onEvenFabric(event: any, targetIndex: number): void {
    logger.debug(`##in onEvenFabric()`);
    const tx = this.getTransactionFromFabricEvent(event, targetIndex);
    if (tx == null) {
      logger.warn(`##onEvenFabric(): invalid event: ${JSON.stringify(event)}`);
      return;
    }
    ...
    try {
      ...
      logger.error(
        `##onEvenFabric(): onEvent, err: ${err}, event: ${JSON.stringify(
          event,
        )}`,
      );
    }
  ...
  getTransactionFromFabricEvent(event: any, targetIndex: any): any {
    try {
  ...
  executeNextTransaction(txInfo: any, txId: string): void {
    ...
    try {
      ...
      if (transactionInfo != null) {
        logger.debug(
          `##onEvent(A), transactionInfo: ${JSON.stringify(transactionInfo)}`,
        );
      } else {
      ...
      switch (txStatus) {
        ...
        case CartradeStatus.Completed:
          logger.warn(
            "##WARN: already completed, tx info: ${JSON.stringify(transactionInfo)}",
          );
          return;
        default:
  ...
  getOperationStatus(tradeID: string): Record<string, unknown> {
    ...
    const transactionStatusData = businessLogicInquireCartradeStatus.getCartradeOperationStatus(
      tradeID,
    );
  ...
  getTxIDFromEvent(
    ...
    logger.debug(`##event: ${JSON.stringify(ledgerEvent)}`);

    switch (ledgerEvent.verifierId) {
      case config.cartradeInfo.ethereum.validatorID:
        return this.getTxIDFromEventEthereum(ledgerEvent.data, targetIndex);
      case config.cartradeInfo.fabric.validatorID:
  ...
  getTxIDFromEventEthereum(event: any, targetIndex: number): string | null {
    logger.debug(`##in getTxIDFromEventEthereum()`);
    const tx = this.getTransactionFromEthereumEvent(event, targetIndex);
    if (tx == null) {
      logger.warn(`#getTxIDFromEventEthereum(): skip(not found tx)`);
      return null;
    }
    ...
    try {
      ...
      if (typeof txId !== "string") {
        logger.warn(
          `#getTxIDFromEventEthereum(): skip(invalid block, not found txId.), event: ${JSON.stringify(
            event,
          )}`,
        );
        return null;
      ...
      logger.debug(`###getTxIDFromEventEthereum(): txId: ${txId}`);
      return txId;
    } catch (err) {
      logger.error(
        `##getTxIDFromEventEthereum(): err: ${err}, event: ${JSON.stringify(
          event,
        )}`,
      );
      return null;
  ...
  getTxIDFromEventFabric(event: any, targetIndex: number): string | null {
    ...
    try {
      ...
      if (typeof txId !== "string") {
        logger.warn(
          `#getTxIDFromEventFabric(): skip(invalid block, not found txId.), event: ${JSON.stringify(
            event,
          )}`,
        );
        return null;
      ...
      logger.error(
        `##getTxIDFromEventFabric(): err: ${err}, event: ${JSON.stringify(
          event,
        )}`,
      );
      return null;
  ...
  getEventDataNum(ledgerEvent: LedgerEvent): number {
    ...
    const event: any = ledgerEvent.data;
    let retEventNum = 0;
