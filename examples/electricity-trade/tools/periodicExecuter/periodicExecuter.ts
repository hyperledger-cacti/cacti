...
const submitCommand = 'sawtooth batch submit -f batches.intkey --url http://rest-api:8008';
const incCommand = 'intkey create_batch --key-name ' + keyString + ' --value-inc-rand ' + valueAdd;

if (process.argv.length === 6) {
    const valueInitial = process.argv[5];

    const setCommand = 'intkey create_batch --key-name ' + keyString + ' --value-set ' + valueInitial;
