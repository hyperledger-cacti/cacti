...
export class BusinessLogicElectricityTrade extends BusinessLogicBase {
  ...
  getTxIDFromEventEthereum(event: object, targetIndex: number): string | null {
    ...
    try {
      ...
      if (typeof txId !== "string") {
        logger.warn(
          `#getTxIDFromEventEthereum(): skip(invalid block, not found txId.), event: ${json2str(
            event,
          )}`,
        );
        return null;
      ...
      logger.error(
        `##getTxIDFromEventEthereum(): err: ${err}, event: ${json2str(event)}`,
      );
      return null;
    }
  }
