...
describe("CBDC E2E API Scenarios", () => {
  ...
  beforeEach(async () => { 
    ...
    try {
      const aliceFabricBalance = await getBalanceApi.getBalance(ALICE_USER, "FABRIC");
      const aliceBesuBalance = await getBalanceApi.getBalance(ALICE_USER, "BESU");
      const bobFabricBalance = await getBalanceApi.getBalance(BOB_USER, "FABRIC");
      const bobBesuBalance = await getBalanceApi.getBalance(BOB_USER, "BESU");
      
      log.info(`After reset - Alice: Fabric=${aliceFabricBalance.data.amount}, Besu=${aliceBesuBalance.data.amount}`);
      log.info(`After reset - Bob: Fabric=${bobFabricBalance.data.amount}, Besu=${bobBesuBalance.data.amount}`);
    } catch (error) {
  ...
  async function resetUserBalances() {
    ...
    try {
      const aliceFabricResponse = await getBalanceApi.getBalance(ALICE_USER, "FABRIC");
      const aliceBesuResponse = await getBalanceApi.getBalance(ALICE_USER, "BESU");
      ...
      log.info(`Alice's current balances - Fabric: ${aliceFabricBalance}, Besu: ${aliceBesuBalance}`);
      
      if (aliceFabricBalance > 0) {
        log.info(`Attempting to transfer ${aliceFabricBalance} Fabric tokens from Alice to Bob`);
        try {
          ...
          log.info(`Transfer API response data: ${JSON.stringify(transferResponse.data)}`);
          
          if (transferResponse.status === 200) {
            log.info(`Successfully transferred ${aliceFabricBalance} Fabric tokens`);
          } else {
      ...
      if (aliceBesuBalance > 0) {
        log.info(`Attempting to transfer ${aliceBesuBalance} Besu tokens from Alice to Bob`);
        try {
          const transferResponse = await transferApi.transfer({
            from: ALICE_USER,
            to: BOB_USER,
          ...
          if (transferResponse.status === 200) {
            log.info(`Successfully transferred ${aliceBesuBalance} Besu tokens`);
          } else {
            log.warn(`Transfer API returned status ${transferResponse.status}`);
          }
        } catch (error) {
      ...
      if (aliceFabricBalance > 0 || aliceBesuBalance > 0) {
        ...
        const finalFabricResponse = await getBalanceApi.getBalance(ALICE_USER, "FABRIC");
        const finalBesuResponse = await getBalanceApi.getBalance(ALICE_USER, "BESU");
        ...
        log.info(`Alice's final balances after reset - Fabric: ${finalFabricBalance}, Besu: ${finalBesuBalance}`);
        
        if (finalFabricBalance > 0 || finalBesuBalance > 0) {
          try {
            if (finalFabricBalance > 0) {
              await infrastructure.getFabricEnvironment().transferTokensFabric(
                ALICE_USER, 
                BOB_USER, 
            ...
            if (finalBesuBalance > 0) {
              await infrastructure.getBesuEnvironment().transferTokensBesu(
                ALICE_USER, 
                BOB_USER, 
                finalBesuBalance
              );
            ...
            const fallbackFabricResponse = await getBalanceApi.getBalance(ALICE_USER, "FABRIC");
            const fallbackBesuResponse = await getBalanceApi.getBalance(ALICE_USER, "BESU");
            
            log.info(`After fallback - Alice: Fabric=${fallbackFabricResponse.data.amount}, Besu=${fallbackBesuResponse.data.amount}`);
        ...
        log.info("No tokens to reset - Alice already has 0 balance on both chains");
      }
   ...
   */
  async function setupAliceWithCBDC(amount: number = ESCROW_AMOUNT, ledger: "FABRIC" | "BESU" = "FABRIC") {    
    await mintApi.mint({
      user: ALICE_USER,
   ...
   */
  async function createAssetReference(user: string, amount: number, ledger: "FABRIC" | "BESU" = "BESU") {    
    ...
    const approvedResponse = await getAmountApprovedApi.getAmountApproved(user, ledger);
    const approvedAmount = parseInt(approvedResponse.data);
    expect(approvedAmount).toBe(amount);
    
    log.info(`Asset reference created: ${user} approved ${approvedAmount} CBDC on ${ledger}`);
   ...
   */
  async function lockAssetReference(locker: string, recipient: string, amount: number, sourceChain: "FABRIC" | "BESU", receiverChain: "FABRIC" | "BESU") {    
    try {
      ...
      log.info(`Asset reference locked by ${locker} for transfer to ${recipient}`);
  ...
  test("Alice successfully escrows CBDC", async () => {    
    ...
    const aliceInitialBalance = parseInt(aliceInitialBalanceResponse.data.amount);
    ...
    const approvedAmountResponse = await getAmountApprovedApi.getAmountApproved(
      ALICE_USER, 
      "FABRIC"
    );
    ...
    if (bridgeResponse.data && typeof bridgeResponse.data === 'object') {
      if ('statusResponse' in bridgeResponse.data && bridgeResponse.data.statusResponse) {
    ...
    log.info(`Bridge transaction initiated Session ID: ${sessionId || 'Not provided'}`);
    ...
    while (!transactionCompleted) {
      ...
      try {
        const fabricSessionsResponse = await getSessionsReferencesApi.getSessionsReferences("FABRIC");
        const besuSessionsResponse = await getSessionsReferencesApi.getSessionsReferences("BESU");
        const allSessions = [...fabricSessionsResponse.data, ...besuSessionsResponse.data];
        const completedSession = allSessions.find(session => 
          session.status === "DONE" || 
          session.status === "COMPLETED"
        );
        if (completedSession) {
          transactionCompleted = true;
          log.info(`Transaction completed via session status! Session: ${JSON.stringify(completedSession)}`);
          break;
        }
        if (attempts % 5 === 0) {
          try {
            const currentAliceBalance = await getBalanceApi.getBalance(ALICE_USER, "FABRIC");
            const currentAliceBalanceValue = parseInt(currentAliceBalance.data.amount);            
            if (currentAliceBalanceValue === 0) {
              transactionCompleted = true;
              break;
            }
          } catch (balanceError) {
            log.warn(`Error checking balance during monitoring: ${balanceError}`);
          }
        }
      } catch (error) {
        log.warn(`Error checking session status (attempt ${attempts}): ${error}`);
      }
    }
    ...
    expect(aliceFinalBalance).toBeLessThanOrEqual(aliceInitialBalance - ESCROW_AMOUNT);
    ...
    const finalFabricSessions = await getSessionsReferencesApi.getSessionsReferences("FABRIC");
    const finalBesuSessions = await getSessionsReferencesApi.getSessionsReferences("BESU");
    
    log.info(`Final Fabric sessions: ${JSON.stringify(finalFabricSessions.data)}`);
    log.info(`Final Besu sessions: ${JSON.stringify(finalBesuSessions.data)}`);
  }, TIMEOUT);
    
  test("API validation - Invalid amounts should be rejected", async () => {
    try {
      ...
      if (response.status === 200) {
        log.info("API accepted negative amount request - checking if business logic handles it correctly");
        
        const balanceResponse = await getBalanceApi.getBalance(ALICE_USER, "FABRIC");
        const balance = parseInt(balanceResponse.data.amount);
  ...
  test("Alice successfully creates an asset reference in the Besu network", async () => {
    await setupAliceWithCBDC(ESCROW_AMOUNT, "BESU");

    const approvedAmount = await createAssetReference(ALICE_USER, ESCROW_AMOUNT, "BESU");
    expect(approvedAmount).toBe(ESCROW_AMOUNT);
    
    const finalApprovedResponse = await getAmountApprovedApi.getAmountApproved(ALICE_USER, "BESU");
    const finalApprovedAmount = parseInt(finalApprovedResponse.data);
    expect(finalApprovedAmount).toBe(ESCROW_AMOUNT);
    
    log.info(`Asset reference with conceptual ID ${ASSET_REFERENCE_ID} created successfully`);
    log.info(`Asset reference details: ${ESCROW_AMOUNT} CBDC approved for Alice`);
  }, TIMEOUT);

  test("Bob successfully locks an asset reference in the Besu network", async () => {
    await setupAliceWithCBDC(ESCROW_AMOUNT, "BESU");
    await createAssetReference(ALICE_USER, ESCROW_AMOUNT, "BESU");
    
    const lockResult = await lockAssetReference(ALICE_USER, BOB_USER, ESCROW_AMOUNT, "BESU", "FABRIC");
    expect(lockResult.success).toBe(true);
    expect(lockResult.response?.status).toBe(200);
    
    log.info(`Asset reference with ID ${ASSET_REFERENCE_ID} is locked for bridge transaction`);
    log.info("Lock initiated by Alice for transfer to Bob");
  }, TIMEOUT);

  test("Bob fails to lock an already locked asset reference", async () => {
    await setupAliceWithCBDC(ESCROW_AMOUNT, "BESU");
    await createAssetReference(ALICE_USER, ESCROW_AMOUNT, "BESU");

    const bobLockResult = await lockAssetReference(ALICE_USER, BOB_USER, ESCROW_AMOUNT, "BESU", "FABRIC");
    ...
    const approvedAmount = await getAmountApprovedApi.getAmountApproved(ALICE_USER, "BESU");
    const currentApproved = parseInt(approvedAmount.data);
    
    expect(currentBalance === 0 || currentApproved === 0).toBe(true);
    
    log.info(`Asset reference with ID ${ASSET_REFERENCE_ID} is properly locked`);
  }, TIMEOUT);

  test("Bob successfully deletes an asset reference", async () => {
    await setupAliceWithCBDC(ESCROW_AMOUNT, "BESU");
    await createAssetReference(ALICE_USER, ESCROW_AMOUNT, "BESU");
    const lockResult = await lockAssetReference(ALICE_USER, BOB_USER, ESCROW_AMOUNT, "BESU", "FABRIC");
    expect(lockResult.success).toBe(true);
    ...
    while (!transactionCompleted) {
      ...
      try {
        ...
        if (currentBalance === 0 && bobCurrentBalance >= ESCROW_AMOUNT) {
          transactionCompleted = true;
          log.info("Bridge transaction completed - asset reference effectively deleted");
        }       
      } catch (error) {
        log.warn(`Error checking deletion status: ${error}`);
      }
    }
    const finalAliceBalance = await getBalanceApi.getBalance(ALICE_USER, "BESU");
    const finalApproved = await getAmountApprovedApi.getAmountApproved(ALICE_USER, "BESU");
    expect(parseInt(finalAliceBalance.data.amount)).toBe(0);
    expect(parseInt(finalApproved.data)).toBe(0);

    log.info(`Asset reference with ID ${ASSET_REFERENCE_ID} successfully deleted`);
  }, TIMEOUT);

  test("Bridge entity deletes asset reference and burns tokens", async () => {
    await setupAliceWithCBDC(ESCROW_AMOUNT, "BESU");
    await createAssetReference(ALICE_USER, ESCROW_AMOUNT, "BESU");
    const lockResult = await lockAssetReference(ALICE_USER, BOB_USER, ESCROW_AMOUNT, "BESU", "FABRIC");
    expect(lockResult.success).toBe(true);
    
    const initialBobBesuBalance = await getBalanceApi.getBalance(BOB_USER, "BESU");
    let transactionCompleted = false;
    let attempts = 0;
    while (!transactionCompleted) {
      await new Promise(resolve => setTimeout(resolve, 3000));
      ...
      try {
        ...
        const bobCurrentFabricBalance = parseInt(bobFabricBalance.data.amount);
        
        log.info(`Attempt ${attempts}: Alice Besu: ${aliceCurrentBalance}, Bob Besu: ${bobCurrentBesuBalance}, Bob Fabric: ${bobCurrentFabricBalance}`);
        
        if (aliceCurrentBalance === 0 && bobCurrentFabricBalance >= ESCROW_AMOUNT) {
          transactionCompleted = true;
          log.info("Bridge entity completed asset reference deletion and token transfer");
        }
      } catch (error) {
    ...
    const finalAliceBalance = await getBalanceApi.getBalance(ALICE_USER, "BESU");
    const finalBobBesuBalance = await getBalanceApi.getBalance(BOB_USER, "BESU");
    const finalBobFabricBalance = await getBalanceApi.getBalance(BOB_USER, "FABRIC");
    ...
    expect(aliceFinalBalance).toBe(0);
    expect(bobFinalBesuBalance).toBe(parseInt(initialBobBesuBalance.data.amount));
    expect(bobFinalFabricBalance).toBeGreaterThanOrEqual(ESCROW_AMOUNT);
    
    log.info(`Alice's tokens burned/consumed from Besu sidechain: ${aliceFinalBalance} CBDC`);
    log.info(`Bob received tokens on Fabric mainchain: ${bobFinalFabricBalance} CBDC`);
  }, TIMEOUT);

  test("Error scenario: Cannot create asset reference without sufficient balance", async () => {
    ...
    try {
      ...
      log.info("Correctly rejected asset reference creation with insufficient balance");
      expect(error).toBeDefined();
    }
  }, TIMEOUT);
});
