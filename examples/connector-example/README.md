# Cactus connector-example

## Abstract

Cactus **connector-example** is a sample application that utilizes `cactus-plugin-ledger-connector-besu` in order to interact with the Besu network from a different DLT. 
This example works under the pretense that a project (either a DLT _Nodejs project_ or a _non_-Node project) has been setup and requires a way to interact with another project based on Hyperledger Besu. Users can implement smart contracts between two different blockchains based on connectors [currently avaliable](https://github.com/hyperledger/cactus/blob/main/whitepaper/whitepaper.md#5611-ledger-connector-besu-plugin) within Hyperledger Cactus.

![connector-example image](./images/connector-example.jpg)

## Required software components
- OS: Linux (CentOS7 or Ubuntu 20.07)
- Docker (recommend: v17.06.2-ce or greater)
- Docker-compose (recommend: v1.14.0 or greater)
- node.js v12 (recommend: v12.20.2 or greater)

## Prerequisites for `Node` projects

Within the root of the project, run the following command to install
dependencies: 
```
`npm run configure`
```

Within the project root folder, run the following command to compile the plugin and create the dist directory: 
```
`npm run tsc` 
```

## Usage within the your project 

Listed below are the steps to use `Hyperledger Cactus`'s connectors within your project. 
- The first way (indicated by `Node:` ) covers a NodeJS project that wants to use `Hyperledger Cactus`'s connectors. 
- The second way covers a _non_-NodeJS project that wants to use `Hyperledger Cactus`'s connectors. 


## For Node Projects
### 1) `Node:` Installing the module

To use `cactus-plugin-ledger-connector-besu`, first run this command within the terminal to download the module: 
``` 
npm i @hyperledger/cactus-plugin-ledger-connector-besu
```

_or_ within the root `package.json`'s  `devDependencies`, add: 
``` typescript
"@hyperledger/cactus-plugin-ledger-connector-besu": "version number",
```
and rerun your `configure` script

### 2) `Node:` Importing modules

Add the following import statement within your project (specifically within the file you want to use the connector) to import the public-api 
``` typescript
import { PluginFactoryLedgerConnector } from "@hyperledger/cactus-plugin-ledger-connector-besu"; 
```

## For _non_-Node projects
...
Create a connector by inserting the following code: 
``` typescript 
...
The following are calls that can be made to the plugin API through the `cactus-plugin-ledger-connector-besu`:
``` typescript 
...
## Call Example
The following code is an example of how to invoke smart-contract functions 
that we have deployed on the network: 
...
Implementing different smart-contract functions will require different arguments; however, the process will be similar to the one listed above. 


## Implementing Privacy Features 
Private transactions using Besu are currently enabled. To implement private transactions, the `privateFor` and `privateFrom` must be populated:
...
More information about Besu Private Transactions can be found [here](https://besu.hyperledger.org/en/stable/Concepts/Privacy/Private-Transactions/).

The following is a call example to deploy a private contract: 
``` typescript
