...
const validatorRregistryConf: any = JSON.parse(
  fs.readFileSync(
    path.resolve(__dirname, "../config/validator-registry.json"),
    "utf8",
  ),
);
...
export class VerifierBase implements Verifier {
  ...
  constructor(ledgerInfo: string) {
    ...
    const ledgerInfoObj: any = JSON.parse(ledgerInfo);
  ...
  sendSignedTransaction(
    ...
    logger.debug(
      `call: sendSignedTransaction, contract = ${JSON.stringify(
        contract,
    ...
    if (this.isTemplateFunction("sendSignedTransaction", template)) {
      this.execTemplateFunctionAsync(
        "sendSignedTransaction",
        contract,
        template,
        args,
  ...
  // TODO: Method name confirmation
  requestLedgerOperationNeo(contract: any, method: any, args: any): void {
    logger.debug("call : requestLedgerOperation");
    try {
      const socketOptions = {
        rejectUnauthorized: config.socketOptions.rejectUnauthorized, // temporary avoidance since self-signed certificates are used
  ...
  execSyncFunction(
    ...
    logger.debug(
      `call: execSyncFunction, contract = ${JSON.stringify(
        contract,
      )}, method = ${JSON.stringify(
        method,
      )}, template = ${template}, args = ${JSON.stringify(args)}, `,
    ...
    if (this.isTemplateFunction("execSyncFunction", template)) {
      return this.execTemplateFunction(
        "execSyncFunction",
        contract,
        template,
        args,
  ...
  execSyncFunctionCallVerifier(
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        logger.debug(
          `##in execSyncFunctionCallVerifier, contract = ${JSON.stringify(
            contract,
        ...
        logger.debug(`socketOptions = ${JSON.stringify(socketOptions)}`);
        const socket: Socket = io(this.validatorUrl, socketOptions);
        socket.on("connect_error", (err: Error | null) => {
          logger.error("##connect_error:", err);
          // end communication
          socket.disconnect();
          reject(err);
        });
        socket.on("connect_timeout", (err: Error | null) => {
          logger.error("####Error:", err);
          // end communication
          socket.disconnect();
          reject(err);
        });
        socket.on("error", (err: Error | null) => {
          logger.error("####Error:", err);
          socket.disconnect();
          reject(err);
        });
        socket.on("response", (result: any) => {
          logger.debug("#[recv]response, res: " + JSON.stringify(result));
          if (reqID === result.id) {
            responseFlag = true;
            logger.debug(
              `##execSyncFunctionCallVerifier: resObj: ${JSON.stringify(
                result.resObj,
              )}`,
            );
  ...
  execTemplateFunction(
    ...
    logger.debug(
      `##in execTemplateFunction, functionName: ${functionName}, contract = ${JSON.stringify(
        contract,
      )}, template = ${template}, args = ${JSON.stringify(args)}, `,
    );
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        // method not found.
        logger.warn(
          `##Warning: execTemplateFunction: template not found, template: ${template}`,
        );
        resolve({
          status: 400,
          msg: `BAD REQUEST(template not found, template: ${template})`,
        });
      } catch (err) {
  ...
  execTemplateFunctionAsync(
    ...
    logger.debug(
      `##in execTemplateFunctionAsync, functionName: ${functionName}, contract = ${JSON.stringify(
        contract,
      )}, template = ${template}, args = ${JSON.stringify(args)}, `,
    );
    ...
    for (const rec of validatorRregistryConf[functionName]) {
      ...
      if (template === rec["template"]) {
        ...
        logger.debug(
          `##execTemplateFunctionAsync: template: ${template}, data: ${dataValue}`,
        );
        return;
      }
    }
    ...
    // method not found.
    logger.warn(
      `##Warning: execTemplateFunctionAsync: template not found, template: ${template}`,
    );
  }

  isTemplateFunction(key: string, template: string): boolean {
    logger.debug(
      `##call : isTemplateFunction, key: ${key}, template: ${template}`,
    );
      ...
      try {
        ...
        httpReq.onload = function () {
          try {
            ...
            const event = VerifierBase.makeOpenApiEvent(
              responseObj,
              validatorID,
            );
            ...
            if (!eventListener) {
              throw new Error(
                "VerifierBase#requestLedgerOperationHttp() eventListener was null.",
              );
            }
  ...
  static makeOpenApiEvent(resp: any, validatorID: string): LedgerEvent {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        socket.on("eventReceived", (res: any) => {
          ...
          logger.debug("#[recv]eventReceived, res: " + JSON.stringify(res));

          logger.debug(
            `##set eventListener: ${this.eventListener}, ${this.constructor.name}, ${this.validatorID}`,
          );
          const eventListener = this.eventListener;
          ...
          if (eventListener != null) {
            ...
            VerifierAuthentication.verify(this.validatorKeyPath, res.blockData)
              .then((decodedData) => {
                logger.debug(`decodedData = ${JSON.stringify(decodedData)}`);
                const resultObj = {
                  status: res.status,
