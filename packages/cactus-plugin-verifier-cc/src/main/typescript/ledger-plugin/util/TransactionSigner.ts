...
const configVerifier: any = JSON.parse(
  fs.readFileSync(
    path.resolve(__dirname, "../../config/verifier-config.json"),
    "utf8",
  ),
);
...
export class TransactionSigner {
  ...
  // stand alone fabric-sig package in future.
  static _preventMalleability(sig: any, curveParams: any) {
    const halfOrder: any =
    ...
    if (sig.s.cmp(halfOrder) === 1) {
      ...
      const bigNum: any =
        TransactionSigner.ordersForCurve[curveParams.name].order;
  ...
  static async setupChannel(channelName: string): Promise<any> {
