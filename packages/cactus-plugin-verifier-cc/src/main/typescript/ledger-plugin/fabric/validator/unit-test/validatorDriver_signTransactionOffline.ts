...

{
  ...
  const signProposal = (proposalBytes: any, paramPrivateKeyPem: any) => {
  ...
  const TLSSetup = async (client: any, enrollmentID: any, secret: any) => {
    const tlsOptions = {
      trustedRoots: [],
      verify: false,
    };
  ...
  const setupChannel = async (channelName: any) => {
  ...
  const Invoke = async (reqBody: any) => {
    ...
    let invokeResponse: any; //Return value from chain code
    let channel: any;
