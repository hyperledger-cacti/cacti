...
export async function mainFabricValidator(): Promise<void> {
 ...
 */
 ...
 function normalizePort(val: any) {
  ...
  function onListening(theServer: https.Server) {
    const addr = theServer.address() as AddressInfo;
    ...
    const keyPath = path.join(__dirname, config.sslParam.key);
    const certPath = path.join(__dirname, config.sslParam.cert);
    ...
    io.on('connection', function(client: any) {
         ...
         *       }
        **/
        client.on('request', function(data: any) {
        ...
        client.on('request2', function(data: any) {
            ...
            if ((data.method.type === "evaluateTransaction")
        ...
        **/
        client.on('startMonitor', function() {
            ...
            const cb = function(callbackData: any) {
                let emitType = "";
        ...
        **/
        ...
        client.on('stopMonitor', function(reason: unknown) {
            logger.debug(`Stop reason: ${JSON.stringify(reason)}`);
            Smonitor.stopMonitor(client.id);
        });
    
        client.on('disconnect', function(reason: unknown) {
