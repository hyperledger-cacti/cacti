/**
 * Provides a containerized `solana-test-validator` localnet for the integration
 * tests via {@link SolanaTestLedger} (the same Docker-based pattern the Ethereum
 * connector uses with `GethTestLedger`). Each test file gets its own throwaway
 * container; `startTestValidator()` returns the dynamically-mapped JSON-RPC URL.
 *
