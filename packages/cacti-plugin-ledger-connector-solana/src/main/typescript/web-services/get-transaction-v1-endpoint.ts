...
export class GetTransactionEndpoint implements IWebServiceEndpoint {
  ...
  public get oasPath(): (typeof OAS.paths)["/api/v1/plugins/@hyperledger/cacti-plugin-ledger-connector-solana/get-transaction"] {
    return OAS.paths[
      "/api/v1/plugins/@hyperledger/cacti-plugin-ledger-connector-solana/get-transaction"
    ];
  }
  ...
  public async handleRequest(req: Request, res: Response): Promise<void> {
    ...
    try {
      res.json(await this.options.connector.getTransaction(req.body));
    } catch (ex) {
