...
export async function enrollUser(
  ...
  const userIdentity: any = X509WalletMixin.createIdentity(
    connectionProfile.organizations.Org1.mspid,
    enrollment.certificate,
    enrollment.key.toBytes(),
  );
...
export async function getUserCryptoFromWallet(
  ...
  const submitterIdentity: any = await wallet.export(name);
  const certPem = (submitterIdentity as any).certificate;
  const privateKeyPem = (submitterIdentity as any).privateKey;
  const mspId = (submitterIdentity as any).mspId;
  let certType = "";
  if ((submitterIdentity as any).type == "X509") {
    certType = "X.509";
  }
  (submitterIdentity as any).type;
  return [certPem, privateKeyPem, mspId, certType];
}
