...
export class FabricDriverServer {
  ...
  private signingCredential: FabricSigningCredential;
  ...
  constructor(options: FabricDriverServerOptions) {
    ...
    if (options.monitorSyncPeriod) {
      this.monitorSyncPeriod = options.monitorSyncPeriod;
    }
    ...
    if (options.walletPath) {
      ...
      this.walletPath = path.join(process.cwd(), `wallet-${this.networkName}`);
    }
  ...
  async driverState(request: Query): Promise<Ack> {
    try {
      if (this.mock) {
        ...
        await this.fabricCommunication(
          request,
          this.gatewayOptions,
  ...
  async fabricCommunication(
    ...
    const [result, invokeError] = await handlePromise(
      this.connector.invoke({
        ...
        contractName: parsedAddress.contract,
    ...
    if (invokeError) {
      ...
      this.logger.info(`Result of fabric invoke ${result}`);
  ...
  async spawnSubscribeEventHelper(
    ...
    if (subscriptionOp == EventSubOperation.SUBSCRIBE) {
      this.events.subscribeEventHelper(
        request,
        client,
        this.gatewayOptions,
        this.signingCredential,
      ...
      this.events.unsubscribeEventHelper(
        request,
        client,
        this.gatewayOptions,
        this.signingCredential,
  ...
  async monitorService(): Promise<void> {
    ...
    while (true) {
      await this.listner.monitorBlockForMissedEvents(
        this.gatewayOptions,
        this.signingCredential,
      );
