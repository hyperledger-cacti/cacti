...
class LevelDBConnector implements DBConnector {
  private static readonly className = "LevelDBConnector";
  ...
  constructor(
    ...
    const loggerOptions: ILoggerOptions = {
      level: logLevel,
      label: LevelDBConnector.className,
    };
  ...
  async open(i: number = 0): Promise<boolean> {
    this.logger.debug(`attempt #${i} to open database ${this.DB_NAME}`);
    try {
  ...
  async read(key: any): Promise<any> {
    ...
    try {
      ...
      this.logger.debug(`read() got value: ${JSON.stringify(value)}`);
    } catch (error: any) {
      this.logger.error(
  ...
  async filteredRead(
    ...
    try {
      ...
      this.logger.error(`filteredRead error: ${error.toString()}`);
      if (error.code == "LEVEL_DATABASE_NOT_OPEN") {
        throw new DBNotOpenError(error.toString());
      } else {
        throw new DBError(error.toString());
