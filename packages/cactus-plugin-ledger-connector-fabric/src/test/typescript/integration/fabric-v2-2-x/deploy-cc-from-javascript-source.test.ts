...
const log: Logger = LoggerProvider.getOrCreate({
  label: "deploy-cc-from-javascript-source.test",
  level: logLevel,
});
...
describe("Deploy CC from Javascript Source Test", () => {
  ...
  test(`${testCase}`, async () => {
    ...
    expect(res.status).toBe(200); // Check if res.status is 200
    expect(success).toBeTruthy(); // Check if success is truthy
    const {
      approveForMyOrgList,
      installList,
      queryInstalledList,
    ...
    log.info(`BassicAssetTransfer.Create(): ${JSON.stringify(createRes.data)}`); // Log the createRes data
