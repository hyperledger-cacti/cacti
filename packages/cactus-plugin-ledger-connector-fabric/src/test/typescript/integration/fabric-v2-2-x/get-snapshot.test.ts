...
test(testCase, async (t: Test) => {
  ...
  const ledger = new FabricTestLedgerV1({
    ...
    envVars: new Map([["FABRIC_VERSION", "2.3.0"]]),
    logLevel,
  });
  ...
  const snapshotRequest: getSnapshotRequest = {
    privateData: false,
    channelName: channelName,
    snapshotHeight: "0",
    tlsRootCertFile: org1Env.CORE_PEER_TLS_ROOTCERT_FILE,
    peerAddress: org1Env.CORE_PEER_ADDRESS,
    snapshotPath: `/snapshots`,
  };

  await plugin.getSnapshot(snapshotRequest);
  await new Promise((resolve) => setTimeout(resolve, 10000));
