...
export const CHIA_TEST_LEDGER_DEFAULT_OPTIONS = Object.freeze({
  containerImageVersion: "2021-01-08-7a055c3",
  containerImageName: "ghcr.io/chianetwork/chia-docker",
  rpcApiHttpPort: 8545,
  rpcApiWsPort: 8546,
  envVars: ["TESTNET=TRUE"],
});
...
export class ChiaTestLedger implements ITestLedger {
  ...
  constructor(public readonly options: IChiaTestLedgerConstructorOptions = {}) {
    if (!options) {
      throw new TypeError(`ChiaTestLedger#ctor options was falsy.`);
    }
    this.containerImageVersion =
      options.containerImageVersion ||
      CHIA_TEST_LEDGER_DEFAULT_OPTIONS.containerImageVersion;
    this.containerImageName =
      options.containerImageName ||
      CHIA_TEST_LEDGER_DEFAULT_OPTIONS.containerImageName;
    this.rpcApiHttpPort =
      options.rpcApiHttpPort || CHIA_TEST_LEDGER_DEFAULT_OPTIONS.rpcApiHttpPort;
    this.rpcApiWsPort =
      options.rpcApiWsPort || CHIA_TEST_LEDGER_DEFAULT_OPTIONS.rpcApiWsPort;
    this.envVars = options.envVars || CHIA_TEST_LEDGER_DEFAULT_OPTIONS.envVars;
  ...
  public getContainer(): Container {
    const fnTag = "ChiaTestLedger#getContainer()";
    if (!this.container) {
  ...
  /**
   * Output is based on the static 'dev' Chia keys generation.
   *
   * @see https://docs.chia.net/wallet-rpc#add_key
   */
  public getGenesisAccountPubKey(): string {
    return "9394584cbf86582e48cd8c9b2f1ca308ca9fca1cdbb434d50f5c2adda186dd4ac417ff1b6c0281d1b2d2";
  }
  ...
  public getGenesisAccountPrivKey(): string {
    return "961de0c8a9db6bc82dd5163acfe7ec23e07c231de4c4597c248011584f8bed8b1a62db812bc2527127f3adb9cd973fcc";
  }
   ...
   */
  public async createChiaTestAccount(seedMoney = 10e8): Promise<Account> {
    const fnTag = `ChiaTestLedger#getChiaTestAccount()`;
  ...
  public async getChiaKeyPair(): Promise<IKeyPair> {
    const publicKey =
      "9394584cbf86582e48cd8c9b2f1ca308ca9fca1cdbb434d50f5c2adda186dd4ac417ff1b6c0281d1b2d2";
    const privateKey =
      "961de0c8a9db6bc82dd5163acfe7ec23e07c231de4c4597c248011584f8bed8b1a62db812bc2527127f3adb9cd973fcc";
  ...
  public async start(omitPull = false): Promise<Container> {
    ...
    return new Promise<Container>((resolve, reject) => {
      const eventEmitter: EventEmitter = docker.run(
        ...
        {
          ExposedPorts: {
            [`${this.rpcApiHttpPort}/tcp`]: {}, // Chia RPC - HTTP
            [`${this.rpcApiWsPort}/tcp`]: {}, // Chia RPC - WebSocket
            "8888/tcp": {}, // orion Client Port - HTTP
            "8080/tcp": {}, // orion Node Port - HTTP
            "9001/tcp": {}, // supervisord - HTTP
            "9545/tcp": {}, // Chia metrics
          },
  ...
  public async waitForHealthCheck(timeoutMs = 360000): Promise<void> {
    const fnTag = "ChiaTestLedger#waitForHealthCheck()";
  ...
  public stop(): Promise<unknown> {
    const fnTag = "ChiaTestLedger#stop()";
    return new Promise((resolve, reject) => {
      if (this.container) {
  ...
  public destroy(): Promise<unknown> {
    const fnTag = "ChiaTestLedger#destroy()";
    if (this.container) {
  ...
  protected async getContainerInfo(): Promise<ContainerInfo> {
    ...
    if (aContainerInfo) {
      ...
      throw new Error(`ChiaTestLedger#getContainerInfo() no image "${image}"`);
    }
  }

  public async getRpcApiPublicPort(): Promise<number> {
    const fnTag = "ChiaTestLedger#getRpcApiPublicPort()";
  ...
  public async getContainerIpAddress(): Promise<string> {
    const fnTag = "ChiaTestLedger#getContainerIpAddress()";
    const aContainerInfo = await this.getContainerInfo();
