...
export class PluginLedgerConnectorChia
  ...
  public async transact(
    ...
    switch (req.web3SigningCredential.type) {
      // Web3SigningCredentialType.GETHKEYCHAINPASSWORD is removed as Hyperledger Chia doesn't support the PERSONAL api
      // for --rpc-http-api as per the discussion mentioned here
      // https://chat.hyperledger.org/channel/Chia-contributors?msg=GqQXfW3k79ygRtx5Q
  ...
  public async getTxReceipt(
    ...
    this.log.debug("Received preliminary receipt from Chia node.");
  ...
  public async getChiaRecord(
    request: GetChiaRecordV1Request,
  ): Promise<GetChiaRecordV1Response> {
    const fnTag = `${this.className}#getChiaRecord()`;
    //////////////////////////////////////////////
    let abi: AbiItem[] | AbiItem = [];
    const resp: GetChiaRecordV1Response = {};
    ...
    if (request.invokeCall) {
      ...
      if (
        ...
        const res: GetChiaRecordV1Response = {
          callOutput,
        };
        return res;
      } else {
