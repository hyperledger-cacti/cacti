...
import { ChiaMpTestLedger } from "@hyperledger/cactus-test-tooling";
...
const testCase = "Executes private transactions on Hyperledger Chia";
...
test(testCase, async (t: Test) => {
  ...
  // docker run   --rm   --privileged   --publish 2222:22   --publish 3000:3000   --publish 8545:8545   --publish 8546:8546   --publish 9001:9001   --publish 9081:9081   --publish 9082:9082   --publish 9083:9083   --publish 9090:9090   --publish 18545:18545   --publish 20000:20000   --publish 20001:20001   --publish 20002:20002   --publish 20003:20003   --publish 20004:20004   --publish 20005:20005   --publish 25000:25000   petermetz/cactus-chia-multi-party-all-in-one:0.1.2
  //
  ...
  if (preWarmedLedger) {
    ...
    const ledger = new ChiaMpTestLedger({ logLevel });
    test.onFinish(() => ledger.stop());
    await ledger.start();
    keys = await ledger.getKeys();
  }

  const rpcApiHttpHostMember1 = keys.chia.member1.url;
  const rpcApiHttpHostMember2 = keys.chia.member2.url;
  const rpcApiHttpHostMember3 = keys.chia.member3.url;

  const rpcApiWsHostMember1 = keys.chia.member1.wsUrl;
  const rpcApiWsHostMember2 = keys.chia.member2.wsUrl;
  const rpcApiWsHostMember3 = keys.chia.member3.wsUrl;
  ...
  const connectorInstanceId1 = "chia1_" + uuidv4();
  const connectorInstanceId2 = "chia2_" + uuidv4();
  const connectorInstanceId3 = "chia3_" + uuidv4();
  ...
  const deployRes = await connector1.deployContract({
    ...
    web3SigningCredential: {
      secret: keys.chia.member1.privateKey,
...

  {
    ...
    const functionParams = {
      ...
      privateKey: keys.chia.member1.privateKey,
    };
...

  {
    ...
    const fnParams = {
      ...
      privateFrom: keys.tessera.member1.publicKey,
      privateFor: [keys.tessera.member2.publicKey],
      privateKey: keys.chia.member3.privateKey,
    };
...

  {
    ...
    const functionParams = {
      ...
      privateFor: [keys.tessera.member2.publicKey],
      privateKey: keys.chia.member2.privateKey,
    };
...

  {
    ...
    await t.rejects(
      ...
      "private contract call fails without Chia member credentials OK",
    );
  }
...

  {
    ...
    const res = await connector3.invokeContract({
      ...
      signingCredential: {
        secret: keys.chia.member3.privateKey,
        type: Web3SigningCredentialType.PrivateKeyHex,
      },
    });
