...
import { ChiaApiClientOptions } from "../../../../../main/typescript/api-client/chia-api-client";

import { installOpenapiValidationMiddleware } from "@hyperledger/cactus-core";
import OAS from "../../../../../main/json/openapi.json";

const logLevel: LogLevelDesc = "TRACE";
const testCase = "Chia API";
...
test(testCase, async (t: Test) => {
  ...
  const testEthAccount1 = await chiaTestLedger.createEthTestAccount();
  const testEthAccount2 = await chiaTestLedger.createEthTestAccount();
  ...
  const options: IPluginLedgerConnectorChiaOptions = {
    instanceId: uuidv4(),
    rpcApiHttpHost,
    rpcApiWsHost,
    pluginRegistry,
  ...
  const connector = new PluginLedgerConnectorChia(options);
  ...
  const apiClient = new ChiaApiClient(chiaApiClientOptions);
  ...
  test(`${testCase} - ${fRecord} - ${cOk}`, async (t2: Test) => {
    ...
    const res = await apiClient.getChiaRecordV1(
      parameters as GetChiaRecordV1Request,
    );
    t2.equal(
      res.status,
      200,
  ...
  test(`${testCase} - ${fRecord} - ${cWithoutParams}`, async (t2: Test) => {
    try {
      const parameters = {};
      await apiClient.getChiaRecordV1(parameters as GetChiaRecordV1Request);
    } catch (e) {
      t2.equal(
        e.response.status,
        400,
