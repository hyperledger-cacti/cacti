...
describe(testCase, () => {
  ...
  test(`${testCase} - ${fInitialize} - ${cInvalidParams}`, async () => {
    ...
    await expect(
      api.initializeV1(
        parameters as unknown as InitializeRequest,
      ),
    ).rejects.toMatchObject({
      response: {
        status: 400,
  ...
  test(`${testCase} - ${fNew} - ${cWithoutParams}`, async () => {
    ...
    await expect(
      api.newContractV1(
        parameters as unknown as NewContractObj,
      ),
    ).rejects.toMatchObject({
      response: {
        status: 400,
  ...
  test(`${testCase} - ${fRefund} - ${cWithoutParams}`, async () => {
    ...
    await expect(
      api.refundV1(
        parameters as unknown as RefundReq,
      ),
    ).rejects.toMatchObject({
      response: {
        status: 400,
  ...
  test(`${testCase} - ${fWithdraw} - ${cWithoutParams}`, async () => {
    ...
    await expect(
      api.withdrawV1(
        parameters as unknown as WithdrawReq,
      ),
    ).rejects.toMatchObject({
      response: {
        status: 400,
  ...
  test(`${testCase} - ${fStatus} - ${cWithoutParams}`, async () => {
    ...
    await expect(
      api.getStatusV1(
        parameters as unknown as GetStatusRequest,
      ),
    ).rejects.toMatchObject({
      response: {
        status: 400,
