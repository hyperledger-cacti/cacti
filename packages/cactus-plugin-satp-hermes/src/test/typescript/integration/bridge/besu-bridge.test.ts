...
import SATPContract from "./SATPContract.json";
import SATPWrapperContract from "./SATPWrapperContract.json";
...
beforeAll(async () => {
    ...
    });
  {
    ...
    const deployOutSATPContract = await testing_connector.deployContract({
      ...
      bytecode: SATPContract.evm.bytecode.object,
      gas: 999999999999999,
    });
    ...
    const deployOutWrapperContract = await testing_connector.deployContract({
      ...
      constructorArgs: [
        bridgeEthAccount.address,
        web3.utils.soliditySha3("BRIDGE_ROLE"),
      ],
      web3SigningCredential: {
      ...
      bytecode: SATPWrapperContract.evm.bytecode.object,
      gas: 999999999999999,
    });
...
describe("Besu Bridge Test", () => {
  ...
  it("Should Wrap a token", async () => {
    ...
    expect(response3.tokenType).toBe(TokenType.ERC20);
  ...
  it("Should Lock a token", async () => {
    ...
    expect(response2.tokenType).toBe(TokenType.ERC20);
