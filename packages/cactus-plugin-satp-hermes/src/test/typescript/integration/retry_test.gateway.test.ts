...
let fabricEnvWithWrongBridgeId: FabricTestEnvironment;
let besuEnv: BesuTestEnvironment;
let gateway: SATPGateway;
// OU=department1 was purposely changed to OU=department instead of OU=department1
// to cause a test failure
const wrong_bridge_id =
  "x509::/OU=org2/OU=client/OU=department/CN=bridge::/C=UK/ST=Hampshire/L=Hursley/O=org2.example.com/CN=ca.org2.example.com";
afterAll(async () => {
  ...
  await fabricEnvWithWrongBridgeId.tearDown();
...
beforeAll(async () => {
...

  {
    ...
    fabricEnvWithWrongBridgeId =
      await FabricTestEnvironment.setupTestEnvironment(
        satpContractName,
        wrong_bridge_id,
        logLevel,
      );
    log.info("Fabric Ledger with wrong bridge_id started successfully");

    await fabricEnvWithWrongBridgeId.deployAndSetupContracts(
      ClaimFormat.DEFAULT,
    );
  }
...
describe("SATPGateway sending a token from Besu to Fabric", () => {
  it("should realize a transfer", async () => {
    ...
    const options: SATPGatewayConfig = {
      ...
      bridgesConfig: [
        besuEnv.besuConfig,
        fabricEnvWithWrongBridgeId.fabricConfig,
      ],
    ...
    const reqWithWrongBridgeId = getTransactRequest(
      "mockContext",
      besuEnv,
      fabricEnvWithWrongBridgeId,
      "100",
    ...
    const res = await dispatcher?.Transact(reqWithWrongBridgeId);
    ...
    const responseBalance1 =
      await fabricEnvWithWrongBridgeId.apiClient.runTransactionV1({
        contractName: fabricEnvWithWrongBridgeId.satpContractName,
        channelName: fabricEnvWithWrongBridgeId.fabricChannelName,
        params: [fabricEnvWithWrongBridgeId.bridge_id],
        methodName: "ClientIDAccountBalance",
        invocationType: FabricContractInvocationType.Send,
        signingCredential: fabricEnvWithWrongBridgeId.fabricSigningCredential,
      });
    ...
    const responseBalance2 =
      await fabricEnvWithWrongBridgeId.apiClient.runTransactionV1({
        contractName: fabricEnvWithWrongBridgeId.satpContractName,
        channelName: fabricEnvWithWrongBridgeId.fabricChannelName,
        params: [fabricEnvWithWrongBridgeId.clientId],
        methodName: "ClientIDAccountBalance",
        invocationType: FabricContractInvocationType.Send,
        signingCredential: fabricEnvWithWrongBridgeId.fabricSigningCredential,
      });
