 ...
 * - Stage 0, 1 are always complete.
 * - Stage 2 partial if client; complete if server.
 */
...
afterAll(async () => {
  if (crashManager1 || crashManager2) {
    ...
    crashManager1.remoteRepository.destroy();

    crashManager2.stopScheduler();
    crashManager2.localRepository.destroy();
    crashManager2.remoteRepository.destroy();
  }
...
describe("Stage 2 Recovery Test", () => {
  it("should recover Stage 2 hashes and timestamps and update session state to RECOVERED", async () => {
    ...
    expect(updatedSessionDataClient?.hashes?.stage2).toEqual(
      updatedSessionDataServer?.hashes?.stage2,
    );

    expect(updatedSessionDataClient?.processedTimestamps?.stage2).toEqual(
      updatedSessionDataServer?.processedTimestamps?.stage2,
    );

    expect(updatedSessionDataClient?.signatures?.stage2).toEqual(
      updatedSessionDataServer?.signatures?.stage2,
    );

    expect(
      updatedSessionDataClient?.hashes?.stage2?.lockAssertionReceiptMessageHash,
    ).toBe("h11");
    expect(
      updatedSessionDataClient?.processedTimestamps?.stage2
        ?.lockAssertionReceiptMessageTimestamp,
    ).toBeDefined();
    expect(
      updatedSessionDataClient?.signatures?.stage2
