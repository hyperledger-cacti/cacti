...
export class EthereumTestEnvironment {
  ...
  private constructor(
    ...
    this.tokenContract = tokenContract1;
  ...
  public async init(logLevel: LogLevelDesc): Promise<void> {
    ...
    const SATPTokenContract2 = {
      ...
      bytecode: SATPNFTokenContract.bytecode,
    };
    ...
    this.keychainPlugin3.set(
      this.tokenContract, //Working around undefined string to define string error
      JSON.stringify(SATPTokenContract2),
    );

    const pluginRegistry = new PluginRegistry({
      plugins: [this.keychainPlugin2, this.keychainPlugin3],
    });
  ...
  public async giveRoleToBridge(
    ...
    contractTokenType: string = "ERC20",
  ): Promise<void> {
    switch (contractTokenType) {
      ...
      case "NONFUNGIBLE":
        ...
        break;
      default:
        throw new Error("provided asset type is not supported");
    }
    this.log.info("BRIDGE_ROLE given to SATPWrapperContract successfully");
  }
  ...
  public get nonFungibleAsset(): Asset {
    return {
      id: EthereumTestEnvironment.ETH_ASSET_ID,
      referenceId: EthereumTestEnvironment.ETHREFERENCE_ID,
      owner: WHALE_ACCOUNT_ADDRESS,
