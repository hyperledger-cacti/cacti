...
describe("CrashManager Tests", () => {
  ...
  it("should process logs from counterparty gateway", async () => {
    mockSession = createMockSession("10000", "3");
    ...
    const updatedLogEntry: LocalLog = {
      sessionId: sessionId,
      type: "type_server",
    ...
    const recoverUpdateMessage = {
      ...
      hashRecoverMessage: "",
      recoveredLogs: [updatedLogEntry],
      senderSignature: "",
    } as RecoverUpdateMessage;

    const result = await crashManager["processRecoverUpdateMessage"](
