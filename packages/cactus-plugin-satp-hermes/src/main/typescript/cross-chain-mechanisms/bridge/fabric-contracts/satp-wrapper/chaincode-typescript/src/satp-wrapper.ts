  ...
  implements ITraceableContract
{
  ...
  public async wrap(
    ...
    // Store the token ID in a list of wrapped tokens
    const wrappedTokensKey = "wrappedTokens";
    const wrappedTokensBytes = await ctx.stub.getState(wrappedTokensKey);
    let wrappedTokens: string[] = [];

    if (wrappedTokensBytes && wrappedTokensBytes.length > 0) {
      wrappedTokens = JSON.parse(wrappedTokensBytes.toString());
    }

    if (!wrappedTokens.includes(tokenId)) {
      wrappedTokens.push(tokenId);
      await ctx.stub.putState(
        wrappedTokensKey,
        Buffer.from(JSON.stringify(wrappedTokens)),
      );
    }
    return true;
  }
  ...
  public async unwrap(ctx: Context, tokenId: string): Promise<boolean> {
    ...
    // Remove the unwrapped token from the list of wrapped tokens
    const wrappedTokensKey = "wrappedTokens";
    const wrappedTokensBytes = await ctx.stub.getState(wrappedTokensKey);
    let wrappedTokens: string[] = [];
    ...
    const index = wrappedTokens.indexOf(tokenId);
    if (index > -1) {
      wrappedTokens.splice(index, 1);
      await ctx.stub.putState(
        wrappedTokensKey,
        Buffer.from(JSON.stringify(wrappedTokens)),
      );
   ...
   * Retrieves all token IDs stored in the world state.
   * @param ctx The transaction context.
   * @returns string[]
   */
  @Transaction(false)
  @Returns("string[]")
  public async GetAssets(ctx: Context): Promise<string[]> {
    const wrappedTokensKey = "wrappedTokens";
    const wrappedTokensBytes = await ctx.stub.getState(wrappedTokensKey);
    ...
    return JSON.parse(wrappedTokensBytes.toString());
  }
