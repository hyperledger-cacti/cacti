...
export class MonitorService {
  ...
  public async init(): Promise<void> {
    ...
    if (!this.isEnabled) {
      this.logger.info("MonitorService disabled by config.");
      return;
    }
    ...
    if (!this.sdk) {
      ...
      this.logger.warn("MonitorService is already initialized, skipping init.");
    }
  }
  ...
  public startSpan(
    ...
    if (!this.isEnabled) return {} as unknown as Span;
    if (!this.sdk) {
      throw new Error("NodeSDK not initialized");
    }
  ...
  public async shutdown(): Promise<void> {
    ...
    } else this.logger.warn("MonitorService is already shut down.");
  }
