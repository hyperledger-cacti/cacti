      ...
      try {
        try {
          const networkId = {
            ...
            ledgerType:
              clientSessionData.recipientGatewayNetworkId as LedgerType,
          };
          if (!networkId) {
            throw new Error(
  ...
  async cleanup(
    ...
    return context.with(ctx, () => {
      try {
        ...
        try {
          sessionId = session.getSessionId();
          state.details = `Stage 0 cleanup completed for session ${sessionId}`;
          state.rollbackLogEntries.push(
            create(RollbackLogEntrySchema, {
              sessionId,
              stage: SATPStage[1],
              timestamp: new Date().toISOString(),
              action: "CLEANUP_STAGE_0",
              status: "SUCCESS",
          ...
          state.details = `Stage 0 cleanup failed: ${error}`;
          state.rollbackLogEntries.push(
            create(RollbackLogEntrySchema, {
              sessionId,
              stage: SATPStage[1],
