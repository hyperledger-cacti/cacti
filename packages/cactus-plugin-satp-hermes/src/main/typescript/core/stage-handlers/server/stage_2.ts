...
import { LockAssertionReceiptMessage, LockAssertionRequestMessage } from "../../../generated/proto/cacti/satp/v02/stage_2_pb";
...
export class Stage2ServerHandler {
    ...
    async lockAssertionResponse(
      ...
      const fnTag = `${gateway.className}#lockAssertionResponse()`;
      ...
      commonBody.hashPreviousMessage = sessionData.hashes.stage2.lockAssertionRequestMessageHash;
      ...
      const messageSignature = PluginSATPGateway.bufArray2HexStr(
        gateway.sign(JSON.stringify(lockAssertionReceiptMessage)),
      );
    ...
    checkLockAssertionRequestMessage(
      ...
      const fnTag = `${gateway.className}#checkLockAssertionRequestMessage()`;
      ...
      if (
          ...
          sessionData.hashes.stage1.transferCommenceResponseMessageHash == undefined ||
          sessionData.lastSequenceNumber == undefined
          ) {
          throw new Error(
      ...
      if (!gateway.verifySignature(request.common, request.common.serverGatewayPubkey)) {
          throw new Error(
      ...
      if (request.common.messageType != MessageType.LOCK_ASSERT) {
          throw new Error(
              `${fnTag}, wrong message type for LockAssertionRequest`,
          );
      }
      ...
      if (
          request.common.hashPreviousMessage != sessionData.hashes.stage1.transferCommenceResponseMessageHash  
        ) {
          throw new Error(
      ...
      if(request.lockAssertionFormat == undefined || request.lockAssertionClaim == undefined){
        throw new Error(
