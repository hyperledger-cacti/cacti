...
export function populateClientSessionData(
  ...
  const fnTag = "session_utils#populateClientSessionData";
...
export function copySessionDataAttributes(
  ...
  const fnTag = "session_utils#copySessionDataAttributes";
  const tracer = trace.getTracer("satp-hermes-tracer");
  const span = tracer.startSpan(fnTag);
  const ctx = trace.setSpan(context.active(), span);
  context.with(ctx, () => {
    try {
      ...
      destSessionData.accessControlProfile =
        srcSessionData.accessControlProfile;
      ...
      destSessionData.senderGatewayOwnerId =
        srcSessionData.senderGatewayOwnerId;
      ...
      destSessionData.multipleClaimsAllowed =
        srcSessionData.multipleClaimsAllowed;
      ...
      destSessionData.transferClaimsFormat =
        srcSessionData.transferClaimsFormat;
      destSessionData.clientTransferNumber =
        srcSessionData.clientTransferNumber;
      destSessionData.serverTransferNumber =
        srcSessionData.serverTransferNumber;
...
export function saveTimestamp(
  ...
  const fnTag = "session_utils#saveTimestamp";
  const tracer = trace.getTracer("satp-hermes-tracer");
  const span = tracer.startSpan(fnTag);
  const ctx = trace.setSpan(context.active(), span);
    ...
    try {
      ...
      switch (stageMessage) {
        ...
        case MessageType.TRANSFER_COMMENCE_RESPONSE:
          timestamps.stage1.transferCommenceResponseMessageTimestamp =
            timestamp;
          break;
        ...
        case MessageType.COMMIT_PREPARE:
          timestamps.stage3.commitPreparationRequestMessageTimestamp =
            timestamp;
          break;
        ...
        case MessageType.COMMIT_FINAL:
          timestamps.stage3.commitFinalAssertionRequestMessageTimestamp =
            timestamp;
          break;
        ...
        case MessageType.COMMIT_TRANSFER_COMPLETE_RESPONSE:
          timestamps.stage3.transferCompleteResponseMessageTimestamp =
            timestamp;
          break;
      }
    } catch (error) {
...
export function saveHash(
  ...
  const fnTag = "session_utils#saveHash";
  const tracer = trace.getTracer("satp-hermes-tracer");
  const span = tracer.startSpan(fnTag);
  const ctx = trace.setSpan(context.active(), span);
  return context.with(ctx, async () => {
...
export function saveSignature(
  ...
  const fnTag = "session_utils#saveSignature";
  const tracer = trace.getTracer("satp-hermes-tracer");
  const span = tracer.startSpan(fnTag);
  const ctx = trace.setSpan(context.active(), span);
  return context.with(ctx, async () => {
    try {
      ...
      switch (stageMessage) {
        ...
        case MessageType.TRANSFER_COMMENCE_REQUEST:
          ...
          break;
        case MessageType.TRANSFER_COMMENCE_RESPONSE:
          signatures.stage1.transferCommenceResponseMessageSignature =
            signature;
          break;
        case MessageType.LOCK_ASSERT:
        ...
        case MessageType.ASSERTION_RECEIPT:
          ...
          break;
        case MessageType.COMMIT_PREPARE:
          signatures.stage3.commitPreparationRequestMessageSignature =
            signature;
          break;
        case MessageType.COMMIT_READY:
          ...
          break;
        case MessageType.COMMIT_FINAL:
          signatures.stage3.commitFinalAssertionRequestMessageSignature =
            signature;
          break;
        case MessageType.ACK_COMMIT_FINAL:
        ...
        case MessageType.COMMIT_TRANSFER_COMPLETE:
          ...
          break;
        case MessageType.COMMIT_TRANSFER_COMPLETE_RESPONSE:
          signatures.stage3.transferCompleteResponseMessageSignature =
            signature;
          break;
      }
    } catch (error) {
...
export function getPreviousMessageType(
  ...
  const fnTag = "session_utils#getPreviousMessageType";
  const tracer = trace.getTracer("satp-hermes-tracer");
  const span = tracer.startSpan(fnTag);
  const ctx = trace.setSpan(context.active(), span);
  return context.with(ctx, () => {
...
export function getMessageHash(
  ...
  const fnTag = "session_utils#getMessageHash";
  const tracer = trace.getTracer("satp-hermes-tracer");
  const span = tracer.startSpan(fnTag);
  const ctx = trace.setSpan(context.active(), span);
  return context.with(ctx, () => {
...
export function getMessageTimestamp(
  ...
  const fnTag = "session_utils#getMessageTimestamp";
  const tracer = trace.getTracer("satp-hermes-tracer");
  const span = tracer.startSpan(fnTag);
  const ctx = trace.setSpan(context.active(), span);
  return context.with(ctx, () => {
...
export function setError(
  ...
  const fnTag = "session_utils#setError";
  const tracer = trace.getTracer("satp-hermes-tracer");
  const span = tracer.startSpan(fnTag);
  const ctx = trace.setSpan(context.active(), span);
  return context.with(ctx, async () => {
...
export function setErrorChecking(
  ...
  const fnTag = "session_utils#setErrorChecking";
  const tracer = trace.getTracer("satp-hermes-tracer");
  const span = tracer.startSpan(fnTag);
  const ctx = trace.setSpan(context.active(), span);
  return context.with(ctx, async () => {
...
export function getSessionActualStage(
  ...
  const fnTag = "session_utils#getSessionActualStage";
  const tracer = trace.getTracer("satp-hermes-tracer");
  const span = tracer.startSpan(fnTag);
  const ctx = trace.setSpan(context.active(), span);
  return context.with(ctx, () => {
    try {
      ...
      return [stage, messageType] as [SATPStage, MessageType];
    } catch (error) {
      span.setStatus({ code: SpanStatusCode.ERROR, message: String(error) });
      span.recordException(error);
      throw error;
...
export function getCrashedStage(sessionData: SessionData): SATPStage {
  const fnTag = "session_utils#getCrashedStage";
  const tracer = trace.getTracer("satp-hermes-tracer");
  const span = tracer.startSpan(fnTag);
  const ctx = trace.setSpan(context.active(), span);
  return context.with(ctx, () => {
...
export function saveMessageInSessionData(
  ...
  const fnTag = "session_utils#saveMessageInSessionData";
  const tracer = trace.getTracer("satp-hermes-tracer");
  const span = tracer.startSpan(fnTag);
  const ctx = trace.setSpan(context.active(), span);
  context.with(ctx, async () => {
    try {
      ...
      if (isMessage(message, NewSessionRequestSchema)) {
        ...
        sessionData.satpMessages.stage0.preSatpTransferResponseMessage =
          message;
      } else if (isMessage(message, TransferProposalRequestSchema)) {
        sessionData.satpMessages.stage1.transferProposalRequestMessage =
          message;
      } else if (isMessage(message, TransferProposalResponseSchema)) {
        sessionData.satpMessages.stage1.transferProposalReceiptMessage =
          message;
      } else if (isMessage(message, TransferCommenceRequestSchema)) {
        sessionData.satpMessages.stage1.transferCommenceRequestMessage =
          message;
      } else if (isMessage(message, TransferCommenceResponseSchema)) {
        sessionData.satpMessages.stage1.transferCommenceResponseMessage =
          message;
        ...
        sessionData.satpMessages.stage3.commitPreparationRequestMessage =
          message;
        ...
        sessionData.satpMessages.stage3.transferCompleteResponseMessage =
          message;
      } else {
        throw new Error("Message type not found");
      }
...
export function getMessageInSessionData(
  ...
  | TransferCompleteResponse
  | undefined {
  const fnTag = "session_utils#getMessageInSessionData";
  const tracer = trace.getTracer("satp-hermes-tracer");
  const span = tracer.startSpan(fnTag);
  const ctx = trace.setSpan(context.active(), span);
  return context.with(ctx, () => {
    try {
      ...
      switch (messageType) {
        ...
        case MessageType.TRANSFER_COMMENCE_RESPONSE:
          return sessionData.satpMessages.stage1
