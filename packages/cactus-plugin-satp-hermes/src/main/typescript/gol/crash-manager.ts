...
import { SessionType } from "../core/session-utils";
...
export class CrashRecoveryManager {
  ...
  public async checkAndResolveCrashes(): Promise<void> {
    ...
    for (const [sessionId, session] of this.sessions.entries()) {
      ...
      if (sessionData.state === State.COMPLETED) {
  ...
  private async checkCrash(session: SATPSession): Promise<CrashStatus> {
    ...
    try {
      ...
      const lastLog = await this.localRepository.readLastestLog(
  ...
  private async processRecoverUpdateMessage(
    ...
    try {
      ...
      for (const logEntry of recoveredLogs) {
        console.log("data receveived: ", logEntry);
        await this.localRepository.create(logEntry);
      }
