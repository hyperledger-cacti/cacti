...
export default class TokenClientERC1155 extends TokenClient {
  ...
  constructor(
    ...
    super(apiClient, { abi: ERC1155, contractName: "ERC1155", bytecode: "0x" }, address, logLevel);
  ...
  async name(): Promise<string> {
    ...
    try {
      ...
      if (typeof response !== "string") {
        this.log.warn(`Unexpected response type for 'name': ${response}, returning empty string`);
        return "";
      }
      return response;
    } catch (error) {
      this.log.warn(`Method 'name' not implemented on ERC1155 contract at ${this.address}, returning empty string`);
      return "";
    }
  }
  ...
  async symbol(): Promise<string> {
    ...
    try {
      ...
      if (typeof response !== "string") {
        this.log.warn(`Unexpected response type for 'symbol': ${response}, returning empty string`);
        return "";
      }
      return response;
    } catch (error) {
      this.log.warn(`Method 'symbol' not implemented on ERC1155 contract at ${this.address}, returning empty string`);
      return "";
    }
  }
