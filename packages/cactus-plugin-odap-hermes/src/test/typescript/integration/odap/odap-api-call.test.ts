...
describe(testCase, () => {
  ...
  test("runs ODAP between two gateways via openApi", async () => {
    ...
    const serverSessionData = clientOdapGateway.sessions.get(sessionId);
    ...
    expect(clientSessionData.assetProfile).toBe(serverSessionData.assetProfile);
    ...
    expect(clientSessionData.initializationRequestMessageRcvTimeStamp).toBe(
      serverSessionData.initializationRequestMessageRcvTimeStamp,
    );

    expect(
      clientSessionData.initializationRequestMessageProcessedTimeStamp,
    ).toBe(serverSessionData.initializationRequestMessageProcessedTimeStamp);
