...
export class TransferInitiationResponseEndpointV1
  ...
  public get className(): string {
    return TransferInitiationResponseEndpointV1.CLASS_NAME;
  }

  constructor(
    public readonly options: ITransferInitiationResponseEndpointOptions,
  ) {
  ...
  public getPath(): string {
    const apiPath =
      OAS.paths[
        "/api/v1/@hyperledger/cactus-plugin-odap-hemres/phase1/transferinitiationresponse"
      ];
  ...
  public async handleRequest(req: Request, res: Response): Promise<void> {
    ...
    try {
      await this.options.gateway.onTransferInitiationResponseReceived(req.body);
