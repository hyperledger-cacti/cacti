...
const validatorRregistryConf: any = JSON.parse(fs.readFileSync("/etc/cactus/validator-registry.json", 'utf8'));
...
export class VerifierBase implements Verifier {
    ...
    sendSignedTransaction(contract: object, method: object, template: string, args: object): void {
        ...
        logger.debug(`call: sendSignedTransaction, contract = ${JSON.stringify(contract)}, method = ${JSON.stringify(method)}, template = ${template}, args = ${args}, `);
