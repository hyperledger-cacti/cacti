...
export class RequestProofEndpoint implements IWebServiceEndpoint {
  ...
  public get oasPath(): (typeof OAS.paths)["/api/v1/plugins/@hyperledger-cacti/cactus-plugin-ledger-connector-aries/request-proof"] {
    return OAS.paths[
      "/api/v1/plugins/@hyperledger-cacti/cactus-plugin-ledger-connector-aries/request-proof"
    ];
  }
  ...
  public async handleRequest(req: Request, res: Response): Promise<void> {
    ...
    try {
      res
        .status(200)
        .json(
          await this.options.connector.requestProof(
            req.body.agentName,
            req.body.connectionId,
            req.body.proofAttributes,
          ),
        );
    } catch (ex) {
