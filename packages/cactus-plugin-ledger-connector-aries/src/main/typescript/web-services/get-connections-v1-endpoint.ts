...
export interface IGetConnectionsOptions {
  logLevel?: LogLevelDesc;
  connector: PluginLedgerConnectorAries;
}

export class GetConnectionsEndpoint implements IWebServiceEndpoint {
  private readonly log: Logger;

  public get className(): string {
    return "GetConnectionsEndpoint";
  }

  constructor(public readonly options: IGetConnectionsOptions) {
  ...
  public async handleRequest(req: Request, res: Response): Promise<void> {
    ...
    try {
      res
        .status(200)
        .json(
          await this.options.connector.getConnections(
            req.body.agentName,
            req.body.filter,
          ),
        );
