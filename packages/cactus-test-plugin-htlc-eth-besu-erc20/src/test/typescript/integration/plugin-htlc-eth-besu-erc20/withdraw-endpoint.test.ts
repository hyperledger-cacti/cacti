...
test("Test withdraw endpoint", async (t: Test) => {
  ...
  test("Get balance of account", async (t2: Test) => {
  ...
  let id = "";
  test("Get HTLC Id from DemoHelper", async (t2: Test) => {
    const { callOutput } = await connector.invokeContract({
    ...
    t2.ok(callOutput, "result is truthy OK");
    id = callOutput as string;
  });

  test("Withdraw HTLC", async (t2: Test) => {
    const request: WithdrawRequest = {
      id,
      secret:
        "0x3853485acd2bfc3c632026ee365279743af107a30492e3ceaa7aefc30c2a048a",
    };
    const res = await api.withdraw(request);
    t2.equal(res.status, 200, "response status is 200 OK");
  });
});
