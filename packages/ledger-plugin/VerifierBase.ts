...
export class VerifierBase implements Verifier {
  ...
  execSyncFunction(param: LedgerOperation): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      try {
        logger.debug(
          `##in execSyncFunction, LedgerOperation = ${JSON.stringify(param)}`
        );
        ...
        socket.on("response", (result: any) => {
          ...
          if (reqID === result.id) {
            responseFlag = true;
            logger.debug(
              `##execSyncFunction: resObj: ${JSON.stringify(result.resObj)}`
            );
            // Result reply
  ...
  execSyncFunctionNeo(
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        logger.debug(
          `##in execSyncFunction, contract = ${JSON.stringify(
            contract
