...
describe(testcase, () => {
  ...
  beforeAll(async () => {
    ...
    highNetWorthAccounts = Object.keys(ethereumGenesisOptions.alloc).filter(
      (address: string) => {
  ...
  test("Ethereum Ledger Connector Plugin", async () => {
  ...
  test("invoke Web3SigningCredentialType.GETHKEYCHAINPASSWORD", async () => {
    ...
    try {
      ...
      fail(
        "PluginLedgerConnectorEthereum.invokeContract failed to throw error",
      );
    } catch (error) {
