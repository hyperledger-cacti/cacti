...
test("Ethereum Ledger Connector Plugin", async (t: Test) => {
  ...
  t.ok(ethereumGenesisOptions);
  t.ok(ethereumGenesisOptions.alloc);
  ...
  t.comment(
    `Metrics URL: ${apiHost}/api/v1/plugins/@hyperledger/cactus-plugin-ledger-connector-ethereum/get-prometheus-exporter-metrics`,
  );
