...
import puppeteer, { Browser, Request as PuppeteerRequest } from "puppeteer";
...
tap.test("can authenticate/authorize via API server", async (assert: any) => {
  ...
  // pre-fetch available port for cockpit, 3000 if available, random otherwise
  const cockpitPort = await getPort({ port: 4000 });
  const httpServerCockpit = createServer();
  await new Promise((resolve, reject) => {
    httpServerCockpit.once("error", reject);
    httpServerCockpit.listen(cockpitPort, "127.0.0.1", resolve);
  });
  ...
  // pre-fetch available port for API
  const apiPort = await getPort({ port: 3000 });
  const httpServerApi = createServer();
  await new Promise((resolve, reject) => {
    httpServerApi.once("error", reject);
    httpServerApi.listen(apiPort, "127.0.0.1", resolve);
  });
  ...
  cactusApiServerOptions.apiPort = apiPort;
  ...
  const api = new ApiClient(configuration).extendWith(DefaultApiOidc);

  // 7. Issue an API call to the API server via the main SDK verifying that the SDK and the API server both work
  const healthcheckResp: AxiosResponse<HealthCheckResponse> = await api.apiV1ApiServerHealthcheckGet();
