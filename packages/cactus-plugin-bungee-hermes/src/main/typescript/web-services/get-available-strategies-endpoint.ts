...
export interface GetAvailableStrategiesEndpointOptions {
  logLevel?: LogLevelDesc;
  bungee: PluginBungeeHermes;
}

export class GetAvailableStrategiesEndpointV1 implements IWebServiceEndpoint {
  public static readonly CLASS_NAME = "ClientEndpointV1";

  private readonly log: Logger;

  public get className(): string {
    return GetAvailableStrategiesEndpointV1.CLASS_NAME;
  }

  constructor(public readonly options: GetAvailableStrategiesEndpointOptions) {
  ...
  public getPath(): string {
    const apiPath =
      OAS.paths[
        "/api/v1/plugins/@hyperledger-cacti/cactus-plugin-bungee-hermes/get-available-strategies"
      ];
  ...
  public async handleRequest(_req: Request, res: Response): Promise<void> {
    ...
    try {
      const strategies = this.options.bungee.getAvailableStrategies();
