...
export class RunTransactionEndpoint implements IWebServiceEndpoint {
  ...
  public get oasPath(): (typeof OAS.paths)["/api/v1/plugins/@hyperledger-cacti/cactus-plugin-ledger-connector-polkadot/run-transaction"] {
    return OAS.paths[
      "/api/v1/plugins/@hyperledger-cacti/cactus-plugin-ledger-connector-polkadot/run-transaction"
    ];
  }
  ...
  async handleRequest(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const errorMsg = `${reqTag} ${fnTag} Failed to perform transaction:`;
