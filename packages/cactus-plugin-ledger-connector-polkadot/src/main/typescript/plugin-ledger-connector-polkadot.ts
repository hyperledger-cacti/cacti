...
export class PluginLedgerConnectorPolkadot
  ...
  private contractAbi?: Abi | null;
  private contractCode?: CodePromise | null;
  ...
  public setProvider(wsProviderUrl: string): void {
    try {
      ...
      throw Error(`Could not create wsProvider. InnerException: + ${e.stack}`);
    }
  }
  ...
  public async getConsensusAlgorithmFamily(): Promise<
    ConsensusAlgorithmFamily
  > {
    return ConsensusAlgorithmFamily.Substrate;
  }
  ...
  public async transact(
    ...
    try {
      ...
      throw Error(
        `${fnTag} The transaction submission failed. ` +
          `InnerException: ${e.stack}`,
      );
    }
