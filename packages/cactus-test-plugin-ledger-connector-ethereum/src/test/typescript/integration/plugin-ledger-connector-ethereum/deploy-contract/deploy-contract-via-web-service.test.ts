...
describe(testCase, () => {
  ...
  beforeAll(async () => {
    ...
    const ledgerConnectorEthereum = new PluginLedgerConnectorEthereum({
      instanceId: uuidV4(),
      rpcApiHttpHost,
      pluginRegistry: new PluginRegistry({ plugins: [kvStoragePlugin] }),
    });
    plugins.push(ledgerConnectorEthereum);
    ...
    // Start the API server which now is connected to the ethereum ledger
    ...
    // Find a high net worth account in the genesis object of the ethereum ledger
    ethereumGenesisOptions = await ledger.getGenesisJsObject();
  ...
  test(testCase, async () => {
    expect(ethereumGenesisOptions);
    expect(ethereumGenesisOptions.alloc);
    ...
    // 7. Assemble request to invoke the deploy contract method of the ethereum ledger connector plugin via the REST API
