...
export class ConfigService {
  ...
  private static getConfigSchema(): Schema<IBifApiServerOptions> {
    return {
      ...
      bifNodeId: {
        doc:
          "Identifier of this particular BIF node. Must be unique among the total set of BIF nodes running in any " +
      ...
      cockpitWwwRoot: {
        ...
        default:
          "packages/bif-cmd-api-server/node_modules/@hyperledger-labs/bif-cockpit/www/",
      },
      apiHost: {
        doc:
      ...
      keychainSuffixPrivateKey: {
        doc:
          "The key under which to store/retrieve the private key from the keychain of this BIF node (API server)" +
      ...
      keychainSuffixPublicKey: {
        doc:
          "The key under which to store/retrieve the public key from the keychain of this BIF node (API server)" +
          "The complete lookup key is constructed from the ${BIF_NODE_ID}${KEYCHAIN_SUFFIX_PRIVATE_KEY} template.",
