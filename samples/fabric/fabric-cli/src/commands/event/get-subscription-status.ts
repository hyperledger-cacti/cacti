  ...
  run: async toolbox => {
    ...
    if (options.help || options.h) {
      commandHelp(
        print,
        toolbox,
        `fabric-cli event unsubscribe --network=<network1|network2> --user=user1 --request-id="abc123"`,
        'fabric-cli event unsubscribe --network=<network-name> --user=<user-id> --request-id=<request_id>',
        [
            {
                name: '--network',
                description:
