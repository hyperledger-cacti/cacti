...
const command: GluegunCommand = {
  ...
  run: async toolbox => {
    ...
    var funcToCall = AssetManager.reclaimAssetInHTLCusingContractId
    var asset = 'Asset'
    ...
    if (options['contract-id']) {
        try {
            ...
            print.error(`Could not unlock ${asset} in ${options['target-network']}`)
            spinner.fail(`Error`)
        }
    } else {
        try {
            ...
            spinner.info(`Trying ${asset} Unlock with params: ${params[0]}, ${params[1]} locked by ${options.locker} for ${options.recipient}`)
            const res = await AssetManager.reclaimAssetInHTLC(network.contract,
                          params[0],
                          params[1],
                          recipientCert)
            if (!res) {
