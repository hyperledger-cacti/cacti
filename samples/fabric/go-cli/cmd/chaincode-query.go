...
var queryCmd = &cobra.Command{
	Use:   "query --local-network=<network1|network2> --user=<user-id> <channel-name> <contract-name> <function-name> <args>",
	Short: "query chaincode with data",
	Long: `Query chaincode with data.

Example:
  fabric-cli chaincode query --local-network=network1 mychannel interop  Read '["a"]'`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		err := chaincodeQuery(args, localNetwork, username, logDebug)
		if err != nil {
...
func init() {
	...
	queryCmd.Flags().String("local-network", "", "local-network network for command. <network1|network2>")
	queryCmd.Flags().String("user", "", "user for chaincode invoke. (Optional: Default user is used)")
	queryCmd.Flags().String("debug", "false", "shows debug logs when running. Disabled by default. To enable --debug=true")
}

func chaincodeQuery(args []string, localNetwork string, username string, logDebug string) error {
	if len(args) < 4 {
