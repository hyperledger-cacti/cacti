  ...
  run: async toolbox => {
    ...
    if (options.help || options.h) {
      commandHelp(
        print,
        toolbox,
        `fabric-cli asset exchange-step --step=1 --network1=network1 --network2=network2 --secret=secrettext --timeout-duration=100 bob:Type1:A1234:alice:Type2:100`,
    ...
    if (options['step']==true) {
      ...
      try {
        res = AssetManager.createHTLC(network1.contract,
                        assetType,
                        assetId,
                        user2CertN1,
                        hash_secret,
      ...
      try {
        res = AssetManager.createFungibleHTLC(network2.contract,
                        fungibleAssetType,
                        fungibleAssetAmt,
                        user1CertN2,
                        hash_secret,
