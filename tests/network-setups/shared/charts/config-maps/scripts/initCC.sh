...
for org in $ORG_LIST; do
	...
	while [ $res -ne 0 ]; do
		set -x
		nc -zv -w30 "${CC_NAME}-${CHANNEL_NAME}-peer0-${org}-${NW_NAME}" 7052
		res=$?
		set +x
		sleep 5
	done
	while [ ! -d "/var/pvc1/channel_ready/peer0-${org}-${NW_NAME}-${CHANNEL_NAME}" ]; do
		echo "Could not find ${CHANNEL_NAME} on peer0-${org}-${NW_NAME}. Waiting..."
		sleep 5
	done
done

while [ ! -f "/var/pvc1/chaincodes/${CC_NAME}-${CHANNEL_NAME}-peer0-${org}-${NW_NAME}/pkgid" ]; do
	echo "Waiting for chaincode to be installed on peer..."
	sleep 5
done

echo "Chaincode Server Up, Running instantiation"

## approve the definition
for org in $ORG_LIST; do
	set -a
	. /var/pvc1/chaincodes/${CC_NAME}-${CHANNEL_NAME}-peer0-${org}-${NW_NAME}/pkgid
	set +a
	PACKAGE_ID=$CHAINCODE_CCID
	queryInstalled ${org}
  approveForMyOrg ${org}
done
