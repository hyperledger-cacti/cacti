...
echo "Five tmux sessions will be started to run EthSigner and the four validator nodes:"

tmux new-session -d -s EthSigner_session 'cd  SampleBesuNetwork; ethsigner --chain-id=1337 --downstream-http-port=8590 multikey-signer --directory=keys/'
...
tmux new-session -d -s Node-1_session 'cd SampleBesuNetwork/Node-1/; besu --config-file=config1.toml'
#echo "Terminal 2: Go to SampleBesuNetwork/Node-1/; Run besu --config-file=config1.toml"

tmux new-session -d -s Node-2_session 'bash scripts/Node_session.sh 2'
...
tmux new-session -d -s Node-3_session 'bash scripts/Node_session.sh 3'
...
tmux new-session -d -s Node-4_session 'bash scripts/Node_session.sh 4'
