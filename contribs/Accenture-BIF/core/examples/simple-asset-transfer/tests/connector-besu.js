...
const ConnectorBesuEx = require(`../besu/connector`);
const config = require(`./config`);
const { Multisig } = require(`@hyperledger-labs/blockchain-integration-framework`);

chai.use(cp);

describe(`ConnectorBesuExample Constructor`, function() {
  ...
  it(`Non provided Besu-specific param should throw`, function() {
    const options = {
...
describe(`ConnectorBesuExample getAccessToken`, function() {
  const options = config.blockchains.besu;
  const connector1 = new ConnectorBesuEx(options);
...
describe(`ConnectorBesuExample addForeignValidator`, function() {
  const options = config.blockchains.besu;
  const connector2 = new ConnectorBesuEx(options);
...
describe(`ConnectorBesuExample verifySignature`, function() {
  const options = config.blockchains.besu;
  const connector3 = new ConnectorBesuEx(options);
...
describe(`ConnectorBesuExample verifyMultisig`, function() {
  const options = config.blockchains.besu;
  const connector4 = new ConnectorBesuEx(options);
...
describe(`ConnectorBesuExample CreateAsset`, function() {
  const options = config.blockchains.besu;
  const connector4 = new ConnectorBesuEx(options);
...
describe(`ConnectorBesuExample LockAsset`, function() {
  const options = config.blockchains.besu;
  const connector5 = new ConnectorBesuEx(options);
...
describe(`ConnectorBesuExample SetProperty`, function() {
  const options = config.blockchains.besu;
  const connector6 = new ConnectorBesuEx(options);
  const asset5 = {
...
describe(`ConnectorBesuExample GetAsset`, function() {
  const options = config.blockchains.besu;
  const connector7 = new ConnectorBesuEx(options);
  const asset6 = {
...
describe(`ConnectorBesuExample copyAsset`, function() {
  const options = config.blockchains.besu;
  const connector4 = new ConnectorBesuEx(options);
  ...
  it(`Copy Asset with wrong signature`, function(done) {
    ...
    const signatures = { [pubkey]: signature };
    chai
      .expect(connector4.copyAsset({ numGood: 2, msg, signatures }).catch(err => err.message))
      .to.eventually.contain(`Transaction Failed`)
      .notify(done);
  });
});
