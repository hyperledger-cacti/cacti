...
1. Client prepares `NetworkQuery` and sends it to Destination Relay via `RequestState` (Client App) API.
  ...
  * **1.4a:** Source relay (inside Relay’s `RequestState`) returns `Ack` back to the destination relay (happens asynchronously with step **1.5**).
  ...
  * **1.5a:** Driver returns `Ack` back to the source relay (happens asynchronously with step **1.7**).
  ...
  * **1.8a:** Source relay (inside `SendDriverState`) returns `Ack` back to the driver (happens asynchronously with step **1.11**).
