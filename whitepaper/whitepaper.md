...
| Failure Criteria           | States of all assets, data, and transaction history on the target blockchain do not match the states on the source blockchain before migration. `Consortium A` is unable to migrate to the target blockchain for several reasons, such as inability to recreate the same smart contract logic, inability to arbitrary recreate native assets on target blockchain, and lack of access to private keys of external accounts. |
