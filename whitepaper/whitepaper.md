...
| Use Case                   | 1. `User A` runs an application/service on a source blockchain that is either permissioned or permissionless.<br>2. `User A` decides to use another blockchain instance (permissioned or permissionless) to run his/her application. <br>3. `User A` migrates the existing assets and data to the target blockchain.<br>4. `User A` runs the application/service on the target blockchain.
| Interworking patterns      | Value transfer, Data transfer |
| Type of Social Interaction | Asset and Data Transfer |
| Narrative                  | `User A` who's application/service is operating on a source blockchain (e.g., a Hyperledger Fabric instance), would want to migrate the assets, data, and functionality to a target blockchain (e.g., Hyperledger Besu) to expand their reach. However, such migration requires lots of technical effort and time. The `Blockchain Migration` feature from `Cactus` can provide support for doing so, by connecting to the source and target blockchains, and performing the migration task.
| Actors                     | 1. `User A`: The entity operating on the source blockchain which aims at performing a migration to the target blockchain. |
| Goals of Actors            | 1. `User A`: Wants to be able to operate his/her application on the target blockchain while gaining its benefits such as better performance, lower cost, or enhanced privacy. |
| Success Scenario           | Asset and data are available on the target blockchain enabling `User A`'s application to operate on the target blockchain.|
...
| Failure Criteria           | All desired assets and data, as well as their histroy not on the target blockchain. `User A` is unable migrate to target blockchain for a number of reasons, such as inability to recreate the same smart contract logic, inability to arbitary recreate native assets on target blockchain, and lack of access to private keys of external accounts. It may also be cost and time prohibitive to recreate entire history on the target blockchain. |
| Prerequisites              | 1. `User A` wants to migrate the data on source blockchain.<br>2. Migration plan and window are informed to all members of the application's governance body, and they agree to the migration.<br>3. `User A` has write and execute permissions on the target blockchain. |
| Comments                   | * `User A` is the governance body of an application/service running on the blockchain or the source blockchain itself (e.g., consortium).<br> * Data include smart contracts and their data originating from the source blockchain. Depending on the use case, a subset of the assets, data, and their history may be recreated on the target blockchain.<br>* This use case relates to the use cases implying asset and data portability (e.g., 2.1). However, migration is mostly one-way and nonreversible.<br>* This use case provides blockchain portability; thus, retains blockchain properties across migration, reduces costs, time to migration, and foster blockchain adoption. |
...
It is, therefore, desirable for an organization to be able to replace the blockchain providing the infrastructure to a particular application/service.

Currently, when a user wants to migrate their application or consortium wants to migrate the entire blockchain (e.g., the source blockchain became obsolete, cryptographic algorithms are no longer secure, and business reasons), the solution is to re-implement business logic using a different blockchain platform, and arbitrary recreate the assets and data on the target blockchain, yielding great effort and time, as well as losing blockchain properties such as immutability, consistency, and transparency.
...
Based on those characteristics and user-defined requirements, `Cactus` proposes a migration with indicators such as predicted cost, time to complete the migration, and likelihood of success.
