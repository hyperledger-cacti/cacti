...
for org in $ORG_LIST; do
	set -x
	nc -zv -w5 "${CC_NAME}-peer0-${org}-${NW_NAME}" 7052
	res=$?
	set +x
	verifyResult $res "Could not find chaincode server at ${CC_NAME}-peer0-${org}-${NW_NAME}:7052"
done
...
for org in $ORG_LIST; do
	set -a
	. /var/pvc1/chaincodes/${CC_NAME}-peer0-${org}-${NW_NAME}/pkgid
	set +a
