...
async function performLockHelper(
  ...
  performLockRequest2["hashBase64"] =
    "ivHErp1x4bJDKuRo6L5bApO/DdoyD/dG0mAZrzLZEIs=";
  ...
  const hashFn = performLockRequest2["hash_fn"];
  const hashBase64 = performLockRequest2["hashBase64"];
  ...
  try {
    ...
    logger.info(
      `Trying ${asset} Lock: ${params[0]}, ${params[1]} by ${locker} for ${recipient}`,
    );
    ...
    logger.info(
      `${asset} Locked with Contract Id: ${res.result}, preimage: ${res.hash.getPreimage()}, hashvalue: ${res.hash.getSerializedHashBase64()}`,
    );
...
async function createAssetHelper(
  ...
  createAssetRequest2["hashBase64"] =
    "ivHErp1x4bJDKuRo6L5bApO/DdoyD/dG0mAZrzLZEIs=";
  ...
  try {
    ...
    logger.info(
      `Trying creating the asset: type: ${ccType}, id: ${id}, by: ${owner}, facevalue: ${facevalue}, maturitydate: ${maturitydate}`,
    );
...
async function extinguishHelper(extinguishRequest: driverPb.ExtinguishRequest) {
...
async function assignAssetHelper(
  ...
  assignAssetRequest2["hashBase64"] =
    "ivHErp1x4bJDKuRo6L5bApO/DdoyD/dG0mAZrzLZEIs=";
  ...
  if (fungible) {
    ...
    try {
      ...
      logger.info(
        `Trying assign asset with params: ${params[0]}, ${params[1]} locked by ${locker} for ${recipient}`,
      );
      const res = await funcToCall(
        contract,
        params[0],
      ...
      logger.error(
        `Could not assign non-fungible ${asset} in ${targetNetwork}: ${error}`,
      );
      throw new Error(
        `Could not assign non-fungible ${asset} in ${targetNetwork}: ${error}`,
      );
