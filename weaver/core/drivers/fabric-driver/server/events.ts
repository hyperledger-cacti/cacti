...
import { getDriverKeyCert } from "./walletSetup.js";
import { DBConnector, DBKeyNotFoundError, LevelDBConnector } from "./dbConnector.js";
import { checkIfArraysAreEqual, handlePromise, relayCallback } from "./utils.js";
import { registerListenerForEventSubscription, unregisterListenerForEventSubscription } from "./listener.js";
import { getNetworkGateway } from "./fabric-code.js";
...
async function subscribeEventHelper(
    ...
    if (error) {
        ...
        // gRPC response.
        logger.info(`Sending to the relay the eventSubscription error Ack: ${JSON.stringify(ack_send_error.toObject())}`);
        ...
        // gRPC response.
        logger.info(`Sending to the relay the eventSubscription Ack: ${JSON.stringify(ack_send.toObject())}`);
...
async function addEventSubscription(
    ...
    logger.info(`adding to driver, subscription of the eventSub: ${JSON.stringify(eventSub.toObject())}`);
    let db: DBConnector;
    try {
        ...
        try {
            ...
            for (const subscriptionSerialized of subscriptions) {
                ...
                if (checkIfArraysAreEqual(subscription.getPolicyList(), query.getPolicyList()) &&
                    ...
                    logger.info(`found subscription for query with requestId: ${subscription.getRequestId()}`);
                    await db.close();
...
const deleteEventSubscription = async (
    ...
    logger.info(`deleting from driver subscription of the eventMatcher: ${JSON.stringify(eventMatcher.toObject())} with requestId: ${requestId}`);
    var subscriptions: Array<string>;
...
async function signEventSubscriptionQuery(
    ...
    logger.info(`driver ready to provide sign on query: ${JSON.stringify(inputQuery.toObject())}`);
