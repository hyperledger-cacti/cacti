...
describe("Membership", () => {
  ...
  const cert = fs
    ...
    .toString();
  const cordaCert = fs
    .readFileSync(`${__dirname}/data/cordaCert.pem`)
    .toString();
  ...
  const nonce = "123";
  const fabricAttestedMembership = getForeignAttestedMembership(
    `${__dirname}/data/fabricMembership.json`,
    fabricMemberId,
  );
  const cordaAttestedMembership = getForeignAttestedMembership(
    `${__dirname}/data/cordaMembership.json`,
    cordaMemberId,
  );
  ...
  function getForeignAttestedMembership(filename, memberId) {
    ...
    const membershipSerialized64 = Buffer.from(
  ...
  describe("verify attestation", () => {
    it("test success", async () => {
      const verify = utils.validateAttestedMembership(
        fabricAttestedMembership.getMembership(),
        nonce,
        fabricAttestedMembership.getAttestation(),
      );
      expect(verify).to.equal(true);
    });
    it("test fail 1", async () => {
  ...
  describe("verify Fabric Member In Membership", () => {
    it("test success", async () => {
      const membership = utils.deserializeMembership64(
        fabricAttestedMembership.getMembership(),
      );
      const verify = utils.verifyMemberInMembership(
        membership,
        fabricAttestedMembership.getAttestation(),
      );
      expect(verify).to.equal(true);
  ...
  describe("verify Corda Member In Membership", () => {
    it("test success", async () => {
      const membership = utils.deserializeMembership64(
        cordaAttestedMembership.getMembership(),
      );
