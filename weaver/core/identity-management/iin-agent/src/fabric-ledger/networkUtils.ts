...
import membershipPb from "@hyperledger-cacti/cacti-weaver-protos-js/common/membership_pb";
...
const getNetworkGateway = async (
  ...
  try {
    ...
    if (!fs.existsSync(connectionProfilePath)) {
      throw new Error(
        "Connection profile does not exist at path: " + connectionProfilePath,
      );
    }
    const ccp = JSON.parse(
      fs.readFileSync(connectionProfilePath, "utf8").toString(),
    );
    ...
    if (!identity) {
      console.log(
        `An identity for the user "${userName}.com" does not exist in the wallet`,
      );
    ...
    await gateway.connect(ccp, {
      ...
      discovery: {
        enabled: true,
        asLocalhost: config.local === "false" ? false : true,
      },
    });
    return gateway;
  } catch (error) {
...
const getNetworkContract = async (
  ...
  try {
    const gateway = await getNetworkGateway(
      walletPath,
...
async function getMSPConfiguration(
  ...
  try {
    ...
    const membership = MembershipManager.getMSPConfigurations(network, [
      config.mspId,
    ]);
...
async function getAllMSPConfigurations(
  ...
  try {
    ...
    const membership = MembershipManager.getAllMSPConfigurations(
      network,
      config.ordererMspIds,
    );
    // Disconnect from the gateway.
    gateway.disconnect();
    return membership;
...
async function invokeFabricChaincode(
  ...
  console.log("Running invocation on Fabric channel and chaincode");
  try {
    const gatewayAndContract = await getNetworkContract(
      walletPath,
      connectionProfilePath,
      configFilePath,
      channelId,
