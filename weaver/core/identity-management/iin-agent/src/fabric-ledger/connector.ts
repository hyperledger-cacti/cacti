...
export class FabricConnector extends LedgerBase {
  ...
  constructor(
    ...
    this.connectionProfilePath =
      config.ccpPath && config.ccpPath.length > 0
        ? config.ccpPath
        : path.resolve(__dirname, "./", "connection_profile.json");
    if (!fs.existsSync(this.connectionProfilePath)) {
      throw new Error(
        "Connection profile does not exist at path: " +
          this.connectionProfilePath,
      );
    }
    this.walletPath =
      config.walletPath && config.walletPath.length > 0
        ? config.walletPath
        : path.join(process.cwd(), `wallet-${this.networkId}-${this.memberId}`);
  }
  ...
  async getAttestedMembership(
    securityDomain: string,
    nonce: string,
  ): Promise<iin_agent_pb.AttestedMembership> {
    const membership = await getAllMSPConfigurations(
      this.walletPath,
      this.connectionProfilePath,
      this.configFilePath,
      this.ledgerId,
    ...
    const membershipSerializedBase64 = Buffer.from(
      membership.serializeBinary(),
    ).toString("base64");
    const certAndSign = await this.agentSignMessage(
      membershipSerializedBase64 + nonce,
    );
  ...
  // Collect security domain membership info
  async counterAttestMembership(
    attestedMembershipSetSerialized64: string,
    securityDomain: string,
    nonce: string,
  ): Promise<iin_agent_pb.CounterAttestedMembership> {
    const certAndSign = await this.agentSignMessage(
      attestedMembershipSetSerialized64 + nonce,
    );
  ...
  // record Membership
  async recordMembershipInLedger(
    counterAttestedMembership: iin_agent_pb.CounterAttestedMembership,
  ): Promise<any> {
    const counterAttestedMembershipSerialized64 = Buffer.from(
      counterAttestedMembership.serializeBinary(),
    ).toString("base64");
    try {
      const res = await invokeFabricChaincode(
        this.walletPath,
        this.connectionProfilePath,
        this.configFilePath,
        this.ledgerId,
        this.contractId,
        "CreateMembership",
        [counterAttestedMembershipSerialized64],
      );
      return res;
    } catch (e) {
      return await invokeFabricChaincode(
        this.walletPath,
        this.connectionProfilePath,
        this.configFilePath,
        this.ledgerId,
  ...
  async queryContract(): Promise<string> {
    return await queryFabricChaincode(
      this.walletPath,
      this.connectionProfilePath,
      this.configFilePath,
      this.ledgerId,
  ...
  private async agentSignMessage(
    message,
  ): Promise<{ certificate: string; signature: string }> {
    const wallet = await getWallet(this.walletPath);
    type KeyCert = { key: any; cert: any };
    const keyCert =
      await InteroperableHelper.getKeyAndCertForRemoteRequestbyUserName(
        wallet,
        this.iinAgentUserName,
      );
    const signature = signMessage(message, keyCert.key.toBytes());
