...
export const securityDomainMap = new Map<
  string,
  string | iin_agent_pb.AttestedMembership
>();
export const counterAttestationsMap = new Map<
  string,
  string | iin_agent_pb.CounterAttestedMembership
>();

// key: Nonce
export const foreignAgentResponseCount = new Map<
  string,
  { current: number; total: number }
>();
export const localAgentResponseCount = new Map<
  string,
  { current: number; total: number }
>();

export function getSecurityDomainMapKey(
...
export function syncExternalStateFromIINAgent(
  remoteSecurityDomainUnit: iin_agent_pb.SecurityDomainMemberIdentity,
  securityDomain: string,
  memberId: string,
) {
  const nonce = uuidv4();
  const remoteSecurityDomain = remoteSecurityDomainUnit.getSecurityDomain();
  const remoteSecurityDomainDNS =
    utils.getSecurityDomainDNS(remoteSecurityDomain);
  ...
  syncExternalStateFromIINAgentHelper(
    remoteSecurityDomainUnit,
    securityDomain,
    memberId,
    nonce,
    remoteSecurityDomainDNS,
  )
    .then()
    .catch((error) => {
...
const syncExternalStateFromIINAgentHelper = async (
  remoteSecurityDomainUnit: iin_agent_pb.SecurityDomainMemberIdentity,
  securityDomain: string,
  memberId: string,
  nonce: string,
  remoteSecurityDomainDNS: any,
) => {
  const remoteSecurityDomain = remoteSecurityDomainUnit.getSecurityDomain();
  console.log(
    "syncExternalStateFromIINAgent:",
    remoteSecurityDomain,
    "-",
    remoteSecurityDomainUnit.getMemberId(),
    " nonce:",
    nonce,
  );

  const requestingSecurityDomain =
    new iin_agent_pb.SecurityDomainMemberIdentity();
  ...
  for (const iinAgent in remoteSecurityDomainDNS) {
    const iinAgentClient = utils.getIINAgentClient(
      remoteSecurityDomain,
      iinAgent,
      remoteSecurityDomainDNS,
    );
    ...
    console.log(
      `Requesting attested memberships from: ${remoteSecurityDomain} - ${iinAgent}`,
    );
    iinAgentClient.requestIdentityConfiguration(request, utils.defaultCallback);
    totalAttesterCount++;
  }
  foreignAgentResponseCount.set(nonce, {
    current: 0,
    total: totalAttesterCount,
  });
};

// Generates security domain unit's state/configuration
export const requestIdentityConfiguration = async (
  request: iin_agent_pb.SecurityDomainMemberIdentityRequest,
) => {
  ...
  if (ledgerBase.memberId != sourceMemberId) {
    throw new Error(
      `This IIN Agent's member Id: ${ledgerBase.memberId} doesn't match with provided member Id: ${sourceMemberId} in request.`,
    );
  }
  const [attestedMembership, error] = await utils.handlePromise(
    ledgerBase.getAttestedMembership(sourceSecurityDomain, request.getNonce()),
  );
  const iinAgentClient = utils.getIINAgentClient(
    request.getRequestingNetwork()!.getSecurityDomain(),
    request.getRequestingNetwork()!.getMemberId(),
  );
  if (error) {
...
export function sendIdentityConfiguration(
  attestedMembership: iin_agent_pb.AttestedMembership,
  securityDomain: string,
  memberId: string,
) {
  ...
  sendIdentityConfigurationHelper(
    attestedMembership,
    securityDomain,
    memberId,
    nonce,
...
const sendIdentityConfigurationHelper = async (
  attestedMembership: iin_agent_pb.AttestedMembership,
  securityDomain: string,
  memberId: string,
  nonce: string,
  ...
  const secDomMapKey = getSecurityDomainMapKey(
    remoteSecurityDomain,
    remoteMemberId,
    nonce,
  );
  ...
  try {
    ...
    securityDomainMap.set(
      getSecurityDomainMapKey(remoteSecurityDomain, remoteMemberId),
      attestedMembership,
    );
  } catch (e) {
  ...
  const currForeignAgentResponsesCount =
    foreignAgentResponseCount.get(nonce).current;
  const totalForeignAgentResponsesCount =
    foreignAgentResponseCount.get(nonce).total;
  foreignAgentResponseCount.set(nonce, {
    current: currForeignAgentResponsesCount + 1,
    total: totalForeignAgentResponsesCount,
  });
  ...
  for (const remoteAgent in remoteSecurityDomainDNS) {
    const key = getSecurityDomainMapKey(
      remoteSecurityDomain,
      remoteAgent,
      nonce,
    );
    ...
    attestedMembershipOrError =
      attestedMembershipOrError as iin_agent_pb.AttestedMembership;
  ...
  const localResponderKey = getSecurityDomainMapKey(
    "LOCAL_COUNTER_ATTESTATION_REQUEST",
    "",
    nonce,
  );
  if (counterAttestationsMap.has(localResponderKey)) {
    const requestingMemberCounterAttestation = counterAttestationsMap.get(
      localResponderKey,
    ) as iin_agent_pb.CounterAttestedMembership;
    const requestingMember = requestingMemberCounterAttestation
      .getAttestationsList()[0]
      .getUnitIdentity()!;
    const attestedMembershipSetSerialized64 =
      requestingMemberCounterAttestation.getAttestedMembershipSet();

    let counterAttestedMembership;
    if (errorMsg.length === 0) {
      counterAttestedMembership = await ledgerBase.counterAttestMembership(
        attestedMembershipSetSerialized64,
        securityDomain,
        nonce,
      );
    } else {
      counterAttestedMembership = utils.generateErrorCounterAttestation(
        errorMsg,
        securityDomain,
        memberId,
        nonce,
    ...
    const iinAgentClient = utils.getIINAgentClient(
      requestingMember.getSecurityDomain(),
      requestingMember.getMemberId(),
    );
    iinAgentClient.sendAttestation(
    ...
    if (errorMsg.length === 0) {
      const attestedMembershipSet =
        new iin_agent_pb.CounterAttestedMembership.AttestedMembershipSet();
      attestedMembershipSet.setMembership(membership);
      attestedMembershipSet.setAttestationsList(attestations);
      const attestedMembershipSetSerialized64 = Buffer.from(
        attestedMembershipSet.serializeBinary(),
      ).toString("base64");

      console.log(
        "Received Attested Membership Set",
        JSON.stringify(attestedMembershipSet.toObject()),
      );

      const myCounterAttestedMembership =
        await ledgerBase.counterAttestMembership(
          attestedMembershipSetSerialized64,
          securityDomain,
          nonce,
        );
      ...
      for (const iinAgent in securityDomainDNS) {
        ...
        console.log(
          `Requesting counter attested memberships from: ${securityDomain} - ${iinAgent}`,
        );
        iinAgentClient.requestAttestation(
          myCounterAttestedMembership,
          utils.defaultCallback,
        );
      ...
      if (totalCounterAttesterCount === 0) {
        ...
        if (resultError) {
          console.error(
            "Error submitting counter attested membership to ledger:",
            resultError,
          );
        } else {
          console.log(
            `Succesfully recorded membership of ${remoteSecurityDomain} with result: ${result}`,
          );
        }
      } else {
        localAgentResponseCount.set(nonce, {
          current: 0,
          total: totalCounterAttesterCount,
        });
        counterAttestationsMap.set(
          getSecurityDomainMapKey(
            "LOCAL_COUNTER_ATTESTATION_RESPONSE",
            memberId,
            nonce,
          ),
          myCounterAttestedMembership,
      ...
      console.error(
        "Error while fetching attested membership from foreign network:",
        errorMsg,
      );
      return;
    }
