...
import {
  localAgentResponseCount,
  counterAttestationsMap,
  getSecurityDomainMapKey,
  securityDomainMap,
  foreignAgentResponseCount,
...
export const requestAttestation = async (
  counterAttestedMembership: iin_agent_pb.CounterAttestedMembership,
  localSecurityDomain: string,
  localMemberId: string,
  refreshTime: number,
) => {
  // create local security domain
  const localSecurityDomainUnit =
  ...
  const attestedMembershipSet = utils.deserializeAttestedMembershipSet64(
    counterAttestedMembership.getAttestedMembershipSet(),
  );
  ...
  for (const attestation of attestations) {
    ...
    if (
      ...
      typeof attestedMembershipOrError === "string"
    ) {
      ...
      if (
        Math.floor((timeStampCurrent - timeStampCached) / 1000) > refreshTime
      ) {
        // fetch fresh membership info (move to a separate function)
        // create a request
        const request = new iin_agent_pb.SecurityDomainMemberIdentityRequest();
  ...
  const requesterMemberID = counterAttestations[0]
    .getUnitIdentity()!
    .getMemberId();
  if (counter > 0) {
  ...
  const requesterSecurityDomain = counterAttestations[0]
    .getUnitIdentity()!
    .getSecurityDomain();
  const requesterSecurityDomainDNS = utils.getSecurityDomainDNS(
    requesterSecurityDomain,
  );
  const requesterIINAgentClient = utils.getIINAgentClient(
    requesterSecurityDomain,
    requesterMemberID,
    requesterSecurityDomainDNS,
  );
  if (errorMsg != "") {
    // send the error back to the requester
    const errorCounterAttestedMembership =
      utils.generateErrorCounterAttestation(
        errorMsg,
        localSecurityDomain,
        localMemberId,
        nonce,
      );
    requesterIINAgentClient.sendAttestation(
      errorCounterAttestedMembership,
      utils.defaultCallback,
    );
  } else {
    // create new attestations on foreign security domain's state info sent by requester
    const localLedgerBase = utils.getLedgerBase(
      localSecurityDomain,
      localMemberId,
    );
    const localCounterAttestedMembership =
      await localLedgerBase.counterAttestMembership(
        counterAttestedMembership.getAttestedMembershipSet(),
        localSecurityDomain,
        nonce,
      );
...
export function sendAttestation(
  ...
  if (!attestation.hasUnitIdentity()) {
    const errorMsg =
      "attestation has no SecurityDomainMemberIdentity associated with it";
  ...
  const localKey = getSecurityDomainMapKey(
  ...
  if (
    !(
      counterAttestationsMap.has(localKey) && localAgentResponseCount.has(nonce)
    )
  ) {
...
const sendAttestationHelper = async (
  counterAttestedMembership: iin_agent_pb.CounterAttestedMembership,
  securityDomain: string,
  memberId: string,
  peerAgentMemberId: string,
  nonce: string,
) => {
  const attestation = counterAttestedMembership.getAttestationsList()[0];
  ...
  const myCounterAttestedMembership = counterAttestationsMap.get(
    localKey,
  ) as iin_agent_pb.CounterAttestedMembership;
  const attestedMembershipSet64 =
    myCounterAttestedMembership.getAttestedMembershipSet();
  const attestedMembershipSet =
    iin_agent_pb.CounterAttestedMembership.AttestedMembershipSet.deserializeBinary(
      Buffer.from(attestedMembershipSet64, "base64"),
    );
  const remoteSecurityDomain = attestedMembershipSet
    .getAttestationsList()[0]
    .getUnitIdentity()!
    .getSecurityDomain();

  const counterAttestationsMapKey = getSecurityDomainMapKey(
    remoteSecurityDomain,
    peerAgentMemberId,
    nonce,
  );
  ...
  const currLocalAgentResponsesCount =
    localAgentResponseCount.get(nonce).current;
  const totalLocalAgentResponsesCount =
    localAgentResponseCount.get(nonce).total;
  localAgentResponseCount.set(nonce, {
    current: currLocalAgentResponsesCount + 1,
    total: totalLocalAgentResponsesCount,
  });
  ...
  const counterAttestations = myCounterAttestedMembership.getAttestationsList();
  ...
  for (const localAgent in securityDomainDNS) {
    ...
    counterAttestedMembershipOrError =
      counterAttestedMembershipOrError as iin_agent_pb.CounterAttestedMembership;
    const attestation =
      counterAttestedMembershipOrError.getAttestationsList()[0];
    ...
    if (
      attestedMembershipSet64 !==
      counterAttestedMembershipOrError.getAttestedMembershipSet()
    ) {
  ...
  if (errorMsg.length === 0) {
    ...
    console.log(
      "Received Counter Attested Membership",
      JSON.stringify(counterAttestedMembership.toObject()),
    );
