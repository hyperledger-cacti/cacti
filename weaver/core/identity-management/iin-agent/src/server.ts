...
} from "./protocols/externalOperations";
import {
  requestAttestation,
  sendAttestation,
} from "./protocols/localOperations";
import {
  getLedgerBase,
  delay,
  getAllRemoteSecurityDomainDNS,
} from "./common/utils";
...
iinAgentServer.addService(iin_agent_pb_grpc.IINAgentService, {
  ...
  syncExternalState: (
    call: { request: iin_agent_pb.SecurityDomainMemberIdentity },
    callback: (_: any, object: ack_pb.Ack) => void,
  ) => {
    const ack_response = new ack_pb.Ack();
    try {
      const securityDomain = process.env.SECURITY_DOMAIN
        ? process.env.SECURITY_DOMAIN
        : "network1";
      const memberId = process.env.MEMBER_ID
        ? process.env.MEMBER_ID
        : "Org1MSP";
      const nonce = syncExternalStateFromIINAgent(
        call.request,
        securityDomain,
        memberId,
      );
  ...
  requestIdentityConfiguration: (
    call: { request: iin_agent_pb.SecurityDomainMemberIdentityRequest },
    callback: (_: any, object: ack_pb.Ack) => void,
  ) => {
    const ack_response = new ack_pb.Ack();
    try {
  ...
  sendIdentityConfiguration: (
    call: { request: iin_agent_pb.AttestedMembership },
    callback: (_: any, object: ack_pb.Ack) => void,
  ) => {
    const ack_response = new ack_pb.Ack();
    let nonce = "";
  ...
  requestAttestation: (
    call: { request: iin_agent_pb.CounterAttestedMembership },
    callback: (_: any, object: ack_pb.Ack) => void,
  ) => {
    const ack_response = new ack_pb.Ack();
    try {
      ...
      const memberId = process.env.MEMBER_ID
        ? process.env.MEMBER_ID
        : "Org1MSP";
      const attestationValidityTime = process.env.ATTESTATION_VALIDITY_TIME
        ? parseInt(process.env.ATTESTATION_VALIDITY_TIME)
        : 3600;
      requestAttestation(
        call.request,
        securityDomain,
        memberId,
        attestationValidityTime,
      );
      ack_response.setMessage("");
      ack_response.setStatus(ack_pb.Ack.STATUS.OK);
      ack_response.setRequestId("");
...
const loopSyncExternalState = async () => {
  const delayTime: number = parseInt(
    process.env.SYNC_PERIOD ? process.env.SYNC_PERIOD : "3600",
  );
  console.log("SYNC PERIOD: ", delayTime);
  const localSecurityDomain = process.env.SECURITY_DOMAIN
    ? process.env.SECURITY_DOMAIN
    : "network1";
  const localMemberId = process.env.MEMBER_ID
    ? process.env.MEMBER_ID
    : "Org1MSP";
...
if (process.env.IIN_AGENT_TLS === "true") {
  if (
    !(
      process.env.IIN_AGENT_TLS_CERT_PATH &&
      fs.existsSync(process.env.IIN_AGENT_TLS_CERT_PATH) &&
      process.env.IIN_AGENT_TLS_KEY_PATH &&
      fs.existsSync(process.env.IIN_AGENT_TLS_KEY_PATH)
    )
  ) {
    throw new Error(
      "Missing or invalid IIN agent TLS credentials on " +
