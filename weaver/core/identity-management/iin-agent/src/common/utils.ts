...
export function getIINAgentClient(
  ...
  if (!(participantId in securityDomainDNS)) {
    throw new Error(
      `DNS for member: ${participantId} of ${securityDomain} not defined in DNS Config`,
    );
  }
  ...
  if (iinAgent.tls === "true") {
    if (!iinAgent.tlsCACertPath || iinAgent.tlsCACertPath == "") {
      ...
      if (!(iinAgent.tlsCACertPath && fs.existsSync(iinAgent.tlsCACertPath))) {
        throw new Error(
          "Missing or invalid IIN Agent's tlsCACertPaths: " +
            iinAgent.tlsCACertPath,
        );
      }
...
export function getLedgerBase(
  ...
  if (dltType == "fabric") {
    const ledgerBase = new FabricConnector(
      ledgerId,
      process.env.WEAVER_CONTRACT_ID,
      securityDomain,
...
export function getLedgerId(securityDomain: string): any {
  const secDomConfigPath = process.env.SECURITY_DOMAIN_CONFIG_PATH
    ? process.env.SECURITY_DOMAIN_CONFIG_PATH
    : path.resolve(__dirname, "../", "../", "security-domain-config.json");
  if (!fs.existsSync(secDomConfigPath)) {
    throw new Error(
      "Security Domain config does not exist at path: " + secDomConfigPath,
    );
  }
  const secDomConfig = JSON.parse(
    fs.readFileSync(secDomConfigPath, "utf8").toString(),
  );
  return secDomConfig[securityDomain];
}

export function getSecurityDomainDNS(securityDomain: string): any {
  const dnsConfigPath = process.env.DNS_CONFIG_PATH
    ? process.env.DNS_CONFIG_PATH
    : path.resolve(__dirname, "../", "../", "dnsconfig.json");
  if (!fs.existsSync(dnsConfigPath)) {
    throw new Error("DNS config does not exist at path: " + dnsConfigPath);
  }
  const dnsConfig = JSON.parse(
    fs.readFileSync(dnsConfigPath, "utf8").toString(),
  );
  return dnsConfig[securityDomain];
}

export function getAllRemoteSecurityDomainDNS(
...
export function verifySignature(
  ...
  return InteroperableHelper.verifySignature(
    message,
    certificate,
    signature,
    algorithm,
...
export function deserializeMembership64(
  dataSerialized64: string,
): membership_pb.Membership {
  return membership_pb.Membership.deserializeBinary(
    Uint8Array.from(Buffer.from(dataSerialized64, "base64")),
  );
}
export function deserializeAttestedMembershipSet64(
  dataSerialized64: string,
): agent_pb.CounterAttestedMembership.AttestedMembershipSet {
  return agent_pb.CounterAttestedMembership.AttestedMembershipSet.deserializeBinary(
    Uint8Array.from(Buffer.from(dataSerialized64, "base64")),
  );
}

export function validateAttestedMembership(
  membershipSerialized64: string,
  nonce: string,
  attestation: agent_pb.Attestation,
) {
  ...
  if (nonce !== nonce_attestation) {
    console.error(
      `Error: Nonce doesn't match. Expected: ${nonce}, received: ${nonce_attestation}`,
    );
    return false;
  }
...
export function verifyMemberInMembership(
  membership: membership_pb.Membership,
  attestation: agent_pb.Attestation,
) {
  ...
  if (securityDomain !== membership.getSecuritydomain()) {
    console.error(
      `Error: Security domain of attestor doesn't match. Expected: ${membership.getSecuritydomain()}, Attestor's: ${securityDomain}`,
    );
    return false;
  }
...
function validateCertificateUsingCA(
  cert: string,
  signerCACert: string,
  isSignerRootCA: boolean,
): boolean {
  ...
  if (!isCertificateWithinExpiry(x509Cert)) {
    console.error(
      `Certificate is outside of validity. Cert validity from ${x509Cert.validFrom} to ${x509Cert.validTo}`,
    );
    return false;
  }
  if (x509Cert.issuer !== x509SignerCACert.subject) {
    console.error(
      `Certificate issuer ${x509Cert.issuer} does not match signer subject ${x509SignerCACert.subject}`,
    );
    return false;
  }
  return true;
...
export function generateErrorAttestation(
  errorMsg,
  securityDomain,
  memberId,
  nonce,
...
export function generateErrorCounterAttestation(
  ...
  const errorCounterAttestedMembership =
    new agent_pb.CounterAttestedMembership();
