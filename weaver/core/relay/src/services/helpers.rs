...
pub fn update_event_subscription_status(
    ...
    let driver_error_constants = fs::read_to_string("./driver/driver-error-constants.json")
        .expect("Unable to read file: ./driver/driver-error-constants.json");
    ...
    let driver_sub_exists_error = driver_error_constants_json
        .get("SUB_EXISTS")
        .unwrap()
        .as_str()
        .unwrap();
    let driver_sub_exists_error_without_args = *(driver_sub_exists_error
        .split("{0}")
        .collect::<Vec<&str>>()
        .first()
        .unwrap());
        ...
        Ok(fetched_event_sub_state) => {
            ...
            if new_status == ack::Status::Ok {
                    ...
                    Some(status) => match status {
                        event_subscription_state::Status::UnsubscribePendingAck => {
                            target = EventSubscriptionState {
                                ...
                                publishing_request_id: fetched_event_sub_state
                                    .publishing_request_id
                                    .to_string(),
                                message: message.to_string(),
                                event_matcher: fetched_event_sub_state.event_matcher,
                                event_publication_specs: fetched_event_sub_state
                                    .event_publication_specs,
                            };
                        }
                ...
                match message.find(driver_sub_exists_error_without_args) {
                    Some(_index) => {
                        let old_request_id = *(message
                            .split(driver_sub_exists_error_without_args)
                            .collect::<Vec<&str>>()
                            .last()
                            .unwrap());
                        ...
                        let old_event_sub_key =
                            get_event_subscription_key(old_request_id.to_string());
                        let mut existing_event_sub_state = db
                            .get::<EventSubscriptionState>(old_event_sub_key.to_string())
                            .expect(&format!(
                                "No EventSubscriptionState found in DB for request_id provided {}",
                                old_request_id.to_string()
                            ));

                        let new_event_pub_spec = fetched_event_sub_state
                            .event_publication_specs
                            .first()
                            .unwrap();
                        let mut unique_pub_spec_flag = true;
                        for event_pub_spec in
                            existing_event_sub_state.event_publication_specs.iter()
                        {
                        ...
                        if unique_pub_spec_flag {
                            existing_event_sub_state
                                .event_publication_specs
                                .push((*new_event_pub_spec).clone());
                            let updated_event_sub_state = EventSubscriptionState {
                                ...
                                event_publication_specs: existing_event_sub_state
                                    .event_publication_specs
                                    .clone(),
                            };
            ...
            println!(
                "{:?}\n",
                db.get::<EventSubscriptionState>(event_sub_key.to_string())
                    .unwrap()
            )
        }
        Err(e) => {
...
pub async fn get_driver_client(
    ...
    if use_tls {
        ...
        let channel = Channel::from_shared(driver_address)
            .unwrap()
            .tls_config(tls)?
            .connect()
            .await
...
pub async fn driver_sign_subscription_helper(
    ...
    match result {
        Ok(driver_info) => {
            ...
            println!(
                "Sending Sign EventSubscription Request to driver: {:?}",
                event_subscription.clone()
            );
            ...
            Err(Error::Simple(format!(
                "Error while requesting signature from driver: {:?}",
                signed_query
            )))
        }
        Err(e) => Err(e),
...
pub fn mark_event_states_deleted(
    fetched_event_states: EventStates,
    request_id: String,
...
pub fn delete_event_pub_spec(
    ...
    let mut event_sub_state = db
        .get::<EventSubscriptionState>(event_sub_key.to_string())
        .expect(&format!(
            "No EventSubscriptionState found in DB for request_id provided {}",
    ...
    if event_sub_state.status == event_subscription_state::Status::DuplicateQuerySubscribed as i32 {
        ...
        del_event_pub_spec = updated_state
            .clone()
            .event_publication_specs
            .first()
            .unwrap()
            .clone();
        println!(
            "Removed EventSubscriptionState from database: {:?}",
            updated_state
        );
        ...
        event_sub_state = db
            ...
            .expect(&format!(
                "No EventSubscriptionState found in DB for request_id provided {}",
                updated_state.publishing_request_id.to_string()
            ));
    }
