    ...
    const f = (node) => {
      if (node.children) {
        node.children.forEach((child) => {
          output.push(
            [child.constructor.name, child.value ? child.value : ""].join(" "),
          );
          f(child);
        });
      }
  ...
  visitChildren(ctx) {
    ...
    if (ctx.constructor.name === "RootContext") {
      ...
      if (
        ...
        (parentNode.children[0].constructor.name === "BooleanExpressionNode" ||
          parentNode.children[0].constructor.name === "CountExpressionNode")
      ) {
        this.popNode();
      }
