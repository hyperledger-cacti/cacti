...
pub mod data_transfer_server {
    ...
    #[async_trait]
    pub trait DataTransfer: std::marker::Send + std::marker::Sync + 'static {
        /// the requesting relay sends a RequestState request to the remote relay with a
        /// query defining the data it wants to receive
    ...
    /// definitions of all messages used in the datatransfer protocol
    #[derive(Debug)]
    pub struct DataTransferServer<T> {
        inner: Arc<T>,
    ...
    impl<T> Clone for DataTransferServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "relay.datatransfer.DataTransfer";
    impl<T> tonic::server::NamedService for DataTransferServer<T> {
