...
import * as membership_pb from "@hyperledger-cacti/cacti-weaver-protos-js/common/membership_pb";
import * as iin_agent_pb from "@hyperledger-cacti/cacti-weaver-protos-js/identity/agent_pb";
...
const getUserCertBase64 = async (networkName: string, username: string) => {
  const wallet = await getWalletForNetwork(networkName);
  const userId = await wallet.get(username);
  if (!userId) {
    throw new Error(
      `User ${username} not present in wallet of ${networkName}.`,
    );
  }
  return Buffer.from((userId as X509Identity).credentials.certificate).toString(
    "base64",
  );
};
...
const walletSetup = async (
  ...
  if (!identity) {
    ...
    if (!register) {
      logger.error(
        `Identity ${userName} does not exist. Please add user in the network.\n`,
      );
      return;
    }
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          `user ${userName} registration with Fabric CA failed with error: ${error}`,
        );
      } else {
        try {
          ...
          throw new Error(
            `user ${userName} registration/enrollment with Fabric CA failed with error: ${error}`,
          );
        }
      }
    }
...
const generateAccessControl = async (
  ...
  const updatedRules = templateJSON.rules.map((rule) => {
    if (rule.principalType == "ca") {
      ...
      logger.error("Error Invalid Principal Type in template file");
    }
    return rule;
  });
...
const generateMembership = async (
  ...
  if (iinAgent) {
    ...
    if (otherNetworkNames.length > 0) {
      ...
      const membershipProto = new membership_pb.Membership();
      ...
      Object.keys(membershipJSON.members).forEach((memberName, index) => {
        ...
        const memberProto = new membership_pb.Member();
      ...
      const serializedMembershipBase64 =
        Buffer.from(serializedMembership).toString("base64");
      ...
      const localKeyCert = await getKeyAndCertForRemoteRequestbyUserName(
        localWallet,
        "iinagent",
      );
      // Sign <membership + nonce> using wallet identity
      const securityDomainMember =
      ...
      const localAttestation = new iin_agent_pb.Attestation();
      localAttestation.setUnitIdentity(securityDomainMember);
      localAttestation.setCertificate(localKeyCert.cert);
      const localSig = InteroperableHelper.signMessage(
        membershipBase64WithNonce,
      ...
      const serializedttestedMembershipSetBase64 = Buffer.from(
        serializedAttestedMembershipSet,
      ).toString("base64");
      const serializedttestedMembershipSetBase64WithNonce =
        serializedttestedMembershipSetBase64 + nonce;
      for (const otherNetworkName of otherNetworkNames) {
        ...
        const otherKeyCert = await getKeyAndCertForRemoteRequestbyUserName(
          otherWallet,
          "iinagent",
        );
        // Sign <attested membership set + nonce> using wallet identity
        const otherSecurityDomainMember =
          new iin_agent_pb.SecurityDomainMemberIdentity();
        otherSecurityDomainMember.setSecurityDomain(otherNetworkName);
        otherSecurityDomainMember.setMemberId(
          getNetworkConfig(otherNetworkName).mspId,
        );
        const otherAttestation = new iin_agent_pb.Attestation();
        otherAttestation.setUnitIdentity(otherSecurityDomainMember);
        otherAttestation.setCertificate(otherKeyCert.cert);
        const otherSig = InteroperableHelper.signMessage(
          serializedttestedMembershipSetBase64WithNonce,
        ...
        const counterAttestedMembership =
          new iin_agent_pb.CounterAttestedMembership();
        counterAttestedMembership.setAttestedMembershipSet(
        ...
        fs.writeFileSync(
          path.join(
            credentialsPath,
            `attested-membership-${otherNetworkName}.proto.serialized`,
          ),
          Buffer.from(counterAttestedMembership.serializeBinary()).toString(
            "base64",
          ),
        );
      }
...
const getWalletForNetwork = async (networkName: string) => {
  const walletPath = process.env.WALLET_PATH
    ? process.env.WALLET_PATH
    : path.join(__dirname, "../", `wallet-${networkName}`);
