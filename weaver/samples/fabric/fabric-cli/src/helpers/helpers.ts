...
const addAssets = ({
  ...
  valuesList.forEach(async (item: [string, string]) => {
    const currentQuery = invocationSpec
      ...
      : {
          channel: channelName,
          contractName: contractName ? contractName : "simpleasset",
          ccFunc: "",
          args: [],
        };
    ...
    const userCert = Buffer.from(userId.credentials.certificate).toString(
      "base64",
    );
...
const pledgeAsset = async ({
  ...
  if (assetRef) {
    ...
    if (!item) {
      throw new Error(
        `Cannot find asset owner ${assetOwner} in file ${filepath}`,
      );
    }
  } else {
...
const getUserCertFromFile = (remoteUser: string, remoteNetworkId: string) => {
  ...
  try {
    ...
    logger.debug(
      `credentialsPath: ${credentialsPath} and usersAndCertsFile: ${usersAndCertsFile}`,
    );
    ...
    logger.debug(
      `remoteUser: ${remoteUser} and certificate: ${usersAndCertsJSON[remoteUser]}`,
    );
...
const saveUserCertToFile = (remoteUser: string, remoteNetworkId: string) => {
  const usersAndCertsFile = remoteNetworkId + "_UsersAndCerts.json";
  ...
  try {
    ...
    const remoteUserCertBase64 = Buffer.from(
      remoteUserId.credentials.certificate,
    ).toString("base64");

    usersAndCertsJSON[remoteUser] = remoteUserCertBase64;
    fs.writeFileSync(path.resolve(filepath), JSON.stringify(usersAndCertsJSON));
  } catch (err) {
    logger.error(
      `User: ${remoteUser} certificate cannot be saved to the file ${usersAndCertsFile}.`,
    );
  }
};
...
const getAssetPledgeDetails = async ({
  ...
  const currentQuery = invocationSpec
    ...
    : {
        ...
        contractName: netConfig.chaincode ? netConfig.chaincode : "simpleasset",
        ccFunc: "",
        args: [],
      };
  ...
  if (!pledgerCert) {
    const pledgerId = JSON.parse(
      fs
        .readFileSync(
          __dirname + "/../wallet-" + sourceNetworkName + "/" + pledger + ".id",
        )
        .toString(),
    );
    pledgerCert = Buffer.from(pledgerId.credentials.certificate).toString(
      "base64",
    );
  }
  if (!recipientCert) {
    const recipientId = JSON.parse(
      fs
        .readFileSync(
          __dirname + "/../wallet-" + destNetworkName + "/" + recipient + ".id",
        )
        .toString(),
    );
    recipientCert = Buffer.from(recipientId.credentials.certificate).toString(
      "base64",
    );
  }
...
const getLocalAssetPledgeDetails = async ({
  ...
  console.log(currentQuery);
  try {
    const pledgeDetails = await query(
      currentQuery,
...
const commandHelp = (
  ...
  if (optionsList.length > 0) {
    ...
    optionsList.forEach((command) => {
      ...
      toolbox.print.info(
        `${command.name} ${Array(numberOfSpaces).fill("\xa0").join("")} ${command.description}`,
      );
    });
    print.info("");
  }
...
const customHelp = (toolbox: Toolbox): void => {
  ...
  commandList.forEach((command) => {
    ...
    const numberOfSpaces = val > 0 ? val : 0;
    toolbox.print.info(
      `${command.commandTitle} ${Array(numberOfSpaces).fill("\xa0").join("")} ${command.description}`,
    );
  });
  toolbox.print.info("");
};
...
const generateViewAddress = async (
  ...
  if (
    viewAddress.indexOf("GetAssetClaimStatus") >= 0 ||
    viewAddress.indexOf("GetTokenAssetClaimStatus") >= 0
  ) {
    ...
    const addressParts = viewAddress
      .substring(viewAddress.indexOf(ccFunc) + ccFunc.length + 1)
      .split(":");
    if (addressParts.length != 6) {
      throw new Error(
        `Expected 6 arguments for ${ccFunc}; found ${addressParts.length}`,
      );
    }
    if (addressParts[5] != sourceNetwork) {
      throw new Error(
        `Passed source network ID ${sourceNetwork} does not match last chaincode argument in view address ${addressParts[5]}`,
      );
    }
    ...
    return (
      viewAddress +
      ":" +
      deserializeAssetPledge(pledgeDetails).getExpirytimesecs()
    );
  } else {
    return viewAddress;
  }
  ...
  try {
    ...
    if (remoteNetConfig.type == "fabric") {
      address =
        address +
        "/" +
        remoteNetConfig.channelName +
        ":" +
        remoteNetConfig.chaincode +
        ":" +
        funcName +
        ":" +
        funcArgs.join(":");
      ...
      address =
        address +
        "/" +
        remoteNetConfig.partyEndPoint +
        "#" +
        remoteNetConfig.flowPackage +
        "." +
        funcName +
        ":" +
        funcArgs.join(":");
      ...
      logger.error(
        `Error: remote network ${remoteNetConfig.type} not supported.`,
      );
      throw new Error(
        `Error: remote network ${remoteNetConfig.type} not supported.`,
      );
    ...
    console.log(
      `Interop query, funcName: ${funcName} \n funcArgs: ${funcArgs} \n and address: ${address}`,
    );
