...
const command: GluegunCommand = {
  ...
  run: async (toolbox) => {
    ...
    if (options.help || options.h) {
      commandHelp(
        ...
        [
          ...
          {
            name: "--id",
            description: "username to be added to the network",
          },
          {
            name: "--secret",
            description:
    ...
    if (!options["target-network"]) {
      print.error(
        "--target-network is required arguement, please specify the network name here",
      );
    }
    if (!options["id"]) {
      print.error(
        "--id is required arguement, please specify the username here",
      );
    }
