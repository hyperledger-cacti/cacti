...
const command: GluegunCommand = {
  ...
  run: async (toolbox) => {
    ...
    for (const network of array) {
      ...
      print.info(
        `Creating network admin wallet identity for network: ${network}`,
      );
      ...
      if (options["iin-agent"] === true) {
        // Create an IIN Agent
        print.info(
          `Creating IIN Agent wallet identity for network: ${network}`,
        );
        await enrollAndRecordWalletIdentity(
          iinAgentUser,
          null,
    ...
    for (const network of array) {
      ...
      // LOAD-CHAINCODE
      print.info(
        `Loading chaincode for network: ${network} ${connProfilePath}`,
      );
