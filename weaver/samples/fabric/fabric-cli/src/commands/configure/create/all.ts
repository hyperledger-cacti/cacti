...
const command: GluegunCommand = {
  name: "all",
  description:
    "Generates the access-control, membership, and verification-policy for the local network",
  run: async (toolbox) => {
    const {
      print,
    ...
    for (const networkName of networkNames) {
      print.info(
        `Creating network admin wallet identity for network: ${networkName}`,
      );
      await enrollAndRecordWalletIdentity(
        "networkadmin",
        null,
        networkName,
        true,
    ...
    logger.info(
      `Generated ${options["local-network"]} secuirty group at ${getCurrentNetworkCredentialPath(
        options["local-network"],
      )} `,
    );
    ...
    const appccid = process.env.DEFAULT_APPLICATION_CHAINCODE
    ...
    let templatePath = path.resolve(
      ...
      "../../../data/interop/" +
        appccid +
        "/accessControlTemplate_" +
        networkEnv.aclPolicyPrincipalType +
        ".json",
    );
    ...
    logger.info(
      `Generated ${options["local-network"]} access control at  ${getCurrentNetworkCredentialPath(
        options["local-network"],
      )} `,
    );
    ...
    logger.info(
      `Generated ${options["local-network"]} verification policy at ${getCurrentNetworkCredentialPath(
        options["local-network"],
      )} `,
    );
    process.exit();
