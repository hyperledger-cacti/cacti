...
const command: GluegunCommand = {
  ...
  run: async (toolbox) => {
    ...
    if (options.help || options.h) {
      commandHelp(
        ...
        [
          ...
          {
            name: "--user",
            description: "User for subscription. Default: user1",
          },
          {
            name: "--request-ids",
            description:
    ...
    if (requestIdList.length !== data.length) {
      throw new Error(
        `Mismatching request ID list and subscription JSON counts. #Request IDs = ${requestIdList.length}, #JSON items = ${data.length}.`,
      );
    }
    for (let i = 0; i < data.length; i++) {
      ...
      // Unsubscribe
      const eventMatcher = EventsManager.createEventMatcher(
        data[i].event_matcher,
      );
      const eventPublicationSpec = EventsManager.createEventPublicationSpec(
        data[i].event_publication_spec,
      );
      ...
      try {
        ...
        if (
          response.getStatus() == EventSubscriptionState.STATUS.UNSUBSCRIBED
        ) {
          console.log(
            "Event Unsubscription Success for requestId:",
