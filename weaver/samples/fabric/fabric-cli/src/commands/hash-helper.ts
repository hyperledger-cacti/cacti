...
const command: GluegunCommand = {
  ...
  run: async (toolbox) => {
    ...
    if (!options["random"] && array.length == 0) {
      print.error(
        `Either set random flag, or provide preimage as array of strings, at end, separated by space`,
      );
      return;
    }
    // Hash
    ...
    if (options["random"]) hash.generateRandomPreimage(22);

    const hashValue = hash.getSerializedHashBase64();
    console.log(
      `HashValue: ${hash.getSerializedHashBase64()} \nPreimage: ${hash.getPreimage()}`,
    );
