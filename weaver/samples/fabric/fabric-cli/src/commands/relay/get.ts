...
const command: GluegunCommand = {
  ...
  run: async (toolbox) => {
    ...
    const relayEndpoint = getNetworkConfig(
      options["local-network"],
    ).relayEndpoint;
    logger.debug(
      `Relay Endpoint: ${relayEndpoint} || ${options.RelayEndpoint}`,
    );
    ...
    const relay = new RelayHelper.Relay(relayEndpoint || options.relayEndpoint);
    relay
      .GetRequest(array[0])
