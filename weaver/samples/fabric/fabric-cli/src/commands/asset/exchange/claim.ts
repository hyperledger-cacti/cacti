...
const command: GluegunCommand = {
  ...
  run: async (toolbox) => {
    ...
    if (
      ...
      print.error(
        `Required either --contract-id or both locker userid in --locker, and asset type:id in --param for non-fungible assets`,
      );
      return;
    }
    if (!options["secret"]) {
    ...
    let funcToCall = AssetManager.claimAssetInHTLCusingContractId;
    let asset = "Asset";
    ...
    if (options["contract-id"]) {
      try {
        ...
        const res = await funcToCall(network.contract, contractId, hash);
        if (!res) {
          throw new Error();
        }
      ...
      try {
        ...
        spinner.info(
          `Trying ${asset} Claim with params: ${params[0]}, ${params[1]} locked by ${options.locker} for ${options.recipient}`,
        );
        const res = await AssetManager.claimAssetInHTLC(
          network.contract,
          params[0],
          params[1],
          lockerCert,
