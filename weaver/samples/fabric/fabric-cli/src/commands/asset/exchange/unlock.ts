...
const command: GluegunCommand = {
  name: "unlock",
  alias: ["-u"],
  description: "Asset Unlock for HTLC",
  run: async (toolbox) => {
    const {
      print,
    ...
    if (options.help || options.h) {
      commandHelp(
        print,
        toolbox,
        `fabric-cli asset exchange unlock --target-network=network1 --locker=alice --contract-id=abc01`,
        "fabric-cli asset exchange unlock --fungible --target-network=<network1|network2> --locker=<locker-userid> --contract-id=<contract-id> --recipient=<recipient-userid> --param=<param>",
        [
          {
            name: "--fungible",
            description: "Flag to indicate fungible asset or non-fungible.",
          },
          {
            name: "--target-network",
          ...
          {
            name: "--contract-id",
            description: "Contract Id generated in Lock.",
          },
          {
            name: "--recipient",
            description:
    ...
    if (
      !options["contract-id"] &&
      (!options["param"] ||
        options["param"].split(":").length != 2 ||
        !options["recipient"])
    ) {
      print.error(
        `Required either --contract-id or both recipient userid in --recipient, and asset type:id in --param for non-fungible assets`,
      );
      return;
    }
    ...
    let funcToCall = AssetManager.reclaimAssetInHTLCusingContractId;
    let asset = "Asset";

    if (options["fungible"]) {
      funcToCall = AssetManager.reclaimFungibleAssetInHTLC;
    ...
    if (options["contract-id"]) {
      try {
        spinner.info(`Trying ${asset} Unlock with Contract ID: ${contractId}`);
        const res = await funcToCall(network.contract, contractId);
        if (!res) {
          throw new Error();
        }
        spinner.info(`${asset} Unlocked: ${res}`);
        spinner.succeed("Asset Exchange: Unlock Complete.");
      } catch (error) {
        print.error(
          `Could not unlock ${asset} in ${options["target-network"]}`,
        );
        spinner.fail(`Error`);
      }
    } else {
      try {
        ...
        const recipientCert = Buffer.from(
          recipientId.credentials.certificate,
        ).toString("base64");
        spinner.info(
          `Trying ${asset} Unlock with params: ${params[0]}, ${params[1]} locked by ${options.locker} for ${options.recipient}`,
        );
        const res = await AssetManager.reclaimAssetInHTLC(
          network.contract,
          params[0],
          params[1],
          recipientCert,
