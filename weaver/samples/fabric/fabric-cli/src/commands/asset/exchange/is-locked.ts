...
const command: GluegunCommand = {
  ...
  run: async (toolbox) => {
    ...
    if (options.help || options.h) {
      commandHelp(
        ...
        [
          ...
          {
            name: "--secret",
            description: "secret text to be used by Asset owner to hash lock.",
          },
          {
            name: "--locker",
            description:
    ...
    let funcToCall = AssetManager.isAssetLockedInHTLCqueryUsingContractId;
    let asset = "Asset";
    ...
    if (options["contract-id"]) {
      try {
        spinner.info(
          `Querying ${asset} Is Locked with ContractId: ${contractId}`,
        );
        ...
        print.error(
          `Could not query ${asset} is-locked in ${options["target-network"]}`,
        );
        spinner.fail(`Error`);
      }
    } else {
      try {
        ...
        const lockerCert = Buffer.from(
          lockerId.credentials.certificate,
        ).toString("base64");
        ...
        spinner.info(
          `Querying ${asset} Is Locked with params: ${params[0]}, ${params[1]} locked by ${options.locker} for ${options.recipient}`,
        );
        const res = await AssetManager.isAssetLockedInHTLC(
          network.contract,
          params[0],
          params[1],
          recipientCert,
        ...
        print.error(
          `Could not query ${asset} is-locked in ${options["target-network"]}, error: ${error}`,
        );
        spinner.fail(`Error`);
      }
    }
