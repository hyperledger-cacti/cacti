...
const command: GluegunCommand = {
  name: "claim",
  description: "Claims pledged asset in destination network",
  run: async (toolbox) => {
    const {
      print,
    ...
    if (options.help || options.h) {
      commandHelp(
        ...
        [
          ...
          {
            name: "--user",
            description: "User (wallet) ID of the claimer",
          },
          {
            name: "--owner",
            description:
          ...
          {
            name: "--type",
            description: "Type of network <bond|token>",
          },
          {
            name: "--pledge-id",
            description: "Pledge Id associated with asset transfer.",
          },
          {
            name: "--param",
            description:
          ...
          },
          {
            name: "--relay-tls",
            description:
              "Flag indicating whether or not the relay is TLS-enabled.",
          },
          {
            name: "--relay-tls-ca-files",
            description:
              "Colon-separated list of root CA certificate paths used to connect to the relay over TLS.",
          },
          {
    ...
    if (!options["type"]) {
      print.error(
        "--type of asset transfer needs to be specified in the format: 'asset_type.remote_network_type'." +
    ...
    const assetIdOrQuantity = transferCategory.includes("token")
      ? parseInt(params[1])
      : params[1];
    ...
    try {
      const userCert = await getUserCertBase64(
        options["dest-network"],
        options["user"],
      );
      const { viewAddress, ownerCert } = await getClaimViewAddress(
        transferCategory,
        options["pledge-id"],
        options["owner"],
      ...
      const applicationFunction = transferCategory.includes("token")
        ? "ClaimRemoteTokenAsset"
        : "ClaimRemoteAsset";
      const { args, replaceIndices } = getChaincodeConfig(
        netConfig.chaincode,
        applicationFunction,
      );
      ...
      const assetIdOrQuantityIndex =
        applicationFunction == "ClaimRemoteTokenAsset"
          ? args.indexOf("<numunits>")
          : args.indexOf("<assetid>");
...
async function getClaimViewAddress(
  transferCategory,
  pledgeId,
  owner,
  sourceNetwork,
  ...
  let funcName = "",
    funcArgs = [];
  const ownerCert = await getUserCertFromFile(owner, sourceNetwork);
  ...
  const viewAddress = generateViewAddressFromRemoteConfig(
    sourceNetwork,
    funcName,
    funcArgs,
  );
