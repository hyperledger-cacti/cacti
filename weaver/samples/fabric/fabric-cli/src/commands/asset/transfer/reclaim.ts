...
const command: GluegunCommand = {
  name: "reclaim",
  description: "Reclaims pledged asset in source network",
  run: async (toolbox) => {
    const {
      print,
    ...
    if (options.help || options.h) {
      commandHelp(
        ...
        [
          ...
          {
            name: "--user",
            description: "User (wallet) ID of the reclaimer",
          },
          {
            name: "--type",
    ...
    try {
      ...
      const viewAddress = getReclaimViewAddress(
        transferCategory,
        assetType,
        assetIdOrQuantity,
        options["pledge-id"],
        userCert,
        options["source-network"],
        pledgeAssetDetails.getRecipient(),
        pledgeAssetDetails.getRemotenetworkid(),
        pledgeAssetDetails.getExpirytimesecs(),
      );
...
function getReclaimViewAddress(
  transferCategory,
  assetType,
  assetIdOrQuantity,
  pledgeId,
