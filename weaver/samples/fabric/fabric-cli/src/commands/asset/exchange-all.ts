...
const command: GluegunCommand = {
  ...
  run: async (toolbox) => {
    ...
    if (options.help || options.h) {
      commandHelp(
        ...
        [
          {
            name: "--network1",
            description: "Asset network for command. <network1|network2>",
          },
          {
            name: "--network2",
            description:
          ...
          {
            name: "--secret",
            description: "secret text to be used Asset owner to hash lock",
          },
          {
            name: "--timeout-epoch",
            description:
    ...
    if (options["timeout-epoch"]) {
      const duration = options["timeout-epoch"] - currTime;
    ...
    if (
      ...
      !net1Config.channelName ||
      !net1Config.chaincode
    ) {
      print.error(
    ...
    if (
      ...
      !net2Config.channelName ||
      !net2Config.chaincode
    ) {
      print.error(
    ...
    const user1CertN1 = Buffer.from(user1IdN1.credentials.certificate).toString(
      "base64",
    );
    const user1IdN2 = await network2U1.wallet.get(user1);
    const user1CertN2 = Buffer.from(user1IdN2.credentials.certificate).toString(
      "base64",
    );

    const user2IdN1 = await network1U2.wallet.get(user2);
    const user2CertN1 = Buffer.from(user2IdN1.credentials.certificate).toString(
      "base64",
    );
    const user2IdN2 = await network2U2.wallet.get(user2);
    const user2CertN2 = Buffer.from(user2IdN2.credentials.certificate).toString(
      "base64",
    );
    ...
    try {
      ...
      res = await AssetManager.createHTLC(
        network1U1.contract,
        assetType,
        assetId,
        user2CertN1,
    ...
    try {
      spinner.info(
        `Trying Fungible Asset Lock: ${fungibleAssetType}, ${fungibleAssetAmt}`,
      );
      res = await AssetManager.createFungibleHTLC(
        network2U2.contract,
        fungibleAssetType,
        fungibleAssetAmt,
        user1CertN2,
      ...
      res = await AssetManager.reclaimAssetInHTLC(
        network1U1.contract,
        assetType,
        assetId,
        user2CertN1,
    ...
    try {
      ...
      res = await AssetManager.claimFungibleAssetInHTLC(
        network2U1.contract,
        contractId,
        hash,
      );
      ...
      res = await AssetManager.reclaimFungibleAssetInHTLC(
        network2U2.contract,
        contractId,
      );
      res = await AssetManager.reclaimAssetInHTLC(
    ...
    try {
      ...
      res = await AssetManager.claimAssetInHTLC(
        network1U2.contract,
        assetType,
        assetId,
        user1CertN1,
