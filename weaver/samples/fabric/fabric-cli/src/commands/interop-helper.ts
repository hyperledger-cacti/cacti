...
const command: GluegunCommand = {
  ...
  run: async (toolbox) => {
    ...
    try {
      const appChaincodeId = process.env.DEFAULT_APPLICATION_CHAINCODE
        ? process.env.DEFAULT_APPLICATION_CHAINCODE
        : "simplestate";
      const applicationFunction = process.env.DEFAULT_APPLICATION_FUNC
      ...
      await interopHelper(
        ...
        await generateViewAddress(
          // Typically a noop, but for some functions, we may want to do some extra processing
          array[0],
          options["local-network"],
