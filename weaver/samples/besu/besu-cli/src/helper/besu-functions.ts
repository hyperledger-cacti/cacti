...
async function getContractInstance(provider, pathToJson) {
  ...
  const jsonFileContents = fs.readFileSync(jsonFile);
  const contractName = contract(JSON.parse(jsonFileContents));
  contractName.setProvider(provider);
  const instance = await contractName.deployed().catch(function () {
...
async function getBalances(tokenContract, sender, recipient) {
  ...
  console.log(
    `Account balance of the recipient: ${recipientBalance.toString()}`,
  );
}

export { getContractInstance, getBalances };
