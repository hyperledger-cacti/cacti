...
import { getContractInstance, getBalances } from "../../helper/besu-functions.js"
...
const command: GluegunCommand = {
  ...
  run: async toolbox => {
    ...
    const provider1 = new Web3.providers.HttpProvider(
      'http://' + network1Config.networkHost + ':' + network1Config.networkPort
    )
    var web3N = new Web3(provider1)
    const interopContract1 = await getContractInstance(
      provider1,
      network1Config.interopContract
    ).catch(function() {
      console.log('Failed getting interopContract1!')
      return
    })
    const tokenContract1 = await getContractInstance(
      provider1,
      network1Config.tokenContract
    ).catch(function() {
    ...
    const provider2 = new Web3.providers.HttpProvider(
      'http://' + network2Config.networkHost + ':' + network2Config.networkPort
    )
    var web3N = new Web3(provider2)
    const interopContract2 = await getContractInstance(
      provider2,
      network2Config.interopContract
    ).catch(function() {
      console.log('Failed getting interopContract2!')
      return
    })
    const tokenContract2 = await getContractInstance(
      provider2,
      network2Config.tokenContract
    ).catch(function() {
    ...
    await tokenContract1
      .approve(interopContract1.address, amount, { from: sender1 })
      .catch(function() {
        console.log('Token1 approval failed!!!')
        return
      })

    const lockTx1 = await interopContract1
      .lockAsset(recipient1, tokenContract1.address, amount, hash, timeLock, {
        from: sender1
      })
      .catch(function() {
    ...
    await tokenContract2
      .approve(interopContract2.address, amount, { from: sender2 })
      .catch(async function() {
        console.log('Token2 approval failed!!!')
        await interopContract1
          .unlockAsset(lockContractId1, {
            from: sender1
          })
          .catch(function() {
    ...
    const lockTx2 = await interopContract2
      .lockAsset(recipient2, tokenContract2.address, amount, hash, timeLock, {
        from: sender2
      })
      .catch(async function() {
    ...
    await interopContract1
      .claimAsset(lockContractId1, preimage, { from: recipient1 })
      .catch(async function() {
    ...
    await interopContract2
      .claimAsset(lockContractId2, preimage, { from: recipient2 })
      .catch(function() {
