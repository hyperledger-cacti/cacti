...
const command: GluegunCommand = {
  ...
  run: async (toolbox) => {
    ...
    if (options.asset_type == "ERC20") {
      await tokenContract
        .transfer(account, options.amount, { from: contractOwner })
        .catch(function () {
          console.log(
      ...
      console.log(
        `New ERC721 token minted with id ${response.logs[0].args.tokenId}`,
      );
      ...
      await tokenContract
        .mint(
          ...
          options.token_id,
          options.amount,
          Web3.utils.utf8ToHex(options.token_data),
          { from: contractOwner },
        )
        .catch(function () {
          console.log(
    ...
    } else var balance = await tokenContract.balanceOf(account);
    console.log(
      `Account balance of ${account} in Network ${
        options.network
      }: ${balance.toString()}`,
    );
    process.exit();
