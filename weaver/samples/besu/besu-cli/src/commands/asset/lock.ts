...
const command: GluegunCommand = {
  ...
  run: async (toolbox) => {
    ...
    console.log(
      `Account balance of the recipient in Network ${
        options.network
    ...
    if (options.asset_type == "ERC721") {
      await tokenContract.approve(tokenContract.address, options.token_id, {
        from: sender,
      });
      try {
      ...
      await tokenContract.setApprovalForAll(tokenContract.address, true, {
        from: sender,
      });
      try {
    ...
    if (lockTx && lockTx.result) {
      ...
      console.log(
        `Lock contract ID: ${lockContractId.toString().substring(2)}`,
      );
    }
