...
const networkHost = process.env.BESU_NETWORK_HOST
  ? process.env.BESU_NETWORK_HOST
  : "localhost";
const networkPort = process.env.BESU_NETWORK_PORT
  ? process.env.BESU_NETWORK_PORT
  : "8545";

const web3 = new Web3(
  new Web3.providers.HttpProvider("http://" + networkHost + ":" + networkPort),
);
...
async function main() {
  truffleConfigTemplateString = fs
    .readFileSync("truffle-config.js")
    .toString()
    .split("\n");
  ...
  for (const line of truffleConfigTemplateString) {
    ...
    if (str.trim().startsWith("network_id:")) {
      str =
        '\t\t\tnetwork_id: "' + networkId + '", // 4447 - default for Ganache';
    }
  ...
  fs.writeFileSync("truffle-config.js", truffleConfigFileString);
}
