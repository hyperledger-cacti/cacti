...
async function init(
  ...
  AliceERC20 = await getContractInstance(
    provider1,
    "../build/contracts/AliceERC20.json",
  ).catch(function () {
  ...
  await AliceERC20.transferFrom(contractOwner1, Alice1, 1, {
    from: contractOwner1,
  }).catch(function (e) {
    console.log(
      "AliceERC20 transfer threw an error; Probably the token supply is used up!",
    );
  });
...
async function getBalances(Alice1, Bob1, Alice2, Bob2) {
  ...
  console.log(
    "Alice balance of AliceERC20 in Network 1",
    AliceAliceERC20Balance.toString(),
  );

  var BobAliceERC20Balance = await AliceERC20.balanceOf(Bob1);
  console.log(
    "Bob balance of AliceERC20 in Network 1",
    await BobAliceERC20Balance.toString(),
  );
...
async function main() {
  ...
  if (!claimSuccess2) {
    ...
    // AliceERC20 tokens locked in the lockContractId1
