...
const AliceERC1155TokenContract = require("../build/contracts/AliceERC1155.json");
...
async function init(
  ...
  interopContract1 = await getContractInstance(
    provider1,
    "../build/contracts/AssetExchangeContract.json",
  ).catch(function () {
    console.log("Failed getting interopContract1!");
  });
  interopContract2 = await getContractInstance(
    provider2,
    "../build/contracts/AssetExchangeContract.json",
  ).catch(function () {
  ...
  AliceERC1155 = await getContractInstance(
    provider1,
    "../build/contracts/AliceERC1155.json",
  ).catch(function () {
    console.log("Failed getting AliceERC1155 token contract!");
  });
  BobERC20 = await getContractInstance(
    provider2,
    "../build/contracts/BobERC20.json",
  ).catch(function () {
    console.log("Failed getting BobERC20 token contract!");
  });

  // Issue AliceERC1155 tokens to Alice in Network 1 and BobERC20 tokens
  ...
  await AliceERC1155.safeTransferFrom(
    contractOwner1,
    Alice1,
    1,
    1,
    ...
    console.log(e);
    console.log(
      "AliceERC1155 transfer threw an error; Probably the token supply is used up!",
    );
  });
  await BobERC20.transfer(Bob2, senderInitialBalance, {
    from: contractOwner2,
  }).catch(function () {
    console.log(
      "BobERC20 transfer threw an error; Probably the token supply is used up!",
    );
  });
}
...
async function claimToken(
  ...
  var claimStatus = await interopContract
    .claimAsset(lockContractId, preimage, {
      from: recipient,
    })
    .catch(function (err) {
      console.log(err);
...
async function lockToken(
  ...
  if (type == "ERC1155") {
    ...
    await tokenContract
      .approve(tokenContract.address, tokenAmount, { from: sender })
      .catch(function () {
        console.log("Token approval failed!!!");
        return false;
      });
  }
  var lockStatus = interopContract
    .lockAsset(
      recipient,
...
async function getBalances(Alice1, Bob1, Alice2, Bob2) {
  var AliceAliceERC1155Balance = await AliceERC1155.balanceOf(Alice1, 1);
  console.log(
    "Alice balance of AliceERC1155 in Network 1",
    AliceAliceERC1155Balance.toString(),
  );

  var BobAliceERC1155Balance = await AliceERC1155.balanceOf(Bob1, 1);
  console.log(
    "Bob balance of AliceERC1155 in Network 1",
    await BobAliceERC1155Balance.toString(),
  );

  var AliceBobERC20Balance = await BobERC20.balanceOf(Alice2);
  console.log(
    "Alice balance of BobERC20 in Network 2",
    AliceBobERC20Balance.toString(),
  );

  var BobBobERC20Balance = await BobERC20.balanceOf(Bob2);
  console.log(
    "Bob balance of BobERC20 in Network 2",
    await BobBobERC20Balance.toString(),
  );
}
...
async function main() {
  ...
  const provider1 = new Web3.providers.HttpProvider(
    "http://" + networkHost1 + ":" + networkPort1,
  );
  const web3N1 = new Web3(provider1);
  const accounts1 = await web3N1.eth.getAccounts();

  const Alice1 = accounts1[1]; // owner of AliceERC1155 and wants swap for BobERC20
  const Bob1 = accounts1[2]; // owner of BobERC20 and wants to swap for AliceERC1155
  ...
  const provider2 = new Web3.providers.HttpProvider(
    "http://" + networkHost2 + ":" + networkPort2,
  );
  const web3N2 = new Web3(provider2);
  const accounts2 = await web3N2.eth.getAccounts();

  const Alice2 = accounts2[1]; // owner of AliceERC1155 and wants swap for BobERC20
  const Bob2 = accounts2[2]; // owner of BobERC20 and wants to swap for AliceERC1155
  ...
  // Creating a HTLC contract for Alice locking her AliceERC1155 tokens for Bob
  let lockTx1 = await lockToken(
    Alice1,
    Bob1,
    AliceERC1155,
    1,
  ...
  let claimSuccess2 = await claimToken(
    interopContract2,
    lockContractId2,
    Alice2,
    preimage,
  ...
  if (!claimSuccess2) {
    ...
    let unlockSuccess2 = await unlockToken(
      interopContract2,
      lockContractId2,
      Bob2,
    );
    let unlockSuccess1 = await unlockToken(
      interopContract1,
      lockContractId1,
      Alice1,
    );
    ...
    // AliceERC1155 tokens locked in the lockContractId1
    let claimSuccess1 = await claimToken(
      interopContract1,
      lockContractId1,
      Bob1,
      preimage,
    ); // we assume Bob's claim to be successful if Alice's claim is successful
