...
async function init(
  ...
  AliceERC721 = await getContractInstance(
    provider1,
    "../build/contracts/AliceERC721.json",
  ).catch(function () {
  ...
  await AliceERC721.transferFrom(contractOwner1, Alice1, 1, {
    from: contractOwner1,
  }).catch(function (e) {
    console.log(
      "AliceERC721 transfer threw an error; Probably the token supply is used up!",
    );
  });
...
async function getBalances(Alice1, Bob1, Alice2, Bob2) {
  ...
  console.log(
    "Alice balance of AliceERC721 in Network 1",
    AliceAliceERC721Balance.toString(),
  );

  var BobAliceERC721Balance = await AliceERC721.balanceOf(Bob1);
  console.log(
    "Bob balance of AliceERC721 in Network 1",
    await BobAliceERC721Balance.toString(),
  );
