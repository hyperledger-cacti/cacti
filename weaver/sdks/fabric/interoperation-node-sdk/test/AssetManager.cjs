...
import assetLocksPb from "@hyperledger-cacti/cacti-weaver-protos-js/common/asset_locks_pb";
...
describe("AssetManager", () => {
  ...
  const numUnits = 1000;
  const recipientECert = fs
    .readFileSync(`${__dirname}/data/anotherSignCert.pem`)
    .toString();
  let lockerECert;
  ...
  beforeEach(async () => {
    ...
    amcTx = sinon.createStubInstance(Transaction);
    const amcStub = sinon.stub(amc, "createTransaction").returns(amcTx);
  });
  ...
  describe("create HTLC for unique asset", () => {
    ...
    it("asset lock fails with invalid parameters", async () => {
      ...
      let assetLockInvocation = await assetManager.createHTLC(
        null,
        assetType,
        assetID,
        recipientECert,
      ...
      expect(assetLockInvocation)
        .to.be.an("object")
        .that.has.all.keys("hash", "result");
      ...
      assetLockInvocation = await assetManager.createHTLC(
        amc,
        "",
        assetID,
        recipientECert,
    ...
    it("submit asset lock invocation", async () => {
      ...
      let expiryTimeSecs = Math.floor(Date.now() / 1000) + 300; // Convert epoch milliseconds to seconds and add 5 minutes
      let lockInfoStr = assetManager.createAssetLockInfoSerialized(
        hash,
        expiryTimeSecs,
      );
      amcTx.submit.withArgs(assetAgreementStr, lockInfoStr).resolves(true);
      let assetLockInvocation = await assetManager.createHTLC(
        amc,
        assetType,
        assetID,
      ...
      expect(assetLockInvocation.hash.getSerializedHashBase64()).to.be.a(
        "string",
      );
      expect(assetLockInvocation.hash.getSerializedHashBase64()).to.equal(
        hashValue,
      );
      expect(assetLockInvocation.result).to.be.a("boolean");
      expect(assetLockInvocation.result).to.equal(true);
      amcTx.submit.withArgs(assetAgreementStr, sinon.match.any).resolves(true);
      assetLockInvocation = await assetManager.createHTLC(
        amc,
        assetType,
        assetID,
      ...
      expect(
        assetLockInvocation.hash.getSerializedHashBase64().length,
      ).to.be.above(0);
      expect(assetLockInvocation.result).to.be.a("boolean");
      expect(assetLockInvocation.result).to.equal(true);
  ...
  describe("create HTLC for fungible asset", () => {
    beforeEach(() => {
      amcTx.submit.resolves("");
    });

    it("asset lock fails with invalid parameters", async () => {
      let expiryTimeSecs = Math.floor(Date.now() / 1000) + 300; // Convert epoch milliseconds to seconds and add 5 minutes
      let assetLockInvocation = await assetManager.createFungibleHTLC(
        null,
        fungibleAssetType,
        numUnits,
        recipientECert,
      ...
      assetLockInvocation = await assetManager.createFungibleHTLC(
        amc,
        "",
        numUnits,
        recipientECert,
    ...
    it("submit asset lock invocation", async () => {
      ...
      amcTx.submit
        .withArgs(assetAgreementStr, lockInfoStr)
        .resolves(contractId);
      let assetLockInvocation = await assetManager.createFungibleHTLC(
        amc,
        fungibleAssetType,
      ...
      amcTx.submit
        .withArgs(assetAgreementStr, sinon.match.any)
        .resolves(contractId);
      assetLockInvocation = await assetManager.createFungibleHTLC(
        amc,
        fungibleAssetType,
  ...
  describe("claim unique asset locked in HTLC", () => {
    ...
    it("asset claim fails with invalid parameters", async () => {
      let assetClaimInvocation = await assetManager.claimAssetInHTLC(
        null,
        assetType,
        assetID,
        lockerECert,
      ...
      assetClaimInvocation = await assetManager.claimAssetInHTLC(
        amc,
        "",
        assetID,
        lockerECert,
    ...
    it("submit asset claim invocation", async () => {
      ...
      amcTx.submit.withArgs(assetAgreementStr, claimInfoStr).resolves(true);
      let assetClaimInvocation = await assetManager.claimAssetInHTLC(
        amc,
        assetType,
        assetID,
  ...
  describe("claim unique asset locked in HTLC using contractId", () => {
    ...
    it("asset claim fails with invalid parameters", async () => {
      let assetClaimInvocation =
        await assetManager.claimAssetInHTLCusingContractId(
          null,
          contractId,
          hash,
        );
      expect(assetClaimInvocation).to.be.a("boolean");
      expect(assetClaimInvocation).to.equal(false);
      assetClaimInvocation = await assetManager.claimAssetInHTLCusingContractId(
        amc,
        "",
        hash,
      );
    ...
    it("submit asset claim invocation", async () => {
      let claimInfoStr = assetManager.createAssetClaimInfoSerialized(hash);
      amcTx.submit.withArgs(contractId, claimInfoStr).resolves(true);
      let assetClaimInvocation =
        await assetManager.claimAssetInHTLCusingContractId(
          amc,
          contractId,
  ...
  describe("claim fungible asset locked in HTLC", () => {
    ...
    it("asset claim fails with invalid parameters", async () => {
      let assetClaimInvocation = await assetManager.claimFungibleAssetInHTLC(
        null,
        contractId,
        hash,
      );
      expect(assetClaimInvocation).to.be.a("boolean");
      expect(assetClaimInvocation).to.equal(false);
      assetClaimInvocation = await assetManager.claimFungibleAssetInHTLC(
        amc,
        "",
        hash,
      );
  ...
  describe("reclaim unique asset locked in HTLC", () => {
    ...
    it("asset reclaim fails with invalid parameters", async () => {
      let assetReclaimInvocation = await assetManager.reclaimAssetInHTLC(
        null,
        assetType,
        assetID,
        recipientECert,
      ...
      assetReclaimInvocation = await assetManager.reclaimAssetInHTLC(
        amc,
        "",
        assetID,
        recipientECert,
    ...
    it("submit asset reclaim invocation", async () => {
      ...
      amcTx.submit.withArgs(assetAgreementStr).resolves(true);
      let assetReclaimInvocation = await assetManager.reclaimAssetInHTLC(
        amc,
        assetType,
        assetID,
  ...
  describe("reclaim unique asset locked in HTLC using contractId", () => {
    ...
    it("asset reclaim fails with invalid parameters", async () => {
      let assetReclaimInvocation =
        await assetManager.reclaimAssetInHTLCusingContractId(null, contractId);
      expect(assetReclaimInvocation).to.be.a("boolean");
      expect(assetReclaimInvocation).to.equal(false);
      assetReclaimInvocation =
        await assetManager.reclaimAssetInHTLCusingContractId(amc, "");
      expect(assetReclaimInvocation).to.be.a("boolean");
      expect(assetReclaimInvocation).to.equal(false);
    });

    it("submit asset reclaim invocation", async () => {
      amcTx.submit.withArgs(contractId).resolves(true);
      let assetReclaimInvocation =
        await assetManager.reclaimAssetInHTLCusingContractId(amc, contractId);
      expect(assetReclaimInvocation).to.be.a("boolean");
      expect(assetReclaimInvocation).to.equal(true);
    });
  });
  ...
  describe("reclaim fungible asset locked in HTLC", () => {
    ...
    it("asset reclaim fails with invalid parameters", async () => {
      let assetReclaimInvocation =
        await assetManager.reclaimFungibleAssetInHTLC(null, contractId);
      expect(assetReclaimInvocation).to.be.a("boolean");
      expect(assetReclaimInvocation).to.equal(false);
      assetReclaimInvocation = await assetManager.reclaimFungibleAssetInHTLC(
        amc,
        "",
      );
      expect(assetReclaimInvocation).to.be.a("boolean");
    ...
    it("submit asset reclaim invocation", async () => {
      amcTx.submit.withArgs(contractId).resolves(true);
      let assetReclaimInvocation =
        await assetManager.reclaimFungibleAssetInHTLC(amc, contractId);
      expect(assetReclaimInvocation).to.be.a("boolean");
      expect(assetReclaimInvocation).to.equal(true);
    });
  });
  ...
  describe("check unique asset lock status in HTLC", () => {
    ...
    it("asset lock status check fails with invalid parameters", async () => {
      let assetLockQuery = await assetManager.isAssetLockedInHTLC(
        null,
        assetType,
        assetID,
        recipientECert,
      ...
      assetLockQuery = await assetManager.isAssetLockedInHTLC(
        amc,
        "",
        assetID,
        recipientECert,
  ...
  describe("check unique asset lock status in HTLC using contractId", () => {
    ...
    it("asset lock status check fails with invalid parameters", async () => {
      let assetLockQuery =
        await assetManager.isAssetLockedInHTLCqueryUsingContractId(
          null,
          contractId,
        );
      expect(assetLockQuery).to.be.a("boolean");
      expect(assetLockQuery).to.equal(false);
      assetLockQuery =
        await assetManager.isAssetLockedInHTLCqueryUsingContractId(amc, null);
      expect(assetLockQuery).to.be.a("boolean");
      expect(assetLockQuery).to.equal(false);
    });
    ...
    it("submit asset lock status query", async () => {
      amcStub
        .withArgs("IsAssetLockedQueryUsingContractId", contractId)
        .resolves(true);
      let assetLockQuery =
        await assetManager.isAssetLockedInHTLCqueryUsingContractId(
          amc,
  ...
  describe("check fungible asset lock status in HTLC", () => {
    ...
    it("asset lock status check fails with invalid parameters", async () => {
      let assetLockQuery = await assetManager.isFungibleAssetLockedInHTLC(
        null,
        contractId,
      );
      expect(assetLockQuery).to.be.a("boolean");
