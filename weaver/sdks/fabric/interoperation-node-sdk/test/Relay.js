...
const ackPb = require("@hyperledger/cacti-weaver-protos-js/common/ack_pb");
...
describe("Relay", () => {
    ...
    describe("#fail relay api calls 2", () => {
        ...
        it("fail send request", async () => {
            const expectedErrMsg =
                "Error: Error with Network Client: Error: Request state received negative Ack error: " + ack.message;
            try {
                const reqId = await relay.SendRequest("", "", "", "", "", "", "", "");
                throw new Error("IllegalState");
            } catch (error) {
