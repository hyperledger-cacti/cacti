...
import ackPb from "@hyperledger-cacti/cacti-weaver-protos-js/common/ack_pb";
...
describe("Relay", () => {
  ...
  describe("#successful relay api calls", () => {
    ...
    before(() => {
      ...
      var packageDefinition = protoLoader.loadSync("networks/networks.proto", {
    ...
    it("successful get request", async () => {
      ...
      expect(JSON.stringify(res.getView().getMeta().toObject())).to.equal(
        JSON.stringify(view.getMeta().toObject()),
      );
    });
  ...
  describe("#fail relay api calls 1", () => {
    ...
    it("fail send request", async () => {
      const expectedErrMsg =
        "Error: Error with Network Client: Error: Request state error: Error: 2 UNKNOWN: mock error";
      try {
        const reqId = await relay.SendRequest("", "", "", "", "", "", "", "");
  ...
  describe("#fail relay api calls 2", () => {
    ...
    it("fail send request", async () => {
      const expectedErrMsg =
        "Error: Error with Network Client: Error: Request state received negative Ack error: " +
        ack.message;
      try {
        const reqId = await relay.SendRequest("", "", "", "", "", "", "", "");
        throw new Error("IllegalState");
    ...
    it("fail process request", async () => {
      // SendRequest error
      const expectedErrMsg =
        "Error: Error: Request state error: Error: Error with Network Client: Error: Request state received negative Ack error: " +
        ack.message;
      try {
        const res = await relay.ProcessRequest("", "", "", "", "", "", "", "");
        throw new Error("IllegalState");
      ...
      ack.message = "";
      const expectedErrMsg2 =
        "Error: Error: Error from view payload : " + relayResponse.error;
      try {
        const res = await relay.ProcessRequest("", "", "", "", "", "", "", "");
        throw new Error("IllegalState");
      } catch (error) {
    ...
    it("fail pending state in process request", async () => {
      ...
      relayResponse.error = "";
      const expectedErrMsg3 =
        "Error: Error: State error: Error: Timeout: State is still pending.";
      try {
        const res = await relay.ProcessRequest("", "", "", "", "", "", "", "");
        throw new Error("IllegalState");
      } catch (error) {
