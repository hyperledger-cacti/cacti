...
const statePb = require("@hyperledger/cacti-weaver-protos-js/common/state_pb");
...
describe("InteroperableHelper", () => {
    ...
    describe("verify get policy criteria lookup", () => {
        ...
        it("validate policy syntax and attributes", async () => {
            const policyJSON = await getPolicyCriteriaForAddress(interopcc, viewAddresses[0]);
        ...
        it("fail to match verificationPolicy", async () => {
            // no match found
            let policyJSON = await getPolicyCriteriaForAddress(interopcc, wrongViewAddress);
            expect(policyJSON).to.equal(null);

            // no policy found
            try {
                policyJSON = await getPolicyCriteriaForAddress(interopcc, wrongViewAddress);
            } catch (error) {
