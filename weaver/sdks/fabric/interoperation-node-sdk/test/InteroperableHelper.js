...
describe("InteroperableHelper", () => {
  ...
  beforeEach(async () => {
    ...
    interopccStub = sinon
      .stub(interopcc, "createTransaction")
      .withArgs("WriteExternalState")
      .returns(writeExternalStateTransaction);
  });
  ...
  describe("deserialize/serialize proposal structures", () => {
    const samplePropJSON = JSON.parse(
      fs.readFileSync(`${__dirname}/data/prop.json`).toString(),
    );
    let deserializedPR;

    it("base64-encoded byte array deserialized into proposal response", () => {
      expect(() => {
        const dpropResp = deserializeRemoteProposalResponseBase64(
          samplePropJSON.resp,
        );
        expect(dpropResp)
          .to.be.an("object")
          .to.include.all.keys("version", "response", "payload", "endorsement");
        expect(dpropResp.response)
          .to.be.an("object")
          .to.include.all.keys("status", "payload");
        expect(dpropResp.endorsement)
          .to.be.an("object")
          .that.has.all.keys("endorser", "signature");
        expect(dpropResp.response.status).to.equal(samplePropJSON.status);
        expect(dpropResp.response.payload).to.be.a("Uint8Array");
        expect(dpropResp.response.payload.toString("utf8")).to.equal(
          samplePropJSON.ccmessage,
        );
  ...
  describe("cryptographic functions", () => {
    it("encrypt and decrypt a message with default 128-bit AES key", () => {
      ...
      expect(() => {
        const encryptedData = ecies.eciesEncryptMessage(
          pubKey,
          Buffer.from(data),
          cryptoOptions,
        );
  ...
  describe("decrypt remote proposal response", () => {
    it("decrypt proposal response using private key", () => {
      ...
      const privKeyPEM = fs.readFileSync(privKeyFile).toString();
      expect(() => {
        const decResp = decryptRemoteProposalResponse(
  ...
  describe("decrypt remote chaincode output", () => {
    it("decrypt chaincode output using private key", () => {
      ...
      const privKeyPEM = fs.readFileSync(privKeyFile).toString();
      expect(() => {
        const decResp = decryptRemoteChaincodeOutput(
          samplePropJSON.eccresp,
          privKeyPEM,
        );
        expect(decResp)
          .to.be.an("object")
  ...
  describe("verify remote proposal response", () => {
    beforeEach(() => {
      const ccResult = fs.readFileSync(
        `${__dirname}/data/exporter_org_msp_config.json`,
      );
    ...
    it("validate plaintext proposal response and endorsement", async () => {
      ...
      const verificationStatus = await verifyDecryptedRemoteProposalResponse(
        dpropResp,
        foreignNetworkId,
      );
    ...
    it("validate plaintext proposal response and endorsement with wrapper", async () => {
      ...
      expect(verificationStatus)
        .to.be.an("object")
        .that.has.all.keys("proposalResponse", "valid");
  ...
  describe("get client credentials for remote transaction", () => {
    ...
    it("obtain key and certificate from wallet identity", async () => {
      ...
      const keyHex = keyutil.getKeyFromPlainPrivatePKCS8PEM(
        keyCert.key.toBytes(),
      ).prvKeyHex;
        ...
        identifiers: [
          {
            ...
            policy: { type: "Signature", criteria: ["Org1MSP"] },
          },
          {
            pattern: "notmatching",
            policy: { type: "Signature", criteria: ["NotMatching"] },
          },
        ],
        viewPatterns: [],
      };
      const vpResult = JSON.stringify(vpJSON);
      const interopccStub = sinon
        .stub(interopcc, "evaluateTransaction")
        .resolves(vpResult);
      interopccStub
        .withArgs("GetVerificationPolicyBySecurityDomain", "network1")
        .resolves(vpResult);
      interopccStub
        .withArgs("GetVerificationPolicyBySecurityDomain", "network2")
        .resolves("");
    });
    ...
    it("fail to match verificationPolicy", async () => {
      ...
      try {
        ...
        expect(error.toString()).to.equal(
          "Error: Error during getPolicyCriteriaForAddress: Error: No verification policy for address " +
            wrongViewAddress,
        );
      }
    });
  ...
  describe("test invoke handler", () => {
    beforeEach(() => {
      ...
      const interopccStub = sinon
        .stub(interopcc, "submitTransaction")
        .resolves(false);
  ...
  describe("test interopFlow", () => {
    ...
    const views64 = [
      Buffer.from(view1.serializeBinary()).toString("base64"),
    ...
    beforeEach(() => {
      ...
      const interopccStub2 = sinon
        .stub(interopcc, "evaluateTransaction")
        .rejects("interopcc error");
      interopccStub2
        .withArgs("GetVerificationPolicyBySecurityDomain", "network1")
        .resolves(vpResult);
      interopccStub2
        .withArgs("VerifyView", views64[0], viewAddresses[0])
        .resolves(true);
      interopccStub2
        .withArgs("VerifyView", views64[1], viewAddresses[1])
        .resolves(true);
    ...
    it("successful data sharing query", async () => {
      ...
      expect(interopResponse)
        .to.be.an("object")
        .that.has.all.keys("views", "result");
    ...
    it("fail data sharing query: relay error", async () => {
      ...
      try {
        ...
        const expectedErrMsg =
          "Error: InteropFlow remote view request error: Error: InteropFlow relay response error: Error: relay error";
    ...
    it("fail data sharing query: view verification error", async () => {
      ...
      try {
        ...
        const expectedErrMsg =
          "Error: InteropFlow remote view request error: Error: View verification failed Error: Unable to verify view: interopcc error";
        expect(error.toString()).to.equal(expectedErrMsg);
      }
    });
