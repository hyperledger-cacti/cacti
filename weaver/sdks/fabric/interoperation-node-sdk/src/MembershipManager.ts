...
async function createLocalMembership(
  ...
  const membership64 = Buffer.from(membership.serializeBinary()).toString(
    "base64",
  );
...
async function readMembership(
  ...
  const contract = network.getContract(weaverCCId);
  return await contract.submitTransaction(
    "GetMembershipBySecurityDomain",
    securityDomain,
  );
}

function getMembershipUnit(
  channel: Channel,
  mspId: string,
): membership_pb.Member {
  ...
  const member = new membership_pb.Member();
...
async function syncMembershipFromIINAgent(
  ...
  try {
    ...
    const iinAgentClient = getIINAgentClient(
      iinAgentEndpoint,
      useTls,
      tlsCACertPath,
    );
    const {
      ...
      syncExternalState: (
        request: agent_pb.SecurityDomainMemberIdentity,
      ) => Promise<common_ack_pb.Ack>;
    } = promisifyAll(iinAgentClient);

    if (typeof syncExternalState === "function") {
      const [resp, error] = await handlePromise(
        syncExternalState(foreignSecurityDomain),
      );
      if (error) {
      ...
      if (resp.getStatus() === common_ack_pb.Ack.STATUS.ERROR) {
        throw new Error(
          `Membership Sync request received negative Ack error: ${resp.getMessage()}`,
        );
      }
  ...
  if (tls) {
    if (!tlsCACertPath || tlsCACertPath == "") {
      ...
      if (!(tlsCACertPath && fs.existsSync(tlsCACertPath))) {
        throw new Error(
          "Missing or invalid IIN Agent's tlsCACertPaths: " + tlsCACertPath,
        );
      }
