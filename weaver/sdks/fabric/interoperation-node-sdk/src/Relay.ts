...
class Relay {
      ...
      if (tlsRootCACertPaths && tlsRootCACertPaths.length > 0) {
        for (let i = 0; i < tlsRootCACertPaths.length; i++) {
          if (!fs.existsSync(tlsRootCACertPaths[i])) {
            throw new Error(
              "Invalid TLS root CA file path: " + tlsRootCACertPaths[i],
            );
          }
          this._tlsRootCACerts =
            this._tlsRootCACerts +
            fs.readFileSync(tlsRootCACertPaths[i]).toString();
        }
      }
    }
  }
  ...
  async SendRequest(
    ...
    try {
      const networkClient = new networksGrpcPb.NetworkClient(
        this.getEndpoint(),
        this._useTls
          ? this._tlsRootCACerts.length == 0
            ? grpcJs.credentials.createSsl()
            : grpcJs.credentials.createSsl(Buffer.from(this._tlsRootCACerts))
      ...
      const {
        ...
        requestState: (
          query: networksPb.NetworkQuery,
        ) => Promise<common_ack_pb.Ack>;
      ...
      if (typeof requestState === "function") {
        ...
        if (resp.getStatus() === common_ack_pb.Ack.STATUS.ERROR) {
          throw new Error(
            `Request state received negative Ack error: ${resp.getMessage()}`,
          );
        }
  ...
  async ProcessRequest(
    ...
    try {
      ...
      //TODO: SLOW DOWN
      const [finalState, stateError] = await helpers.handlePromise(
        this.recursiveState(requestID, dateObj),
      );
      if (stateError) {
  ...
  async recursiveState(requestID: string, dateObj: Date): Promise<any> {
    const [state, error] = await helpers.handlePromise(
      this.GetRequest(requestID, false),
    );
    if (error) {
      throw new Error(`Request state error: ${error}`);
    }
  ...
  async GetRequest(requestId: string, asJson = true): Promise<any> {
    ...
    const {
      ...
      getState: (
        message: networksPb.GetStateMessage,
      ) => Promise<statePb.RequestState>;
    } = helpers.promisifyAll(networkClient);
  ...
  async SendEventSubscribeRequest(
    ...
    try {
      ...
      const {
        ...
        subscribeEvent: (
          eventSubscription: networksPb.NetworkEventSubscription,
        ) => Promise<common_ack_pb.Ack>;
      } = helpers.promisifyAll(networkClient);
      ...
      if (typeof subscribeEvent === "function") {
        const [resp, error] = await helpers.handlePromise(
        ...
        if (resp.getStatus() === common_ack_pb.Ack.STATUS.ERROR) {
          throw new Error(
            `Event Subscription request received negative Ack error: ${resp.getMessage()}`,
          );
        }
        return resp.getRequestId();
      }
   ...
   * @returns {object} The request object from the relay
   */
  async GetEventSubscriptionState(
    requestId: string,
    asJson = true,
  ): Promise<any> {
    const networkClient = new networksGrpcPb.NetworkClient(
    ...
    const {
      ...
      getEventSubscriptionState: (
        message: networksPb.GetStateMessage,
      ) => Promise<eventsPb.EventSubscriptionState>;
    } = helpers.promisifyAll(networkClient);
    const getStateMessage = new networksPb.GetStateMessage();
    getStateMessage.setRequestId(requestId);
    const [state, error] = await helpers.handlePromise(
      getEventSubscriptionState(getStateMessage),
    );
    if (error) {
      throw new Error(`Error: ${error}`);
    }
  ...
  async recursiveEventSubscriptionState(
    requestID: string,
    dateObj: Date,
  ): Promise<any> {
    const [state, error] = await helpers.handlePromise(
      this.GetEventSubscriptionState(requestID, false),
    );
    if (error) {
    ...
    if (
      state.getStatus() ===
        eventsPb.EventSubscriptionState.STATUS.SUBSCRIBE_PENDING ||
      state.getStatus() ===
        eventsPb.EventSubscriptionState.STATUS.SUBSCRIBE_PENDING_ACK ||
      state.getStatus() ===
        eventsPb.EventSubscriptionState.STATUS.UNSUBSCRIBE_PENDING ||
      state.getStatus() ===
        eventsPb.EventSubscriptionState.STATUS.UNSUBSCRIBE_PENDING_ACK
    ) {
      if (dateObj.getTime() < Date.now()) {
        throw new Error("Timeout: State is still pending.");
      } else {
  ...
  async ProcessSubscribeEventRequest(
    ...
    try {
      const [requestID, error] = await helpers.handlePromise(
        this.SendEventSubscribeRequest(
          eventMatcher,
      ...
      //TODO: SLOW DOWN
      const [finalState, stateError] = await helpers.handlePromise(
        this.recursiveEventSubscriptionState(requestID, dateObj),
      );
      if (stateError) {
        throw new Error(`Event Subscription error: ${stateError}`);
      }
      if (
        finalState.getStatus() === eventsPb.EventSubscriptionState.STATUS.ERROR
      ) {
        if (finalState.getMessage()) {
          throw new Error(
            `Error during event subscription : ${finalState.getMessage()}`,
          );
        } else {
  ...
  async SendEventUnsubscribeRequest(
    ...
    try {
      ...
      const {
        ...
        unsubscribeEvent: (
          eventUnsubscription: networksPb.NetworkEventUnsubscription,
        ) => Promise<common_ack_pb.Ack>;
      } = helpers.promisifyAll(networkClient);
      ...
      if (typeof unsubscribeEvent === "function") {
        const [resp, error] = await helpers.handlePromise(
          unsubscribeEvent(eventUnsubscription),
        );
        if (error) {
        ...
        if (resp.getStatus() === common_ack_pb.Ack.STATUS.ERROR) {
          throw new Error(
            `Event Unsubscription request received negative Ack error: ${resp.getMessage()}`,
          );
        }
        return resp.getRequestId();
      }
  ...
  async ProcessUnsubscribeEventRequest(
    ...
    try {
      const [requestID, error] = await helpers.handlePromise(
        this.SendEventUnsubscribeRequest(
          eventMatcher,
          eventPublicationSpec,
          requestId,
          address,
      ...
      if (
        ...
        if (finalState.getMessage()) {
          throw new Error(
            `Error during event unsubscription : ${finalState.getMessage()}`,
          );
        } else {
          throw new Error(
            `Error during event unsubscription : UNKNOWN REASON}`,
          );
        }
      }
      return finalState;
  ...
  async GetEventStates(requestId: string, asJson = true): Promise<any> {
    ...
    const {
      ...
      getEventStates: (
        message: networksPb.GetStateMessage,
      ) => Promise<eventsPb.EventStates>;
    } = helpers.promisifyAll(networkClient);
    const getStateMessage = new networksPb.GetStateMessage();
    getStateMessage.setRequestId(requestId);
    const [state, error] = await helpers.handlePromise(
      getEventStates(getStateMessage),
    );
    if (error) {
      throw new Error(`Error: ${error}`);
    }
