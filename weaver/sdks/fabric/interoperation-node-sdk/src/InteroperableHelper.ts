...
const decryptRemoteProposalResponse = (
  proposalResponseBytes64: string,
  eciesPrivateKeyPEM: any,
) => {
  const privKey = keyutil.getKeyFromPlainPrivatePKCS8PEM(eciesPrivateKeyPEM);
  const propResp = deserializeRemoteProposalResponseBase64(
    proposalResponseBytes64,
  );
  const decryptionOptions = { hashAlgorithm: "SHA2" };
  propResp.payload = eciesCrypto.eciesDecryptMessage(
    privKey,
...
const decryptRemoteChaincodeOutput = (
  proposalResponseBytes64,
  eciesPrivateKeyPEM,
) => {
  const privKey = keyutil.getKeyFromPlainPrivatePKCS8PEM(eciesPrivateKeyPEM);
  ...
  const decryptionOptions = { hashAlgorithm: "SHA2" };
  propResp.response.payload = eciesCrypto.eciesDecryptMessage(
    privKey,
    //@ts-ignore Issue with propResp.payload not being a string but errors when converted to a string
    propResp.response.payload,
    decryptionOptions,
  );
  return propResp;
};
...
const verifyDecryptedRemoteProposalResponse = async (proposalResponse) => {
  ...
  try {
    ...
    logger.error(
      "verifyDecryptedRemoteProposalResponse - getting remote endorser identity failed with: ",
      error,
    );
    return false;
  }
  ...
  try {
    ...
    logger.debug(
      "verifyDecryptedRemoteProposalResponse - have a valid identity",
    );
    ...
    logger.error(
      "verifyDecryptedRemoteProposalResponse - verify failed with: ",
      error,
    );
    return false;
  }
...
const verifyRemoteProposalResponse = async (
  ...
  if (isEncrypted) {
    decryptedProposalResponse = decryptRemoteProposalResponse(
      proposalResponseBase64,
      privKeyPEM,
    );
    ...
    decryptedProposalResponse = deserializeRemoteProposalResponseBase64(
      decryptedProposalResponse,
    );
  }

  const isValid = await verifyDecryptedRemoteProposalResponse(
    decryptedProposalResponse,
  );
  return {
    proposalResponse: (
      serializeRemoteProposalResponse(decryptedProposalResponse) as Buffer
    ).toString("base64"),
    valid: isValid,
  };
};
...
const getResponseDataFromView = (view, privKeyPEM = null) => {
  if (view.getMeta().getProtocol() == statePb.Meta.Protocol.FABRIC) {
    const fabricView = fabricViewPb.FabricView.deserializeBinary(
      view.getData(),
    );
    const fabricViewProposalResponses =
      fabricView.getEndorsedProposalResponsesList();
    let viewAddress = "";
    let responsePayload = "";
    const responsePayloadContents = [];
    let payloadConfidential = false;
    for (let i = 0; i < fabricViewProposalResponses.length; i++) {
      const fabricViewChaincodeAction =
        proposalPb.ChaincodeAction.deserializeBinary(
          fabricViewProposalResponses[i].getPayload().getExtension_asU8(),
        );
      const interopPayload = interopPayloadPb.InteropPayload.deserializeBinary(
    ...
    cordaNotarizedPayloads.forEach((notarizedPayload, i) => {
      const interopPayload = interopPayloadPb.InteropPayload.deserializeBinary(
        notarizedPayload.getPayload_asU8(),
      );
      if (interopPayload.getConfidential()) {
        const confidentialPayload =
          interopPayloadPb.ConfidentialPayload.deserializeBinary(
...
const getEndorsementsAndSignatoriesFromFabricView = (view) => {
  ...
  const endorsedProposalResponses =
    fabricView.getEndorsedProposalResponsesList();
  const serializedEndorsementsWithSignatories = [];
  for (let i = 0; i < endorsedProposalResponses.length; i++) {
    const endorsement = endorsedProposalResponses[i].getEndorsement();
    const endorsementSerializedBase64 = Buffer.from(
      endorsement.serializeBinary(),
    ).toString("base64");
    const sid = identitiesPb.SerializedIdentity.deserializeBinary(
      Uint8Array.from(Buffer.from(endorsement.getEndorser())),
    );
    serializedEndorsementsWithSignatories.push([
      sid.getMspid(),
      endorsementSerializedBase64,
    ]);
  }
...
const getSignatoryOrgMSPFromFabricEndorsementBase64 = (endorsementBase64) => {
  const endorsement = proposalResponsePb.Endorsement.deserializeBinary(
    Buffer.from(endorsementBase64, "base64"),
  );
  const sid = identitiesPb.SerializedIdentity.deserializeBinary(
    Uint8Array.from(Buffer.from(endorsement.getEndorser())),
  );
...
const isPatternAndAddressMatch = (
  pattern: string,
  address: string,
): boolean => {
 ...
 **/
const getPolicyCriteriaForAddress = async (
  contract: Contract,
  address: string,
): Promise<null | any> => {
  try {
    ...
    const [queryResponse, queryResponseError] = await helpers.handlePromise(
      contract.evaluateTransaction(
        "GetVerificationPolicyBySecurityDomain",
        parsedAddress.networkSegment,
      ),
    );
    if (queryResponseError) {
      throw new Error(
        `Error evaluating transaction GetVerificationPolicyBySecurityDomain ${queryResponseError}`,
      );
    }
    ...
    for (const item of verificationPolicy.identifiers) {
      ...
      if (
        ...
        (!matchingIdentifier ||
          item.pattern.length > matchingIdentifier.pattern.length)
      ) {
        matchingIdentifier = item;
      }
    }
 ...
 **/
const parseAndValidateView = async (
  contract: Contract,
  address: string,
  base64ViewProto: string,
  b64ViewContents: Array<string>,
): Promise<Buffer> => {
  try {
    const viewPayload = await contract.evaluateTransaction(
      "ParseAndValidateView",
      address,
      base64ViewProto,
      JSON.stringify(b64ViewContents),
    );
    return viewPayload;
  } catch (e) {
...
const interopFlow = async (
  ...
  if (interopArgIndices.length !== interopJSONs.length) {
    throw new Error(
      `Number of argument indices ${interopArgIndices.length} does not match number of view addresses ${interopJSONs.length}`,
    );
  }
  ...
  for (let i = 0; i < interopJSONs.length; i++) {
    ...
    if (requestResponseError) {
      throw new Error(
        `InteropFlow remote view request error: ${requestResponseError}`,
      );
    }
...
const addAppCCEndorsementPolicy = async (
  ...
  const appContract = new contractApi.ContractImpl(
    await gateway.getNetwork(invokeObject.channel),
    invokeObject.contractName,
    "",
  );
  const appDiscInterests = appContract.getDiscoveryInterests();
  appDiscInterests.forEach((interest) => {
    interopContract.addDiscoveryInterest(interest);
  });
  return interopContract;
};
...
const submitTransactionWithRemoteViews = async (
  ...
  interopContract = await addAppCCEndorsementPolicy(
    interopContract,
    invokeObject,
    gateway,
  );
...
const getRemoteView = async (
  ...
  const [relayResponse, relayResponseError] = await helpers.handlePromise(
    relay.ProcessRequest(
      ...
      keyCert.cert,
      Sign
        ? signMessage(computedAddress + uuidValue, keyCert.key.toBytes())
        : "",
      uuidValue,
...
const getSignatoryNodeFromCertificate = async (certBase64: string) => {
  ...
  if (
    !cert.subjects ||
    !(cert.subjects instanceof Array) ||
  ...
  const subjectComponents = cert.subjects[0].components.filter(
    (subject) => subject.name === "o",
  );
  if (!subjectComponents || subjectComponents.length === 0) {
    throw new Error(
      "No subject found in X.509 certificate with 'O' (Organization) attribute",
    );
  }
