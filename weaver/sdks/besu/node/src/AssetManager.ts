 ...
 **/
/** End file docs */
import * as assetLocksPb from "@hyperledger-cacti/cacti-weaver-protos-js/common/asset_locks_pb";
...
function createHybridAssetExchangeAgreementSerialized(
  assetType: string,
  assetID: string,
  assetData: string,
  numUnits: number,
  ...
  const assetExchangeAgreement =
    new assetLocksPb.HybridAssetExchangeAgreement();
  assetExchangeAgreement.setAssettype(assetType);
  assetExchangeAgreement.setId(assetID);
  assetExchangeAgreement.setRecipient(recipient);
...
function createAssetLockInfoSerialized(hash: Hash, expiryTimeSecs: number) {
  ...
  lockInfoHTLC.setHashbase64(
    Buffer.from(hash.getSerializedHashBase64(), "base64"),
  );
...
const createHTLC = async (
  ...
  const agreement = createAssetExchangeAgreementSerialized(
    assetType,
    assetID,
    "",
  ...
  let lockStatus = await assetManagerContract
    .lockNonFungibleAsset(appContract.address, agreement, lockInfo, {
      from: lockerAddress,
    })
...
const createFungibleHTLC = async (
  ...
  const agreement = createFungibleAssetExchangeAgreementSerialized(
    assetType,
    assetAmount,
    "",
    recipientAddress.slice(2),
  ...
  // Normal invoke function
  let lockStatus = await assetManagerContract
    .lockFungibleAsset(appContract.address, agreement, lockInfo, {
      from: lockerAddress,
    })
    .catch(function (e: any) {
      console.log(e);
...
const createHybridHTLC = async (
  ...
  const agreement = createHybridAssetExchangeAgreementSerialized(
    assetType,
    String(assetID),
    Web3.utf8ToHex(String(assetData)),
    assetAmount,
    recipientAddress.slice(2),
  );
  if (!agreement) {
  ...
  // Normal invoke function
  let lockStatus = await assetManagerContract
    .lockHybridAsset(appContract.address, agreement, lockInfo, {
      from: lockerAddress,
    })
    .catch(function (e: any) {
      console.log(e);
...
const claimAssetInHTLC = async (
  ...
  var claimStatus = await assetManagerContract
    .claimAsset(contractId, claimInfoStr, { from: lockerAddress })
    .catch(function (e: any) {
      console.log(e);
...
const isAssetLockedInHTLC = async (
  ...
  // Normal invoke function
  var lockStatus = await assetManagerContract
    .isAssetLocked(contractId)
    .catch(function (e: any) {
      console.log(e);
