...
import {
  DefaultApi as HtlcCoordinatorOpenCBDCApi,
  PluginFactoryHTLCCoordinatorOpenCBDC,
...
test(testCase, async (t: Test) => {
  ...
  const pluginOptions: IPluginHTLCCoordinatorOpenCBDCOptions = {
    instanceId: uuidv4(),
    logLevel,
    pluginRegistry,
  };
  ...
  pluginRegistry.add(pluginHTLCCoordinatorOpenCBDC);
  ...
  const htlcCoordinatorOpenCBDCApiClient = new HtlcCoordinatorOpenCBDCApi(
    configuration,
  );
  await pluginHTLCCoordinatorOpenCBDC.getOrCreateWebServices();
  await pluginHTLCCoordinatorOpenCBDC.registerWebServices(expressApp);

  const OpenCBDCWsApi = new SocketIoServer(server, {
    path: Constants.SocketIoConnectionPathV1,
  });
  const OpenCBDCConnectorConfiguration = new Configuration({ basePath: apiHost });
  const OpenCBDCConnectorApi = new BesuApi(OpenCBDCConnectorConfiguration);

  await connector.getOrCreateWebServices();
  await connector.registerWebServices(expressApp, OpenCBDCWsApi as any);
  ...
  const approveTokensOutput = await OpenCBDCConnectorApi.invokeContractV1({
    contractName: TestTokenJSON.contractName,
    keychainId,
  ...
  const responseBalance = await OpenCBDCConnectorApi.invokeContractV1({
    contractName: TestTokenJSON.contractName,
    keychainId,
    signingCredential: web3SigningCredential,
  ...
  const allowanceOutput = await OpenCBDCConnectorApi.invokeContractV1({
    contractName: TestTokenJSON.contractName,
    keychainId,
    signingCredential: web3SigningCredential,
    invocationType: EthContractInvocationType.Call,
  ...
  const response = await htlcCoordinatorOpenCBDCApiClient.ownHtlcV1(ownHTLCRequest);
  t.equal(response.status, 200, "response status is 200 OK");
  t.equal(response.data.success, true, "response success is true");
  t.ok(
    response.data,
    "pluginHTLCCoordinatorOpenCBDC.ownHtlcV1() output is truthy OK",
  );
  t.ok(
    response.data.out.transactionReceipt,
    "pluginHTLCCoordinatorOpenCBDC.ownHtlcV1() output.transactionReceipt is truthy OK",
  );

  t.comment("Get HTLC id");
  const responseTxId = await OpenCBDCConnectorApi.invokeContractV1({
  ...
  const response2 = await htlcCoordinatorOpenCBDCApiClient.counterpartyHtlcV1(
    counterpartyHTLCRequest,
  );
