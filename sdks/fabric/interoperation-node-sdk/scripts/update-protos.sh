...
then
    ...
    (cd protos && git fetch && git checkout tags/"$TAG")
else
    # if not, clone repo and checkout specified tag
    echo "Local protos directory doesn't exist, cloning..."
    (git clone https://github.ibm.com:dlt-interoperability/interop-protos.git protos && cd protos && git checkout tags/"$TAG")
fi
